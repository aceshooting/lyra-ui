...
field shows its localized label as a placeholder; file-input keeps its localized drop instruction
...
Tooltip triggers accept `focus-visible` for keyboard-visible focus without changing the existing
`hover focus` default. Fixed-position dropdowns escape transformed virtual-list rows into the
browser top layer when supported.
...
buttons for fenced code via `code-block-chrome`. Both keep their previous defaults. `lr-message-parts`
can opt into collapsed inline tool call/result disclosures with `tool-display="disclosure"`; its
default chip rendering remains the same.
