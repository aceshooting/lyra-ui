...
### `events-reemit-wrapper-missing-stoppropagation` — high

**Check.** A container/list component that attaches an inline listener to a wrapped child item's own bubbling `this.emit()` event and re-emits an enriched or renamed event to its own consumers must call `event.stopPropagation()` (or `stopImmediatePropagation()`) on the original event before or while re-emitting — otherwise the child's original event (`LyraElement.emit()`'s `bubbles: true, composed: true` defaults, `src/internal/lyra-element.ts`) keeps propagating past the container, reaching any consumer listening on the container tag itself, either as a duplicate of the same name (if the re-emit reuses it) or as an undocumented leak under the child's own name.

**Why.** Found live and fixed same-day in two components discovered independently of each other: `<lr-thread-list>`'s bare `lr-select` from `<lr-conversation-item>` double-fired past `<lr-thread-list>` (`detail: undefined` on the leaked copy, right behind the correctly-shaped re-emit) — fixed via commit `cc90b3a0`. `<lr-agent-workspace>`'s `onRetrievalSelect` handler mutated state and re-emitted `lr-retrieval-select` but never stopped `<lr-retrieval-results>`'s own bubbling `lr-select`, which leaked past `<lr-agent-workspace>` under the wrong, undocumented name — fixed via commit `a4c48258`, whose own commit message calls out "the same root cause" as the thread-list fix. Both fixes landed within minutes of each other from a concurrent sweep — this is an active, recurring shape, not a one-off.

**How to verify.** grep -rn '@lr-[a-z-]*=\$\{' packages/lyra-ui/src/components --include=*.class.ts for every inline listener attached to a nested `lr-*` child inside a `render()`/`renderRow()`/`renderItem()` method; for each hit, confirm the child's own handler (open its source) calls `this.emit(...)`, then confirm the container's listener calls `event.stopPropagation()` before or while it re-emits. Reference-quality fix already in the codebase: `packages/lyra-ui/src/components/agent-tools/evaluation-run/evaluation-run.class.ts:278,288` (`event.stopPropagation()` before re-emitting renamed events) and `packages/lyra-ui/src/components/retrieval/knowledge-base/knowledge-base.class.ts:410` (`@lr-row-click=${(e) => e.stopPropagation()}`).

**Known live violation (as of 2026-07-21).** `packages/lyra-ui/src/components/retrieval/retrieval-results/retrieval-results.class.ts:368` (`@lr-load-more=${() => this.emit('lr-load-more')}`, no `stopPropagation()`; source emit at `packages/lyra-ui/src/components/layout/virtual-list/virtual-list.class.ts:833`); `retrieval-results.class.ts:316` (`@lr-chunk-open=${(e) => this.emit('lr-chunk-open', e.detail)}`, no `stopPropagation()`; source emit at `packages/lyra-ui/src/components/retrieval/chunk-inspector/chunk-inspector.class.ts:176` — the leaked original can carry an `anchor` field the container's own event-map type at `retrieval-results.class.ts:26` omits); `packages/lyra-ui/src/components/layout/menu/menu.class.ts:687` (`@lr-menu-item-select=${this.onItemSelect}`, no `stopPropagation()`; source emit at `packages/lyra-ui/src/components/layout/menu/menu-item.class.ts:180`, inherited unchanged by `<lr-dropdown-item>`).

### `events-self-mutation-not-cancelable` — medium

**Check.** A component that mutates its own reactive state/DOM synchronously in direct response to a user action (add/remove/resize/reorder/toggle) and then fires only a plain, non-cancelable notification event should instead make that event `{ cancelable: true }` and check `.defaultPrevented` before applying the mutation — or expose an equivalent pending/hold + finalize/revert escape hatch — whenever a host plausibly needs to persist the change asynchronously and reject or roll it back on failure.

**Why.** `<lr-reorder-list>` shipped exactly this gap: moves were applied synchronously and `lr-reorder` fired only afterward, non-cancelable, so a host persisting the new order over the network had no supported way to hold or revert a move that failed to save. Fixed via commit `3c9f478d` by making `lr-reorder` cancelable (`preventDefault()` holds the move, surfaces a pending state on the affected item) plus new `finalizePendingMove()`/`revertPendingMove()` host methods, deliberately mirroring the already-established `lr-confirm-bar`/`lr-tool-approval-dialog` cancelable `lr-approve`/`lr-deny` + host-writable pending-state pattern.

**How to verify.** For a component that mutates its own state directly inside a user-triggered handler, grep the file for `cancelable` — zero hits alongside a synchronous state mutation is the violation shape. Contrast with `packages/lyra-ui/src/components/retrieval/memory-panel/memory-panel.class.ts:99-108`, which already gates its own mutation behind a pending-confirmation step before ever applying it.

**Known live violation (as of 2026-07-21).** `packages/lyra-ui/src/components/forms/token-input/token-input.class.ts` (`updateValue()`/`removeToken()`, ~lines 251-277, mutate `this.value` before emitting `lr-add`/`lr-remove`/`lr-token-edit`; zero occurrences of `cancelable` anywhere in the file); `packages/lyra-ui/src/components/data/table/table.class.ts:722-770` (column-resize mutates `resizedColumnWidths` before emitting `lr-column-resize`, non-cancelable); `packages/lyra-ui/src/components/layout/app-rail/app-rail.class.ts:482-565` (rail-resize/toggle/mode-change mutate `railWidthPx`/`open`/`mode` before emitting, non-cancelable).
...
### `theming-missing-gap-radius-token` — medium

**Check.** A size-tiered component whose internal layout sets `gap` and/or `border-radius` on its base part should expose those as dedicated `--lr-<component>-gap`/`--lr-<component>-radius` custom properties — following the same `:host([size="..."])`-reassignable pattern already used for padding/font-size — rather than requiring a `::part(base)` override to retune either.

**Why.** `lr-button` shipped `--lr-button-padding-block/-inline` and `--lr-button-font-size` as per-size tokens but left `gap`/`border-radius` only overridable via `::part(base)`; fixed via commit `76690c7d` by adding `--lr-button-gap`/`--lr-button-radius` (declared once on `:host`, matching that neither actually varies by size tier). The same gap — no dedicated token, `::part()` required to retune — exists unaddressed on multiple size-tiered siblings.

**How to verify.** grep a size-tiered component's `.styles.ts` for `gap:` and `border-radius:` — a literal value or a bare shared token (`var(--lr-radius-*)`/`var(--lr-space-*)`) with no component-specific custom-property indirection is the violation shape. Compare against `packages/lyra-ui/src/components/forms/button/button.styles.ts:32-33,99,102` as the fixed reference.

**Known live violation (as of 2026-07-21).** `packages/lyra-ui/src/components/forms/input/input.styles.ts:63` (gap), `:72` (border-radius); `packages/lyra-ui/src/components/forms/select/select.styles.ts:71` (gap), `:78` (border-radius); `packages/lyra-ui/src/components/forms/combobox/combobox.styles.ts:82,125,261` (gap), `:91,130,234,265` (border-radius) — all three have neither token. Lower-priority instances (radius only; `lr-chip` already has `--lr-chip-gap`, and pill-shaped chrome makes corner-radius retuning a rarer ask): `packages/lyra-ui/src/components/overlays/chip/chip.styles.ts:117,200` (`border-radius: var(--lr-radius-pill)` hardcoded); `packages/lyra-ui/src/components/overlays/badge/badge.styles.ts:13` (same, `lr-tag` inherits unchanged); `packages/lyra-ui/src/components/forms/icon-button/icon-button.styles.ts:7` (`border-radius: var(--lr-radius)` hardcoded; icon-button has no `size` attribute at all, so this instance is radius-only, not a full size-tier gap).
