...
test('keeps the checked-in profile authority on the canonical fixture contract', async () => {
  ...
  // Current versions are generated by upgrades; bind them independently to the development
  // manifest while keeping the curated floors, toolchain, and profile topology pinned.
  expected.currentVersions = Object.fromEntries(
    Object.keys(CURRENT_VERSIONS).map((name) => [
      name,
