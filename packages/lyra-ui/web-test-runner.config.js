...
export default {
  ...
  coverageConfig: {
    ...
    // per-commit/per-PR delta reporting stays `informational: true` in
    ...
    // floor(measured - 1.5) after a coverage run. Hand-written literals here
    // drifted twice: commit fb700ac3 widened every metric to a 1% sanity
    // check, and the values that replaced it (75/65/65/75) sat 24-34 points
    // under the real measurement -- a comment claiming they "match this repo's
    // actual coverage with headroom" while thousands of functions could have
    // gone uncovered before the gate fired. Refreshing is now a command with a
    // reviewable diff, and lowering a floor needs an explicit `--allow-lower`.
