...
// A component's generated llms/components/<tag>.md is built from ONLY its own section text (see
// buildComponentFile) -- never the family preamble and never a different tag's own section -- so a
// positional "see the X note above/below" reference that reads fine in the authored llms/<family>.md
// can still dangle once split. findDanglingPositionalReferences() is the fail-closed generator lint
// against exactly that regression; build() itself must refuse to build when one survives.
const [resolvedTagReferenceSection] = splitSections(
  '## `lr-owner`\n\n' +
    'See `<lr-owner>`\'s own note above for the guard snippet.\n\n' +
    'Guard snippet: filter on `event.target`.\n',
);
assert.deepEqual(
  findDanglingPositionalReferences(resolvedTagReferenceSection),
  [],
  'a positional reference to the same tag\'s own section is never dangling',
);
const [danglingTagReferenceSection] = splitSections(
  '## `lr-widget`\n\n' +
    'See `<lr-other-widget>`\'s own note above for the guard snippet.\n',
);
assert.deepEqual(
  findDanglingPositionalReferences(danglingTagReferenceSection),
  [
    '`lr-widget`: "<lr-other-widget>`\'s own note above" points above at `lr-other-widget`\'s own ' +
      'section, which is a different section and will not be present in this component\'s ' +
      'generated llms/components/*.md file. Inline the referenced text or link to the published ' +
      'llms/components/lr-other-widget.md instead of a positional "above".',
  ],
  'a positional reference naming a different tag\'s own section is dangling once split',
);
// A third shape carries no "above"/"below" at all: "this file's `<lr-other>` ... section" reads
// fine while the whole authored llms/<family>.md is open, but is exactly as dangling once split --
// this is the exact rewording (dropping the literal word "above") that let one instance of shape 1
// slip past review once already.
const [resolvedSameFileSectionSection] = splitSections(
  '## `lr-owner`\n\n' +
    'The full semantics are in this file\'s `lr-owner` section\'s own **Properties** list.\n',
);
assert.deepEqual(
  findDanglingPositionalReferences(resolvedSameFileSectionSection),
  [],
  'a "this file\'s ... section" reference naming the same tag\'s own section is never dangling',
);
const [danglingSameFileSectionSection] = splitSections(
  '## `lr-widget`\n\n' +
    'The full semantics are in this file\'s `lr-other-widget` section\'s own **Properties** list.\n',
);
assert.deepEqual(
  findDanglingPositionalReferences(danglingSameFileSectionSection),
  [
    '`lr-widget`: "this file\'s `lr-other-widget` section" points at `lr-other-widget`\'s own ' +
      ...
      'text or link to the published llms/components/lr-other-widget.md instead.',
  ],
  'a "this file\'s ... section" reference naming a different tag\'s own section is dangling once split',
);
const [resolvedNoteReferenceSection] = splitSections(
  '## `lr-widget`\n\n' +
    'The gizmo prop has an unusual default; see the gizmo-defaulting note below.\n\n' +
    'Gizmo defaulting: the default is derived from the ambient locale.\n',
);
assert.deepEqual(
  findDanglingPositionalReferences(resolvedNoteReferenceSection),
  [],
  'a positional note reference whose key word recurs later in the same section is resolved',
);
const [danglingNoteReferenceSection] = splitSections(
  '## `lr-widget`\n\n' +
    'The widget renders eagerly; see the torque-calibration note below.\n',
);
assert.deepEqual(
  findDanglingPositionalReferences(danglingNoteReferenceSection),
  [
    '`lr-widget`: "the torque-calibration note below" refers below to a note/section whose key ' +
      'word(s) [torque, calibration] appear nowhere else in this section -- inline the referenced ' +
      'text instead of a positional "below".',
  ],
  'a positional note reference whose key word appears nowhere else is dangling',
);
for (const [family] of FAMILIES) {
  for (const section of splitSections(readFileSync(new URL(`../llms/${family}.md`, import.meta.url), 'utf8'))) {
    assert.deepEqual(
      findDanglingPositionalReferences(section),
      [],
      `llms/${family}.md "${section.title}" must carry no positional reference that dangles once split`,
    );
  }
}
