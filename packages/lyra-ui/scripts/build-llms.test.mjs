...
// The two shared scrollbar tokens (--lr-scrollbar-width, --lr-scrollbar-gutter) each carry a
// themeInput, so buildTokens() buckets them as `rows` (shared + themeInput), not `plain` --
// which is why this count is 265 while the derived/fixed count below stays 77.
assert.match(tokens, /## Direct theme-backed tokens \(265\)/);
