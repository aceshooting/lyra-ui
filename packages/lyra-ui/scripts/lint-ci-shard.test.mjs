...
test('partitions all 95 current lint command occurrences exactly once', () => {
  ...
  assert.equal(inventory.filter((item) => item.source === 'contract-policy').length, 92);
  assert.deepEqual(
  ...
  assert.deepEqual(lanes.map((lane) => lane.totalWeight), [174, 173, 173]);
