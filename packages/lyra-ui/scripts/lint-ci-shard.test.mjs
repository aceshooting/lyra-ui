...
test('partitions all 96 current lint command occurrences exactly once', () => {
  ...
  assert.equal(inventory.filter((item) => item.source === 'contract-policy').length, 93);
  assert.deepEqual(
  ...
  assert.deepEqual(lanes.map((lane) => lane.totalWeight), [174, 174, 173]);
...
test('assigns unknown valid commands and preserves repeated occurrences by ordinal', () => {
  ...
  assert.equal(inventory.length, 98);
