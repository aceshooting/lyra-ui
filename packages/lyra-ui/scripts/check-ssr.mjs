...
assert.equal(
  globalThis.window,
  undefined,
  'SSR gate must run without a window shim'
);
assert.equal(
  globalThis.document,
  undefined,
  'SSR gate must run without a document shim'
);
...
const populatedFlowEdges = [
  { id: 'fetch-answer', source: 'fetch', target: 'answer' },
];
...
assert.match(
  populatedFlowCanvasHtml,
  /data-node-id="fetch"/,
  'populated lr-flow-canvas SSR lost its first node'
);
assert.match(
  populatedFlowCanvasHtml,
  /data-edge-id="fetch-answer"/,
  'populated lr-flow-canvas SSR lost its edge'
);
...
assert.match(
  ...
  /data-cell-id="summary"/,
  'populated lr-dashboard-grid SSR lost its cell'
);
assert.doesNotMatch(
  populatedDashboardHtml,
...
assert.match(
  populatedTableHtml,
  /Alpha\.pdf/,
  'populated lr-table SSR lost its row content'
);
assert.doesNotMatch(
  populatedTableHtml,
  /<(?:div|p|span)[^>]*part="empty"/,
...
assert.match(
  ...
  /Alpha\.pdf/,
  'populated lr-document-library SSR lost its document row'
);
assert.doesNotMatch(
  populatedDocumentLibraryHtml,
  /<(?:div|p|span)[^>]*part="empty"/,
...
for (const count of [199, 200, 201]) {
  const emojiHtml = await collectResult(
    render(
      html`<lr-emoji-picker .groups=${emojiGroups(count)}></lr-emoji-picker>`,
      {
        elementRenderers: animatedImageContext.elementRenderers,
      }
    )
  ...
  assert.match(
    emojiHtml,
    /part="emoji"/,
    `lr-emoji-picker SSR lost options at the ${count}-item threshold`
  );
  assert.doesNotMatch(
    emojiHtml,
    /<(?:div|p|span)[^>]*part="empty"/,
...
assert.match(
  populatedVirtualListHtml,
  /data-row-index="0"/,
  'populated lr-virtual-list SSR emitted no initial row window'
);
assert.match(
  populatedVirtualListHtml,
  /Alpha/,
  'populated lr-virtual-list SSR lost its first item'
);
...
assert.match(
  populatedQrHtml,
  /part="loading"/,
  'populated lr-qr-code SSR must serialize an honest pending state'
);
assert.doesNotMatch(
  populatedQrHtml,
  /part="empty"/,
  'populated lr-qr-code SSR must not claim it has no data'
);
...
assert.equal(
  [...populatedVideoPlaylistHtml.matchAll(/<button[^>]*part="playlist-item"/g)]
    .length,
  2,
...
const populatedResultFieldHtml = await collectResult(
  render(
    html`<lr-result-field label="Status" value="fallback"
      ><span data-rich-value>Live</span></lr-result-field
    >`,
    { elementRenderers: animatedImageContext.elementRenderers }
  )
);
...
const populatedResultCardHtml = await collectResult(
  render(
    html`<lr-result-card with-actions
      ><button slot="actions">Copy</button>Body</lr-result-card
    >`,
    { elementRenderers: animatedImageContext.elementRenderers }
  )
);
...
assert.match(
  populatedTabGroupHtml,
  /Panel one/,
  'populated lr-tab-group SSR lost its first fallback panel'
);

const populatedMultiSplitHtml = await collectResult(
  render(
    html`<lr-multi-split .sizes=${[50, 50]}
      ><section>One</section>
...
const progressiveSlotCases = [
  {
    tag: 'lr-badge',
    template: html`<lr-badge
      ><span slot="start">●</span>Ready<span slot="end">✓</span></lr-badge
    >`,
    parts: ['start', 'end'],
  },
  {
    tag: 'lr-tag',
    template: html`<lr-tag
      ><span slot="start">●</span>Alpha<span slot="end">✓</span></lr-tag
    >`,
    parts: ['start', 'end'],
  },
  {
  ...
  {
    tag: 'lr-empty',
    template: html`<lr-empty
      ><span>Illustration</span><strong slot="heading">No results</strong
  ...
  {
    tag: 'lr-lightbox',
    template: html`<lr-lightbox open
      ><button slot="actions">Download</button></lr-lightbox
    >`,
    parts: ['actions'],
  },
  {
    tag: 'lr-model-select',
    template: html`<lr-model-select
      ><span slot="label">Model</span><span slot="error">Choose a model</span
  ...
  {
    tag: 'lr-voice-picker',
    template: html`<lr-voice-picker
      ><span slot="label">Voice</span><span slot="error">Choose a voice</span
  ...
  {
    tag: 'lr-option',
    template: html`<lr-option value="alpha"
      ><span slot="start">&#x25cf;</span>Alpha<span slot="end"
        >Primary</span
      ></lr-option
    >`,
    parts: ['start', 'end'],
  },
  {
    ...
    template: html`<lr-input>
      <span slot="label">Name</span><span slot="start">@</span
  ...
  },
  {
    ...
    template: html`<lr-breadcrumb-item
      ><span slot="start">&#x2302;</span>Home<span slot="end"
        >1</span
      ></lr-breadcrumb-item
    >`,
    parts: ['start', 'end'],
  },
  {
    ...
    template: html`<lr-file-input>
      <span slot="label">Attachments</span
...
for (const { tag, template, parts } of progressiveSlotCases) {
  const markup = await collectResult(
    render(template, {
      elementRenderers: animatedImageContext.elementRenderers,
    })
  );
...
for (const [tag, template, overflowPart] of [
  [
    'lr-chip-group',
    html`<lr-chip-group max-visible="2"
  ...
  [
    'lr-avatar-group',
    html`<lr-avatar-group max="2"
      ><lr-avatar label="One"></lr-avatar><lr-avatar label="Two"></lr-avatar
...
function serializedAttributeCount(markup, attribute, value) {
  return (
    markup.match(new RegExp(`\\s${attribute}="${value}"`, 'g'))?.length ?? 0
  );
}
...
for (const [attribute, value] of ratingHostSemantics) {
  ...
  assert.equal(
    ...
    1,
    `lr-rating SSR must serialize ${attribute}=${JSON.stringify(
      value
    )} on its one host owner only`
  );
}
assert.doesNotMatch(
  ratingOpeningTag,
...
assert.deepEqual(
  declaredTags,
  inventoryTags,
  'SSR support matrix must classify every inventory tag once'
);
assert.equal(
  new Set(declaredTags).size,
  declaredTags.length,
  'SSR support tiers must be disjoint'
);
assert.ok(
...
for (const tag of [
  ...
  assert.equal(
    loader.getLyraSsrMode(tag),
    'render-and-hydrate',
    `${tag} must render on the server`
  );
}
...
for (const entry of entries) {
  ...
  assert.match(
    entry.html,
    new RegExp(`<${entry.tag}\\b`),
    `${entry.tag}: server output lost its host`
  );
  assert.match(
    entry.html,
    /data-ssr-probe="lr-[^"]+"/,
    `${entry.tag}: host attributes were not serialized`
  );
  assert.match(
    entry.html,
    new RegExp(`data-ssr-light="${entry.tag}"`),
    `${entry.tag}: light DOM was not preserved`
  );
...
assert.equal(
  new Set(
    publicStateCases.map(
      ({ tag, attribute, value }) => `${tag}\0${attribute}\0${value}`
    )
  ).size,
...
for (const stateCase of publicStateCases) {
  const label = `${stateCase.tag}[${stateCase.attribute}=${JSON.stringify(
    stateCase.value
  )}]`;
  try {
    const stateHtml = await renderSsrStateProbe(
      stateCase,
      animatedImageContext.elementRenderers
    );
    assert.match(
...
if (publicStateFailures.length > 0) {
  assert.fail(
    `public SSR state matrix failed:\n${publicStateFailures.join('\n\n')}`
  );
}
...
assert.equal(
  page?.mode,
  'render-and-hydrate',
  'lr-page is a required SSR/hydration fixture'
);
assert.match(
  page.html,
  /part="base page"/,
  'lr-page SSR output is missing its semantic base'
);
...
assert.equal(
  fileInput?.mode,
  'render-and-hydrate',
  'lr-file-input must remain in the SSR render tier'
);
assert.match(
  fileInput.html,
  /part="[^"]*\bfile-input\b[^"]*"/,
  'lr-file-input did not complete its server render'
);
assert.match(
  fileInput.html,
  /part="[^"]*\bform-control\b[^"]*"/,
  'lr-file-input SSR output is missing form-control chrome'
);
assert.match(
  fileInput.html,
  /part="error"/,
  'lr-file-input SSR output is missing its error surface'
);
...
const fileInputErrorHtml = await renderSsrStateProbe(
  {
    ...
    value: 'Choose a supporting document.',
  },
  animatedImageContext.elementRenderers
);
assert.match(
  fileInputErrorHtml,
  /id="file-input-error"/,
  'lr-file-input SSR error text is missing its owned id'
);
assert.match(
  fileInputErrorHtml,
  /part="error"/,
  'lr-file-input SSR error text is missing its public part'
);
assert.match(
  fileInputErrorHtml,
  /Choose a supporting document\./,
  'lr-file-input SSR error text did not render'
);
assert.doesNotMatch(
  fileInputErrorHtml,
...
assert.equal(
  globalThis.window,
  undefined,
  'server rendering must not install a window shim'
);
assert.equal(
  globalThis.document,
  undefined,
  'server rendering must not install a document shim'
);
