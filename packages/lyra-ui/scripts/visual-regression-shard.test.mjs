...
test('builds the complete retained-baseline/evidence plan without changing enrollment', () => {
  ...
  assert.equal(new Set(captures.map(({ story }) => story.id)).size, 109);
  assert.equal(captures.length, 313);
  assert.equal(evidenceOnly.length, 202);
  assert.equal(captures.length - evidenceOnly.length, 111);
});
...
test('creates deterministic, disjoint, exhaustive, balanced capture shards', () => {
  ...
  assert.deepEqual(shards.map((shard) => shard.length), [105, 104, 104]);
