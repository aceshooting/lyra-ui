...
function buildRootTokensCss(source) {
  ...
  const overrides = (mode, indent) => ROOT_TOKEN_SUBSET
    ...
    .join('\n');
  // Every scope the mode rules above declare into, INCLUDING the compound OS-dark route. Blocks
  // that come later in the same layer only win by tying specificity, and the dark route's
  // `:root:not(.lr-light):not([data-lr-theme='light'])` is 0-3-0 against a bare `:root`'s 0-1-0 --
  // so omitting that arm let the OS-dark route swallow the whole forced-colors and
  // prefers-reduced-motion sets for every visitor whose OS is dark with no explicit scope. Each arm
  // is matched and weighed on its own, so repeating the compound form costs the other three
  // audiences (OS-light `:root`, an explicit light scope, an explicit dark scope) nothing: it
  // simply does not match them, and their own arm still ties and wins on source order.
