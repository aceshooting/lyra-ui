...
const baseTokens = css`
  :host {
    ...
    /* Opt-in theme-level scrollbar hooks. An internal scroll container reads these -- default
       'auto' matches the property's own CSS initial value, so a container that never declared
       scrollbar-width/-gutter before renders identically until a consumer sets the --lr-theme-*
       input on an ancestor. A container that already hardcoded a different literal (carousel,
       scroller, time-input, emoji-picker) reads --lr-theme-scrollbar-width/-gutter directly with
       that literal as its own fallback instead of this pair, so its own default is unaffected. */
    --lr-scrollbar-gutter: var(--lr-theme-scrollbar-gutter, auto);
    --lr-scrollbar-width: var(--lr-theme-scrollbar-width, auto);
    ...
    /* The one pointer-interaction transition. Ninety-odd rules across the library had each
       ...
       one line. A resting rule now writes transition: var(--lr-transition-interactive) and gets
