import { resolveLyraString as resolveRuntimeLyraString } from './localization-runtime.js';
...
const DEFAULT_STRINGS: Record<LyraMessageKey, LyraMessage> = {
  ...
  sequenceStripBucketLabel: '{label}, items {start} to {end}',
  ...
  fileInputRejectedMaxFiles: '{filename}: the maximum number of files has been reached.',
  fileInputRejectedMaxTotalSize: '{filename}: adding this file would exceed the total size limit.',
  ...
  mentionResultCount: { one: '{count} suggestion', other: '{count} suggestions' },
  ...
  approvalQueueLimit: 'Only the first {count} requests are shown.',
  ...
  toastContentIncomplete: 'Notification with incomplete content',
  toastOverflow: 'Notifications not shown: {count}.',
  ...
  matchMismatch: 'The values do not match.',
  ...
  cellHighlightWithLabel: 'Highlight: {value} — {label}',
  ...
  policySummaryLimit: 'Only the first {count} decisions are shown.',
  ...
  toolCallBlockHeaderPending: 'Waiting to use {name}',
  toolCallBlockHeaderRunning: 'Using {name}',
  toolCallBlockHeaderSuccess: 'Used {name}',
  toolCallBlockHeaderError: 'Failed to use {name}',
  toolCallBlockHeaderDenied: 'Use of {name} denied',
  toolCallBlockArgumentsLabel: 'Arguments',
  toolCallBlockResultLabel: 'Result',
  ...
  contextInspectorLimit: 'Only the first {count} segments are shown.',
  ...
  evaluationRunExampleLimit: 'Only the first {count} examples are shown.',
...
/**
 * Resolve a message through the public full-catalog entry. Component classes use the lean runtime
 * directly with their generated per-class defaults, keeping this complete compatibility catalog
 * out of granular component bundles.
 */
