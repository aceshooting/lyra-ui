...
describe("lr-diff-view", () => {
  ...
  it("renders the English size fallback with no locale registered", async () => {
    ...
    expect(
      el.shadowRoot!.querySelector('[part="limit"]')!.textContent
    ).to.equal("Diff is too large to display.");
  });
  ...
  it("defaults maxLines to 5000 and leaves ordinary diffs unchanged", async () => {
    ...
    expect(el.shadowRoot!.querySelectorAll('[part="limit"]').length).to.equal(
      0
    );
  ...
  it("represents an empty document as zero lines while preserving real trailing newlines", async () => {
    ...
    try {
      ...
      expect(
        empty.shadowRoot!.querySelectorAll('[part="line"]').length
      ).to.equal(0);
      const copied = oneEvent(empty, "lr-copy");
      (
        empty.shadowRoot!.querySelector(
          '[part="copy-button"]'
    ...
    expect(
      addition.shadowRoot!.querySelectorAll('[data-type="remove"]').length
    ).to.equal(0);
    expect(
      addition.shadowRoot!.querySelectorAll('[data-type="add"]').length
    ).to.equal(1);
    ...
    const types = [
      ...trailing.shadowRoot!.querySelectorAll('[part="line"]'),
    ].map((line) => line.getAttribute("data-type"));
  ...
  it("rejects aggregate character and comparison work beyond the hard resource budget", async () => {
    ...
    expect(
      characterLimited.shadowRoot!.querySelectorAll('[part="limit"]').length
    ).to.equal(1);
    ...
    expect(
      workLimited.shadowRoot!.querySelectorAll('[part="limit"]').length
    ).to.equal(1);
    expect(
      workLimited.shadowRoot!.querySelectorAll('[part="line"]').length
    ).to.equal(0);
  });
  ...
  it("enforces the default ceiling and accepts explicit Infinity as the documented opt-out", async () => {
    ...
    expect(
      limited.shadowRoot!.querySelector('[part="limit"]')!.textContent
    ).to.equal("Too large");
    ...
    expect(
      unbounded.shadowRoot!.querySelectorAll('[part="limit"]').length
    ).to.equal(0);
    expect(
      unbounded.shadowRoot!.querySelectorAll('[part="line"]').length
    ).to.be.greaterThan(0);
  });
  ...
  it("renders no copy button by default, one when copyable is set", async () => {
    ...
    expect(plain.shadowRoot!.querySelector('[part="copy-button"]') == null).to
      .be.true;
  ...
  it("reports a synchronous clipboard failure without falsely confirming success", async () => {
    ...
    try {
      ...
      let errors = 0;
      let failure:
        | { ok: false; text: string; reason: string; error: unknown }
        | undefined;
    ...
    try {
      ...
      await waitUntil(
        () =>
          el
            .shadowRoot!.querySelector('[part="copy-button"]')!
            .textContent!.trim() === "Échec de la copie"
      );
  ...
  it("does not recompute the diff when only the copy-confirmation state toggles, only when oldText/newText change", async () => {
    ...
    (
      el.shadowRoot!.querySelector('[part="copy-button"]') as HTMLButtonElement
    ).click();
    await el.updateComplete;
    expect((el as unknown as { diffOps: LyraDiffOp[] }).diffOps).to.equal(
      opsBefore
    );
    ...
    el.newText = "a\nY";
    await el.updateComplete;
    expect((el as unknown as { diffOps: LyraDiffOp[] }).diffOps).to.not.equal(
      opsBefore
    );
  });
});
...
describe("layout", () => {
  ...
  it("placeholder cells in split layout carry no +/- prefix", async () => {
    ...
    const oldSide = el.shadowRoot!.querySelector(
      '[part="side"][data-side="old"]'
    )!;
  ...
  it("copies the same unified text regardless of layout", async () => {
    ...
    try {
      ...
      (
        unified.shadowRoot!.querySelector(
          '[part="copy-button"]'
        ) as HTMLButtonElement
      ).click();
      (
        split.shadowRoot!.querySelector(
          '[part="copy-button"]'
        ) as HTMLButtonElement
      ).click();
      const [unifiedEvent, splitEvent] = await Promise.all([
        unifiedCopy,
        splitCopy,
      ]);
  ...
  it("paints a changed plain-text line through its full horizontal overflow width", async () => {
    ...
    expect(getComputedStyle(line).backgroundColor).to.not.equal(
      "rgba(0, 0, 0, 0)"
    );
  });
});
...
describe("syntax highlighting", () => {
  ...
  it("parses highlighted markup with the adopted owner DOMParser", async () => {
    ...
    const el = (await fixture(
      html`<lr-diff-view></lr-diff-view>`
    )) as LyraDiffView;
    const tokenize = (
      el as unknown as {
        tokenizeLines(
          highlighter: { codeToHtml(text: string): string },
          text: string,
          lang: string
        ): string[] | null;
      }
    ...
    try {
      ...
      const lines = tokenize(
        {
          codeToHtml: (text: string) =>
            `<pre><code><span class="line">${text}</span></code></pre>`,
        },
        "owner",
        "text"
      );
  ...
  it('tokenized line count equals the plain split("\\n") line count, including a trailing newline', async () => {
    ...
    expect(
      (el as unknown as { highlightedOldLines: string[] | null })
        .highlightedOldLines?.length
    ).to.equal(expectedLineCount);
  });
  ...
  it("paints a highlighted changed line through its full horizontal overflow width", async () => {
    ...
    await waitUntil(
      () =>
        el.shadowRoot!.querySelector(
          '[part="line"][data-type="add"] span[style]'
        ) != null,
...
describe("contextLines", () => {
  ...
  it("does not fold anything when contextLines is unset, regardless of run length", async () => {
    const el = (await fixture(
      html`<lr-diff-view .oldText=${old8} .newText=${new8}></lr-diff-view>`
    )) as LyraDiffView;
    expect(el.shadowRoot!.querySelector('[data-type="fold"]') == null).to.be
      .true;
    expect(el.shadowRoot!.querySelectorAll('[part="line"]').length).to.equal(
      10
    );
  });

  for (const contextLines of [-1, Number.NaN, Number.POSITIVE_INFINITY]) {
    it(`treats contextLines=${String(
      contextLines
    )} as unset instead of enabling zero-context folding`, async () => {
      const el = (await fixture(
        html`<lr-diff-view
          .oldText=${old8}
          .newText=${new8}
  ...
  it("locale-formats the hidden-line count before interpolating it into the localized message", async () => {
    const oldText = [
      "a",
      ...Array.from({ length: 1236 }, (_, index) => `same-${index}`),
      "z",
    ].join("\n");
    const newText = [
      "A",
  ...
  it("folds equivalently in split layout, one fold marker per side at the same position", async () => {
    ...
    const newSide = el.shadowRoot!.querySelector(
      '[part="side"][data-side="new"]'
    )!;
    ...
    expect(
      oldSide.querySelector('[data-type="fold"]')!.textContent!.trim()
    ).to.equal("2 unchanged lines");
  });
...
describe("RTL / bidi isolation", () => {
  ...
  it("locks a code line to ltr under an RTL ancestor, but leaves the localized fold marker following ambient direction", async () => {
    ...
    const codeLine = el.shadowRoot!.querySelector(
      '[part="line"][data-type="equal"]'
    ) as HTMLElement;
    expect(getComputedStyle(codeLine).direction).to.equal("ltr");

    const foldLine = el.shadowRoot!.querySelector(
...
describe("back-compat", () => {
  it("default (unified, no language) output is byte-identical to today", async () => {
    ...
    const lines = [...el.shadowRoot!.querySelectorAll('[part="line"]')].map(
      (l) => l.textContent
    );
