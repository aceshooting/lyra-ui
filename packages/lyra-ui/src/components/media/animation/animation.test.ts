...
it("rebinds animation, motion, styles, and visibility to the adopted realm and ignores stale observers", async () => {
  ...
  try {
    ...
    expect(frameIo.observedTargets.at(-1)?.ownerDocument === frameDocument).to
      .be.true;
...
it("routes preset geometry through the inheritable public hook before its private default", async () => {
  const wrapper = await fixture<HTMLElement>(html`
    <div style="--lr-animation-slide-distance: 23px">
      <lr-animation name="slide-in-start" iterations="1"
        ><p>inherited</p></lr-animation
      >
      <lr-animation
        name="slide-in-start"
        iterations="1"
        style="--lr-animation-slide-distance: 17px"
        ><p>direct</p></lr-animation
      >
    </div>
  `);
...
it("owns a bounded readonly threshold snapshot and retains only finite values from 0 through 1", () => {
  ...
  Object.defineProperty(mixed, 0, {
    configurable: true,
    get(): never {
      throw new Error("hostile threshold entry");
    },
  });
...
it("falls back to safe IntersectionObserver options when rootMargin is invalid and normalizes thresholds", async () => {
  ...
  try {
    ...
    let attempts = 0;
    class ValidatingObserver extends (OriginalObserver as unknown as {
      new (
        callback: IntersectionObserverCallback,
        options?: IntersectionObserverInit
      ): IntersectionObserver;
      ...
      constructor(
        ...
        attempts += 1;
        if (options?.rootMargin === "not-a-margin")
          throw new SyntaxError("invalid root margin");
        super(callback, options);
      }
    }
    window.IntersectionObserver =
      ValidatingObserver as unknown as typeof IntersectionObserver;
    document.body.append(el);
    try {
      await el.updateComplete;
...
it("fails open and plays immediately when the environment provides no IntersectionObserver support", async () => {
  ...
  try {
    ...
    expect(
      (el as unknown as { visibilityObserver?: IntersectionObserver })
        .visibilityObserver
    ).to.be.undefined;
  } finally {
    (
...
it("fails open and plays immediately when even the safe-defaults fallback IntersectionObserver construction throws", async () => {
  ...
  class AlwaysThrowingObserver {
    constructor() {
      throw new Error(
        "constructor always fails, including with safe-default options"
      );
    }
    observe(): void {}
    unobserve(): void {}
  ...
  (
    window as unknown as { IntersectionObserver: unknown }
  ).IntersectionObserver = AlwaysThrowingObserver;
  try {
    const el = (await fixture(html`
      <lr-animation name="fade-in" play-on-visible iterations="1">
        <p>content</p>
...
it("fails open and plays immediately when the constructed IntersectionObserver throws from observe()", async () => {
  ...
  (
    window as unknown as { IntersectionObserver: unknown }
  ).IntersectionObserver = ThrowsOnObserve;
  try {
    const el = (await fixture(html`
      <lr-animation name="fade-in" play-on-visible iterations="1">
        <p>content</p>
...
it("does not throw and stays inert when there is no slotted target to animate (currentTime/play-on-visible/start all no-op the Animation half)", async () => {
  const el = (await fixture(
    html`<lr-animation name="fade-in" play play-on-visible></lr-animation>`
  )) as LyraAnimation;
  await el.updateComplete;
...
it("cancel() leaves the target reverted, not re-frozen on the first keyframe by the play-state sync", async () => {
  ...
  expect(
    ...
    "the fixture must actually be animating"
  ).to.equal(1);
  ...
  expect(
    target.getAnimations().length,
    "a canceled animation must stay idle"
  ).to.equal(0);
...
it("publishes the complete mirrored animation/easing catalogs and resolves every animation name", async () => {
  ...
  expect(
    Object.keys(animations).filter((name) => name !== "easings")
  ).to.deep.equal(names);
...
it("animates owner-realm SVG and MathML elements from the default slot", async () => {
  const svgEl = (await fixture(html`
    <lr-animation name="fadeIn" iterations="1"
  ...
  const mathEl = (await fixture(html`
    <lr-animation name="fadeIn" iterations="1"
      ><math><mi>x</mi></math></lr-animation
    >
  `)) as LyraAnimation;
...
it("a null registry override disables visible motion but preserves start/finish lifecycle", async () => {
  ...
  try {
    ...
    expect(
      target
        .getAnimations()
        .every((item) => item.effect!.getComputedTiming().duration === 0)
    ).to.be.true;
  } finally {
    cleanup();
    el.remove();
...
it("play-on-visible: observes the slotted target and starts playback once it intersects, then auto-disconnects (repeat defaults to false)", async () => {
  ...
  try {
    ...
    expect(
      latest.disconnected,
      "a single-shot observer disconnects itself after the first intersect"
    ).to.be.true;
  } finally {
    io.restore();
  }
...
it("play-on-visible-repeat: keeps observing and toggles `play` on subsequent enter/leave notifications", async () => {
  ...
  try {
    ...
    expect(
      latest.disconnected,
      "a repeating observer stays connected after intersecting"
    ).to.be.false;
...
it("play-on-visible: passes a custom Element `root` through, and re-observes (disconnecting the previous observer) when rootMargin changes while connected", async () => {
  ...
  try {
    ...
    expect(
      second.disconnected,
      "disconnectedCallback should tear down the still-active observer"
    ).to.be.true;
  } finally {
    io.restore();
    rootEl.remove();
