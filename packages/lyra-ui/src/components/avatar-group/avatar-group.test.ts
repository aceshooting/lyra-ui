...
// `<lyra-avatar-group>` living in its shadow DOM via a forwarding `<slot>` --
...
// flattens to (the real projected `<lyra-avatar>`s).
class AvatarGroupForwarder extends LitElement {
  protected createRenderRoot() {
    return this.attachShadow({ mode: 'open' });
  }
  protected render() {
    return litHtml`<lyra-avatar-group max="2"><slot></slot></lyra-avatar-group>`;
  }
}
...
it('reflects size/shape/tone as attributes for CSS selectors', async () => {
  const el = (await fixture(html`
    <lyra-avatar-group size="lg" shape="square" tone="brand">
      <lyra-avatar></lyra-avatar>
    </lyra-avatar-group>
  `)) as LyraAvatarGroup;
...
describe('dynamic children', () => {
  it('recomputes overflow when an avatar is appended after first render', async () => {
    ...
    const extra = document.createElement('lyra-avatar');
  ...
  it('recomputes overflow when an avatar is removed after first render', async () => {
    ...
    const avatars = Array.from(el.querySelectorAll('lyra-avatar'));
    avatars.slice(2).forEach((a) => el.removeChild(a));
    await new Promise((resolve) => requestAnimationFrame(resolve));
    await el.updateComplete;

    expect(el.shadowRoot!.querySelector('[part="overflow-badge"]')).to.not.exist;
    const remaining = Array.from(el.querySelectorAll('lyra-avatar')) as HTMLElement[];
...
it('reconciles childCount correctly through a forwarding <slot> (children.length under-counts), without a redundant explicit resync alongside it', async () => {
  ...
  // same warning string -- same guard `<lyra-chip-group>`'s equivalent test
  // uses.
  ...
  try {
    ...
    const group = host.shadowRoot!.querySelector('lyra-avatar-group') as LyraAvatarGroup;
    ...
    expect(badge).to.exist;
    expect(badge.textContent!.trim()).to.equal('+1');
    const forwardedAvatars = Array.from(host.querySelectorAll('lyra-avatar')) as HTMLElement[];
...
describe('overflow badge activation', () => {
  it('fires lyra-overflow-click with the correct hiddenCount/hiddenAvatars on click', async () => {
    const el = (await fixture(fiveAvatars())) as LyraAvatarGroup;
    ...
    const ev = await oneEvent(el, 'lyra-overflow-click');
  ...
  it('does not unhide avatars or change its own text/aria-label on click (non-toggle behavior)', async () => {
    ...
    setTimeout(() => badge().click());
    await oneEvent(el, 'lyra-overflow-click');
    await el.updateComplete;
  ...
  it('never renders aria-expanded on the overflow badge, in any state -- regression guard against copying lyra-chip-group by rote', async () => {
    const el = (await fixture(fiveAvatars())) as LyraAvatarGroup;
    const badge = () => el.shadowRoot!.querySelector('[part="overflow-badge"]') as HTMLButtonElement;
...
describe('accessible name (label / host aria-label precedence)', () => {
  it('sets role="group" and aria-label from the label prop', async () => {
    const el = (await fixture(html`
      <lyra-avatar-group label="Team members"><lyra-avatar initials="AB"></lyra-avatar></lyra-avatar-group>
    `)) as LyraAvatarGroup;
  ...
  it('prefers the label prop over a host aria-label when both are set', async () => {
    const el = (await fixture(html`
      <lyra-avatar-group label="Team members" aria-label="Something else">
        <lyra-avatar initials="AB"></lyra-avatar>
      </lyra-avatar-group>
    `)) as LyraAvatarGroup;
    const base = el.shadowRoot!.querySelector('[part="base"]') as HTMLElement;
  ...
  it('falls back to a host aria-label when label is unset', async () => {
    const el = (await fixture(html`
      <lyra-avatar-group aria-label="Team members"><lyra-avatar initials="AB"></lyra-avatar></lyra-avatar-group>
    `)) as LyraAvatarGroup;
    const base = el.shadowRoot!.querySelector('[part="base"]') as HTMLElement;
    expect(base.getAttribute('aria-label')).to.equal('Team members');
  });
  ...
  it('renders no aria-label at all when neither label nor host aria-label is set', async () => {
    const el = (await fixture(html`
      <lyra-avatar-group><lyra-avatar initials="AB"></lyra-avatar></lyra-avatar-group>
    `)) as LyraAvatarGroup;
    const base = el.shadowRoot!.querySelector('[part="base"]') as HTMLElement;
...
describe('localization', () => {
  ...
  it('localizes the collapsed overflow-badge visible text via this.localize(), not a hardcoded "+N"', async () => {
    const el = (await fixture(html`
      <lyra-avatar-group max="1" .strings=${{ showMoreCollapsed: '{count} de plus' }}>
        <lyra-avatar initials="AB"></lyra-avatar>
        <lyra-avatar initials="CD"></lyra-avatar>
        <lyra-avatar initials="EF"></lyra-avatar>
      </lyra-avatar-group>
  ...
  it('localizes the overflow-badge aria-label via this.localize(), not hardcoded English', async () => {
    const el = (await fixture(html`
      <lyra-avatar-group max="1" .strings=${{ showMoreCount: '{count} de plus' }}>
        <lyra-avatar initials="AB"></lyra-avatar>
        <lyra-avatar initials="CD"></lyra-avatar>
        <lyra-avatar initials="EF"></lyra-avatar>
      </lyra-avatar-group>
...
it('flips the resolved overlap margin under dir="rtl" (Chromium resolves logical properties to physical margin-left/margin-right)', async () => {
  const ltrEl = (await fixture(html`
    <lyra-avatar-group dir="ltr">
      <lyra-avatar initials="AB"></lyra-avatar>
      <lyra-avatar initials="CD"></lyra-avatar>
    </lyra-avatar-group>
  `)) as LyraAvatarGroup;
  const ltrSecond = ltrEl.querySelectorAll('lyra-avatar')[1] as HTMLElement;
  ...
  const rtlEl = (await fixture(html`
    <lyra-avatar-group dir="rtl">
      <lyra-avatar initials="AB"></lyra-avatar>
      <lyra-avatar initials="CD"></lyra-avatar>
    </lyra-avatar-group>
  `)) as LyraAvatarGroup;
  const rtlSecond = rtlEl.querySelectorAll('lyra-avatar')[1] as HTMLElement;
...
describe('design tokens reach rendered CSS', () => {
  it('resolves --lyra-avatar-group-avatar-size/-overlap through the size token chain (lg tier)', async () => {
    const el = (await fixture(html`
      <lyra-avatar-group size="lg"><lyra-avatar></lyra-avatar></lyra-avatar-group>
    `)) as LyraAvatarGroup;
    const hostStyle = getComputedStyle(el);
    expect(hostStyle.getPropertyValue('--lyra-avatar-group-avatar-size').trim()).to.equal('2.5rem');
    expect(hostStyle.getPropertyValue('--lyra-avatar-group-overlap').trim()).to.equal('-8px');
  });

  it('resolves --lyra-avatar-group-badge-bg/-color through the tone token chain and applies them to the rendered badge (brand tier)', async () => {
    const el = (await fixture(html`
      <lyra-avatar-group max="0" tone="brand"><lyra-avatar initials="AB"></lyra-avatar></lyra-avatar-group>
    `)) as LyraAvatarGroup;
    const hostStyle = getComputedStyle(el);
    expect(hostStyle.getPropertyValue('--lyra-avatar-group-badge-bg').trim()).to.equal('#ddf4ff');
    expect(hostStyle.getPropertyValue('--lyra-avatar-group-badge-color').trim()).to.equal('#0969da');
...
it('can shrink to a 320px allocation with several overflowing avatars', async () => {
  ...
  `);
  const el = wrapper.querySelector('lyra-avatar-group') as LyraAvatarGroup;
  await el.updateComplete;
...
it('is accessible in an overflowing state', async () => {
  const el = (await fixture(html`
    <lyra-avatar-group max="2" label="Team members">
      <lyra-avatar initials="AB"></lyra-avatar>
      <lyra-avatar initials="CD"></lyra-avatar>
      <lyra-avatar initials="EF"></lyra-avatar>
    </lyra-avatar-group>
