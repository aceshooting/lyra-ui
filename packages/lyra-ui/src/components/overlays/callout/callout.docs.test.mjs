...
const source = readFileSync(join(here, 'callout.class.ts'), 'utf8');

test('lr-callout close-button doc warns that --lr-icon-button-size is a no-op here', () => {
  const csspartDoc = source.match(/@csspart close-button -[^]*?(?=\n\s*\*\s*@csspart)/);
  assert.ok(csspartDoc, 'expected the close-button @csspart doc block');
  assert.match(
    csspartDoc[0],
