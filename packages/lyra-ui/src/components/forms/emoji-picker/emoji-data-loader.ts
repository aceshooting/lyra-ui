...
/** Group id -> `LyraMessageKey`, parallel to `GROUP_LABELS` above. These headings are generated by
 *  this adapter (not authored by the consumer), so they are library-owned user-facing strings and
 *  must follow `registerLyraLocale()`/`.strings` like every other one. The adapter cannot call
 *  `this.localize()` itself -- it is a plain module function with no host element and its result is
 *  cached across the page, so a label baked in here would freeze at whatever locale happened to
 *  load first. It therefore emits the key alongside the English text and `<lr-emoji-picker>`
 *  resolves it at render time, which also keeps a live `setLyraLocale()` switch reactive.
 *  An unknown/future group id has no key and falls back to the generated `Group {id}` label. */
  ...
  for (const entry of entries as Array<{
    ...
    if (!bucket) {
      bucket = {
        ...
        labelKey: GROUP_LABEL_KEYS[entry.group],
        emojis: [],
      };
