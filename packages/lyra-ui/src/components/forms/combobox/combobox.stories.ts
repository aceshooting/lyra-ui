...
export const DynamicOptionAvailability: Story = {
  render: () => html`
    <div style="display:grid; gap:var(--lr-space-s); max-inline-size:22rem;">
      <p style="margin:0;">
        Open the list, press End, then R to remove the active final option or D
        to disable it.
      </p>
      <lr-combobox
        label="Fruit"
          ...
          if (event.key.toLocaleLowerCase() === "d") {
            ...
            const option = value
              ? combobox.querySelector(
                  `lr-option[value="${CSS.escape(value)}"]`
                )
              : null;
            if (option)
  ...
  render: () => html`
    <div style="inline-size: 320px; max-inline-size: 100%;">
      ...
      >
        ...
        >
        <lr-option value="secondary" sub="Backup environment"
          >Secondary deployment</lr-option
        >
      </lr-combobox>
    </div>
  `,
  ...
  render: () => html`
    ...
    >
      <lr-option
        ...
        group="Building A"
        sub="Running"
        dot-color="var(--lr-color-success)"
      >
        Inverter 1
      </lr-option>
      <lr-option
        ...
        group="Building A"
        sub="Idle"
        dot-color="var(--lr-color-text-quiet)"
      >
        Inverter 2
      </lr-option>
      <lr-option
      ...
      const rows = [
        {
          ...
          badge: 12,
          accessibleLabel:
            "Alpine Energy versus Commission, judgment, 12 citations",
    ...
    const reportSelection = (event: Event) => {
      ...
      if (output)
        output.textContent = row
          ? `${row.label} — payload kind: ${data?.kind ?? "unknown"}`
          : "No selection";
    };
    ...
    return html`
      <div>
        ...
        <output aria-live="polite"
          >Select a result to inspect its retained data payload.</output
        >
      </div>
    `;
  },
...
export const CustomFilter: Story = {
  render: () => {
    const startsWith: OptionFilter = (option, query) =>
      option.label.toLowerCase().startsWith(query.toLowerCase());
    return html`
      <lr-combobox
        label="Fruit (starts with…)"
        placeholder="Try “an”…"
        style="max-width: 22rem"
        .filter=${startsWith}
      >
...
export const LiveFilterText: Story = {
  render: () => {
    ...
    const onFilter = (event: Event) => {
      ...
      if (!query) {
        status.textContent =
          "Type to filter — the live text arrives on every keystroke.";
        return;
      }
    ...
    return html`
      <div style="max-width: 22rem">
        ...
        >
          ${fruits.map(
            (fruit) =>
              html`<lr-option value=${fruit.toLowerCase()}>${fruit}</lr-option>`
          )}
        </lr-combobox>
        <output aria-live="polite"
          >Type to filter — the live text arrives on every keystroke.</output
        >
      </div>
    `;
  },
...
export const States: Story = {
  render: () => html`
    <div
      style="display: flex; flex-direction: column; gap: 1.5rem; max-width: 22rem"
    >
...
const seatAvailabilityValidator: LyraComboboxValidator = {
  ...
  checkValidity: (input) => {
    const soldOut =
      (input as unknown as HTMLElement).getAttribute("data-sold-out") ===
      "true";
    return {
      isValid: !soldOut,
  ...
  render: () => {
    ...
    return html`
      <div style="display: grid; gap: var(--lr-space-s); max-width: 22rem">
        ...
        <output aria-live="polite"
          >That performance just sold out — pick another.</output
        >
      </div>
    `;
  },
  ...
  render: () => html`
    ...
    >
      ...
      <lr-select
        aria-label="Status"
        placeholder="Status"
        style="--lr-select-trigger-height: 2.25rem"
      >
      ...
      <lr-combobox
        aria-label="Owner"
        placeholder="Owner"
        style="--lr-combobox-trigger-height: 2.25rem"
      >
...
export const ManagedOverlayStack: Story = {
  ...
  render: () => html`
    <div
      style="display:flex;align-items:start;gap:calc(var(--lr-space-l) + var(--lr-space-s));min-block-size:var(--lr-size-24rem)"
    >
