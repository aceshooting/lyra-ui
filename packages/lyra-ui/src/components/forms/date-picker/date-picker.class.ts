...
function normalizeView(value: unknown): LyraDatePickerView {
  return value === 'months' || value === 'years' || value === 'decades'
    ? value
    : 'days';
}
...
export class LyraDatePicker extends LyraElement<LyraDatePickerEventMap> {
  ...
  @property({ converter: modeConverter, reflect: true }) mode: CalendarMode =
    'single';
  ...
  @property({ type: Boolean, attribute: 'disable-past', reflect: true })
  disablePast = false;
  @property({ type: Boolean, attribute: 'disable-future', reflect: true })
  disableFuture = false;
   ...
   * empty collection instead of reaching the render path. Blank labels are omitted; inadmissible
   * ranges render disabled. Interior days do not all need to be enabled.
   *
   * Applying one commits the range exactly as a two-click selection would (clamped to `min`/`max`,
   * then admitted against endpoint and inclusive `minRange`/`maxRange` constraints before
   * `input` followed by `change`), so a consumer's existing change handling needs no special
  ...
  private resolvePresetRange(preset: Pick<CanonicalDateRangePreset, 'start' | 'end'>): { from: Date; to: Date } | null {
    ...
    const start = bound(preset.start, this.min);
    const end = bound(preset.end, this.max);
    if (!start || !end) return null;
    let from = clampDate(start, min, max);
    let to = clampDate(end, min, max);
    if (to < from) [from, to] = [to, from];
    return this.admitsRange(from, to, min, max, this.resolvedToday()) ? { from, to } : null;
  }
  @property({ type: Number, attribute: 'min-range', reflect: true })
  minRange = 0;
  @property({ type: Number, attribute: 'max-range', reflect: true })
  maxRange = 0;
  @property({ attribute: 'page-by', reflect: true, converter: pageByConverter })
  ...
  private nextLabelAuthored = false;

  @state() private viewDate = localDate(
  ...
  private constraintFocusOwner: Element | undefined;

  static override get observedAttributes(): string[] {
    const attributes = super.observedAttributes;
    observeReactivePropertyWrites(this.prototype, ['previousLabel', 'nextLabel'], (instance: LyraDatePicker, name, value) => {
      const authored = value != null;
      let ownershipChanged = false;
      switch (name) {
        case 'previousLabel':
          // The field initializer precedes its ownership flag; later equal writes are authored.
          if (instance.previousLabelAuthored === undefined) return;
          ownershipChanged = authored !== instance.previousLabelAuthored;
          instance.previousLabelAuthored = authored;
          break;
        case 'nextLabel':
          // The field initializer precedes its ownership flag; later equal writes are authored.
          if (instance.nextLabelAuthored === undefined) return;
          ownershipChanged = authored !== instance.nextLabelAuthored;
          instance.nextLabelAuthored = authored;
          break;
      }
  ...
  protected override willUpdate(changed: PropertyValues): void {
    ...
    if (changed.has('value')) {
      const external = this.committedValue !== this.value;
      this.committedValue = undefined;
      if (external) this._appliedPreset = undefined;
      if (external && this.value) {
        const from = this.selection.from;
        if (from)
          this.viewDate = localDate(from.getFullYear(), from.getMonth(), 1);
      }
    }
    ...
    this.viewDate = this.clampViewAnchor(this.viewDate);
   ...
   */
  private nearestEnabledDate(
    anchor: Date,
    min: Date | null,
    max: Date | null,
    today: Date
  ): Date | null {
let lower = min ?? localDate(0);
    let upper = max ?? localDate(9999, 11, 31);
    if (this.disablePast && today > lower) lower = today;
    if (this.disableFuture && today < upper) upper = today;
    if (lower > upper) return null;
    anchor = clampDate(anchor, lower, upper);
   ...
   */
  private normalizeFocusedDate(
    min: Date | null,
    max: Date | null,
    today: Date
  ): void {
    const anchor = this.focusedDateValue ?? this.selection.from ??
      (this.effectiveView === 'days' ? this.viewDate : null);
    if (!anchor) return;
    ...
    const next = disabled
      ? this.nearestEnabledDate(anchor, min, max, today)
...
const active = activeElementIn(this.renderRoot as ShadowRoot);
  ...
  private selectDate(date: Date): void {
    ...
    if (this.effectiveMode === 'range' && pending.from && !pending.to &&
      !this.admitsRange(pending.from, date, min, max, today)) return;
  ...
  goToDate(date: string | Date): void {
    ...
    const clamped = clampDate(midnight, parseISO(this.min) ?? localDate(0), parseISO(this.max) ?? localDate(9999, 11, 31));
    this.viewDate = localDate(clamped.getFullYear(), clamped.getMonth(), 1);
    this.setFocusedDate(clamped);
    this.focusPending = true;
  }

  private clampViewAnchor(date: Date): Date {
    return clampDate(date, localDate(0), localDate(9999, 12 - this.visibleMonths, 1));
  }
  ...
  private viewPeriodStart(date: Date, view = this.effectiveView): Date {
    ...
    if (view === 'decades')
      return localDate(Math.floor(date.getFullYear() / 10) * 10, 0, 1);
  ...
  private viewPeriods(view = this.effectiveView): ViewPeriod[] {
    ...
    return Array.from({ length: 12 }, (_, index) => {
      ...
      return {
        start,
        end: clampDate(localDate(start.getFullYear(), start.getMonth() + span, 0), null, localDate(9999, 11, 31)),
      };
    }).filter((period) => period.start.getFullYear() <= 9999);
  }
  ...
  private clampViewDateToEnabledPeriod(view = this.effectiveView): Date | null {
    ...
    const next = forward
      ? this.firstEnabledViewPeriod(this.viewPeriodStart(min), 1)
      : backward
      ? this.firstEnabledViewPeriod(this.viewPeriodStart(max), -1)
  ...
  private resolveViewFocus(periods: readonly ViewPeriod[]): Date | null {
    const inView = (candidate: Date | null): Date | null => {
      ...
      return period && !this.viewPeriodDisabled(period.start, period.end)
        ? period.start
        : null;
    };
    ...
    return (
      inView(parseISO(this.focusedViewStart)) ??
      inView(this.selection.from) ??
      inView(this.viewDate) ??
      fallback?.start ??
      null
    );
  }

  private setFocusedViewStart(
    date: Date,
  ...
  private onViewItemFocus(start: Date): void {
    if (!this.restoringViewFocus)
      this.setFocusedViewStart(start, this.effectiveView, true);
  }
  ...
  private firstEnabledViewPeriod(start: Date, direction: -1 | 1): Date | null {
    ...
    for (let index = 0; index < 366; index++) {
      if (candidate.getFullYear() < 0 || candidate.getFullYear() > 9999) return null;
      const end = localDate(
        candidate.getFullYear(),
        candidate.getMonth() + span,
        0
      );
  ...
  private onViewGridKey = (event: KeyboardEvent): void => {
    ...
    const currentSource =
      parseISO(source?.getAttribute('data-view-start') ?? '') ??
      this.resolveViewFocus(periods);
  ...
  private onGridKey = (e: KeyboardEvent): void => {
    ...
    const firstEnabledFrom = (base: Date, dayStep: number): Date | null => {
      let d = base;
      for (let i = 0; i < 366; i++) {
        if (d.getFullYear() < 0 || d.getFullYear() > 9999) return null;
        if (!this.isDisabled(d, min, max, today)) return d;
        d = step(d, dayStep);
      }
      return null;
    ...
    switch (e.key) {
      ...
      case 'PageDown':
        ...
        break;
      case 'Home':
        next = firstEnabledFrom(
          localDate(current.getFullYear(), current.getMonth(), 1),
          1
        );
        break;
      case 'End':
        next = firstEnabledFrom(
          localDate(current.getFullYear(), current.getMonth() + 1, 0),
          -1
        );
        break;
      case 'Enter':
  ...
  private viewDateForFocus(next: Date): Date {
    const firstMonth = this.clampViewAnchor(localDate(
      this.viewDate.getFullYear(),
      this.viewDate.getMonth(),
      1
    ));
  ...
  private renderDay(
    ...
    if (date.getFullYear() < 0 || date.getFullYear() > 9999 || (outside && !this.withOutsideDays)) {
    ...
    const selected =
      this.effectiveMode === 'single' ? isStart : isStart || isEnd;
    const inRange =
    ...
    const focusedDate = this.focusedDateValue;
    const focused = outsideDuplicate
      ? false
      : focusedDate != null
      ...
      ? true
      : !from &&
        fallbackFocusDate != null &&
        isSameDay(date, fallbackFocusDate);
    ...
    let content: unknown = getNumberFormat(this.effectiveLocale).format(
      date.getDate()
    );
    try {
          ...
          <div part="nav">
              ...
              ? html`<button
                ...
                >
                  <span aria-hidden="true" inert
                    ><slot name="previous-icon">${chevronIcon()}</slot></span
                  >
                </button>`
              : html`<span></span>`}
            <button
            ...
            ${isLast
              ? html`<button
                ...
                >
                  <span aria-hidden="true" inert
                    ><slot name="next-icon">${chevronIcon()}</slot></span
                  >
                </button>`
              : html`<span></span>`}
          </div>
  ...
  private viewPeriodHasEnabledDate(start: Date, end: Date): boolean {
    ...
    for (
      let days = 0;
      days < MAX_VIEW_PERIOD_DAYS && date.getTime() <= lastTime;
      days++
    ) {
  ...
  private navView(delta: number): void {
    const months =
      this.effectiveView === 'months'
        ? 12
        : this.effectiveView === 'years'
        ? 144
        : 1440;
    this.viewDate = this.clampViewAnchor(addMonths(this.viewDate, delta * months));
  }
  ...
  private renderView(today: Date): TemplateResult {
    ...
    const yearFormatter = dateTimeFormat(this.effectiveLocale, {
      year: 'numeric',
    });
    ...
    return html`
      ...
      >
        ${rows.map(
          (row) => html`<div part="view-row" role="row">
            ${row.map((item) =>
              this.renderViewItem(
                item.start,
                item.end,
  ...
  override render(): TemplateResult {
    ...
    const fallbackFocusDate =
      this.focusedDateValue || selection.from
        ? null
        : this.firstEnabledDate(min, max, today);
    ...
    return html`<div part="base date-picker" role=${hostAriaLabel(this) === null ? nothing : 'group'} aria-label=${hostAriaLabel(this) ?? nothing}>
      <div class="content">
        ${this.renderPresets()}
