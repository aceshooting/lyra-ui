...
const parts = (el: LyraColorPicker, name: string): HTMLElement[] =>
  Array.from(
    el.shadowRoot!.querySelectorAll(`[part~="${name}"]`)
  ) as HTMLElement[];
const count = (el: LyraColorPicker, name: string): number =>
  el.shadowRoot!.querySelectorAll(`[part~="${name}"]`).length;

async function opened(
  markup = html`<lr-color-picker label="Accent"></lr-color-picker>`
): Promise<LyraColorPicker> {
...
function press(target: HTMLElement, key: string, shiftKey = false): void {
  ...
  target.dispatchEvent(
    new KeyboardEvent("keyup", { key, shiftKey, bubbles: true, composed: true })
  );
}
...
it("renders errorText and an error slot, wiring aria-describedby to the rendered hint/error ids", async () => {
  const el = (await fixture(
    html`<lr-color-picker
      label="Accent"
      hint="Pick a brand color"
      error-text="Not a valid color"
    ></lr-color-picker>`
  )) as LyraColorPicker;
  await el.updateComplete;
...
it("hides the error part when errorText is unset and no error slot content is assigned", async () => {
  const el = (await fixture(
    html`<lr-color-picker></lr-color-picker>`
  )) as LyraColorPicker;
  await el.updateComplete;
...
it("shows a required-field asterisk after the label", async () => {
  ...
  await el.updateComplete;
  expect(
    getComputedStyle(part(el, "form-control-label"), "::after").content
  ).to.contain("*");
});
...
it("publishes pristine and user-invalid state on both accessibility owners and clears it again", async () => {
  const form = (await fixture(html`
    <form>
      <lr-color-picker label="Accent" name="accent" required></lr-color-picker>
    </form>
  `)) as HTMLFormElement;
  const el = form.querySelector("lr-color-picker") as LyraColorPicker;
  await el.updateComplete;
...
it("uses only the canonical value attribute as its reset default", async () => {
  const form = (await fixture(html`
    <form>
      <lr-color-picker name="accent" value="#ff0000"></lr-color-picker>
    </form>
  `)) as HTMLFormElement;
  const el = form.querySelector("lr-color-picker") as LyraColorPicker;
  await el.updateComplete;
...
it("forwards host click/focus/blur and suppresses click when effectively disabled", async () => {
  const form = (await fixture(html`
    <form>
      <fieldset><lr-color-picker></lr-color-picker></fieldset>
    </form>
  `)) as HTMLFormElement;
  const el = form.querySelector("lr-color-picker") as LyraColorPicker;
...
it("keeps direct and fieldset-disabled inline pickers out of programmatic focus", async () => {
  ...
  const form = (await fixture(html`
    <form>
      <fieldset disabled>
        <lr-color-picker inline label="Inherited"></lr-color-picker>
      </fieldset>
    </form>
  `)) as HTMLFormElement;
...
it("visually marks direct and fieldset-cascaded disabled state across every pointer surface", async () => {
  const form = (await fixture(html`
    <form>
      <fieldset disabled>
        <lr-color-picker opacity swatches="#ff0000"></lr-color-picker>
      </fieldset>
    </form>
  `)) as HTMLFormElement;
  const el = form.querySelector("lr-color-picker") as LyraColorPicker;
  ...
  for (const name of pointerParts) {
    expect(
      getComputedStyle(part(el, name)).cursor,
      `fieldset ${name}`
  ...
  await el.updateComplete;
  for (const name of pointerParts) {
    expect(getComputedStyle(part(el, name)).cursor, `direct ${name}`).to.equal(
      "not-allowed"
    );
  }
});
...
it('defaults to size "m", scales the visible swatch, and keeps every trigger at the shared hit-area floor', async () => {
  ...
  for (const [size, px] of Object.entries(expected)) {
    ...
    expect(
      getComputedStyle(trigger, "::before").inlineSize,
      `visible inline-size size=${size}`
    ).to.equal(px);
  }
});
...
it("accepts the Web Awesome size spellings, rendering small/medium/large as s/m/l", async () => {
  ...
  for (const [alias, step] of pairs) {
    ...
    expect(
      getComputedStyle(part(aliasEl, "trigger"), "::before").blockSize,
      `block-size for ${alias}`
    ).to.equal(getComputedStyle(part(stepEl, "trigger"), "::before").blockSize);
    expect(
      getComputedStyle(part(aliasEl, "trigger"), "::before").inlineSize,
      `inline-size for ${alias}`
    ).to.equal(
...
it("lets a consumer hover rule override the trigger part without important", async () => {
  const frame = (await fixture(html`
    <div>
      <style>
        lr-color-picker::part(trigger):hover {
...
it("exposes getHexString() for percent-scaled HSV and optional alpha", async () => {
  const el = (await fixture(
    html`<lr-color-picker uppercase></lr-color-picker>`
  )) as LyraColorPicker;
  expect(el.getHexString(0, 100, 100)).to.equal("#FF0000");
  expect(el.getHexString(120, 100, 100, 50)).to.equal("#00FF0080");
  expect(el.getHexString(Number.NaN, Number.POSITIVE_INFINITY, -1)).to.equal(
    "#000000"
  );
});
...
it("omits the format toggle when without-format-toggle is set", async () => {
  const el = await opened(
    html`<lr-color-picker label="A" without-format-toggle></lr-color-picker>`
  );
...
it("also omits the format toggle for Shoelace no-format-toggle", async () => {
  const el = await opened(
    html`<lr-color-picker label="A" no-format-toggle></lr-color-picker>`
  );
...
it("exposes the upstream button subparts and unprefixed sizing hooks", async () => {
  ...
  try {
    ...
    expect(getComputedStyle(part(el, "grid-handle")).inlineSize).to.equal(
      "18px"
    );
    expect(
      getComputedStyle(part(el, "hue-slider"), "::before").blockSize
    ).to.equal("14px");
    expect(getComputedStyle(part(el, "hue-slider-handle")).inlineSize).to.equal(
      "22px"
    );
...
it("gives the opacity slider a real slider role with a full value contract", async () => {
  const el = await opened(
    html`<lr-color-picker label="A" opacity value="#ff0000"></lr-color-picker>`
  );
  ...
  await el.updateComplete;
  expect(
    part(el, "opacity-slider-handle").getAttribute("aria-valuenow")
  ).to.equal("99");
});
...
it("flips the opacity gradient direction under RTL", async () => {
  const wrapper = (await fixture(html`
    <div dir="rtl">
      <lr-color-picker label="A" opacity value="#ff0000"></lr-color-picker>
    </div>
  `)) as HTMLElement;
  ...
  const gradient = getComputedStyle(
    part(el, "opacity-slider")
  ).getPropertyValue("--lr-color-picker-opacity-gradient");
...
it("accepts a semicolon-separated swatch string, an array, and labelled objects", async () => {
  ...
  await el.updateComplete;
  expect(parts(el, "swatch")[0]!.getAttribute("aria-label")).to.equal(
    "Brand red"
  );
});
...
it("selects a swatch on click and marks the active one with more than colour alone", async () => {
  ...
  expect((event as CustomEvent<{ value: string }>).detail.value).to.equal(
    "#00ff00"
  );
  await el.updateComplete;
...
it("ignores a click on a swatch whose color cannot be parsed", async () => {
  const el = await opened(
    html`<lr-color-picker label="A" swatches="not-a-color"></lr-color-picker>`
  );
...
it("keeps a palette swatch alpha channel when opacity is enabled, and drops it otherwise", async () => {
  ...
  expect(withoutAlpha.getFormattedValue("rgba")).to.equal(
    "rgba(0, 255, 0, 1.00)"
  );
});
...
it("swaps the horizontal grid arrows under RTL", async () => {
  const wrapper = (await fixture(html`
    <div dir="rtl">
      <lr-color-picker label="A" value="#ff0000"></lr-color-picker>
    </div>
  `)) as HTMLElement;
  const el = wrapper.querySelector("lr-color-picker") as LyraColorPicker;
  el.open = true;
...
it("gives the hue slider a real slider role over the full 0-360 range", async () => {
  ...
  press(handle, "End");
  await el.updateComplete;
  expect(part(el, "hue-slider-handle").getAttribute("aria-valuenow")).to.equal(
    "360"
  );
...
it("moves the hue slider with vertical arrows too, honouring the shift multiplier, and ignores unrelated keys", async () => {
  const el = await opened(
    html`<lr-color-picker label="A" value="#00ffff"></lr-color-picker>`
  );
  const handle = part(el, "hue-slider-handle");
...
it("emits input/lr-input per step and change/lr-change once on release", async () => {
  ...
  expect(
    nativeEvents
      ...
      .every((event) => event instanceof InputEvent)
  ).to.be.true;
  expect(
    nativeEvents.find((event) => event.type === "change")?.constructor === Event
  ).to.be.true;
  expect(
    nativeEvents.every(
...
for (const cancellation of ["pointercancel", "lostpointercapture"] as const) {
  it(`rolls a pointer preview and submitted value back silently on ${cancellation}`, async () => {
    ...
    await el.updateComplete;
    expect(
      part(el, "hue-slider-handle").getAttribute("aria-valuenow")
    ).to.equal("180");
    ...
    window.dispatchEvent(
      new PointerEvent("pointermove", { pointerId: 11, clientX: rect.left })
    );
    await el.updateComplete;
    expect(
      part(el, "hue-slider-handle").getAttribute("aria-valuenow")
    ...
    expect(
      el.value,
      "cancellation restores the pre-drag pristine/default relationship"
    ).to.equal("#00ff00");
...
it("keeps an adopted iframe drag on its owner window and releases that window on readoption", async () => {
  ...
  try {
    ...
    expect(
      adoptedValue,
      "adoption rolls the uncommitted preview back"
    ).to.equal(valueBeforeDrag);
    frameWindow.dispatchEvent(
      new frameWindow.PointerEvent("pointermove", {
        pointerId: 81,
        pointerType: "touch",
    ...
    expect(el.value, "the retired iframe listener must be gone").to.equal(
      adoptedValue
    );
  } finally {
    el.remove();
...
it("reserves touch gestures for the two-dimensional grid while preserving vertical page pan on tracks", async () => {
  const el = await opened(
    html`<lr-color-picker label="A" opacity></lr-color-picker>`
  );
  expect(getComputedStyle(part(el, "grid")).touchAction).to.equal("none");
  expect(getComputedStyle(part(el, "hue-slider")).touchAction).to.equal(
    "pan-y"
  );
  expect(getComputedStyle(part(el, "opacity-slider")).touchAction).to.equal(
    "pan-y"
  );
});
...
it("safely handles a zero-size drag rect instead of dividing by zero", async () => {
  const el = await opened(
    html`<lr-color-picker label="A" value="#808080"></lr-color-picker>`
  );
  ...
  await el.updateComplete;
  window.dispatchEvent(
    new PointerEvent("pointermove", { pointerId: 71, clientX: 50, clientY: 50 })
  );
  await el.updateComplete;
...
it("ends a drag from a pointermove that arrives while disabled, before the next render can react", async () => {
  ...
  expect(
    changes,
    "the drag already ended, so pointerup commits nothing"
  ).to.equal(0);
});
...
it("ignores a pointerup or pointercancel for an unrelated pointer id, leaving the active drag armed", async () => {
  ...
  await el.updateComplete;
  expect(changes, "an unrelated pointerup must not commit this drag").to.equal(
    0
  );

  window.dispatchEvent(new PointerEvent("pointercancel", { pointerId: 998 }));
  await el.updateComplete;
  expect(changes, "an unrelated pointercancel must not end this drag").to.equal(
    0
  );
  ...
  expect(
    changes,
    "the drag was still active and now commits normally"
  ).to.equal(1);
});
...
it("opens and closes from the trigger, reflecting open and aria-expanded", async () => {
  ...
  expect(part(el, "trigger").getAttribute("aria-controls")).to.equal(
    part(el, "panel").id
  );
...
it("ignores a dispatched click on the trigger while disabled, even bypassing native click() gating", async () => {
  const el = (await fixture(
    html`<lr-color-picker label="A" disabled></lr-color-picker>`
  )) as LyraColorPicker;
  await el.updateComplete;
  part(el, "trigger").dispatchEvent(
...
it("clicks the field input when click() is called in inline mode", async () => {
  const el = (await fixture(
    html`<lr-color-picker inline label="A"></lr-color-picker>`
  )) as LyraColorPicker;
  await el.updateComplete;
  const input = part(el, "input") as HTMLInputElement;
  let clicks = 0;
...
it("marks the selected swatch with a check mark, not colour alone", async () => {
  ...
  expect(
    getComputedStyle(parts(el, "swatch")[0]!, "::before").content
  ).to.not.contain("✓");
});
...
it("emits migrated input and after-show/after-hide aliases exactly once, never the removed focus/blur aliases", async () => {
  ...
  expect(
    nativeFocusEvents.every((event) => event.bubbles && event.composed)
  ).to.equal(true);
...
it("emits the composed focus/blur relays in inline mode without the removed lr-focus/lr-blur aliases", async () => {
  ...
  el.focus();
  expect(el.shadowRoot!.activeElement === part(el, "grid-handle")).to.equal(
    true
  );
  el.blur();
...
it("never opens while effectively disabled, and closes when disabled mid-interaction", async () => {
  ...
  const form = (await fixture(html`
    <form>
      <fieldset><lr-color-picker label="A"></lr-color-picker></fieldset>
    </form>
  `)) as HTMLFormElement;
...
it("ignores a dispatched click on the eyedropper button while disabled", async () => {
  ...
  try {
    ...
    await el.updateComplete;
    part(el, "eyedropper-button").dispatchEvent(
      new MouseEvent("click", { bubbles: true, cancelable: true })
    );
    await new Promise((resolve) => setTimeout(resolve, 0));
    expect(opens).to.equal(0);
...
it("uses a .strings override for the trigger name and every slider value description", async () => {
  const el = await opened(
    html`<lr-color-picker value="#ff0000" opacity></lr-color-picker>`
  );
  el.strings = {
  ...
  await el.updateComplete;
  expect(part(el, "trigger").getAttribute("aria-label")).to.equal(
    "Sélecteur de couleur"
  );
  expect(part(el, "hue-slider-handle").getAttribute("aria-valuetext")).to.equal(
    "0 degrés"
  );
  expect(
    part(el, "opacity-slider-handle").getAttribute("aria-valuetext")
  ).to.equal("100 pour cent");
  expect(part(el, "grid-handle").getAttribute("aria-valuetext")).to.equal(
    "S 100 L 100"
  );
});
...
it("commits a typed color on Enter and keeps the draft until then", async () => {
  ...
  el.addEventListener("input", (event) =>
    publicInputs.push(event as InputEvent)
  );

  field.value = "#00ff00";
  field.dispatchEvent(
    new InputEvent("input", { bubbles: true, composed: true, data: "0" })
  );
  await el.updateComplete;
  expect(el.value).to.equal("#ff0000");
...
describe("lr-color-picker barred from constraint validation", () => {
  ...
  it("reports no violation inside a disabled fieldset", async () => {
    const form = (await fixture(html`
      <form>
        <fieldset disabled>
          <lr-color-picker required></lr-color-picker>
        </fieldset>
      </form>
    `)) as HTMLFormElement;
    const el = form.querySelector("lr-color-picker") as LyraColorPicker;
...
it("keeps the reflected open attribute in step when a veto arrives through the attribute", async () => {
  ...
  expect(
    ...
    "the attribute cannot outlive the vetoed property"
  ).to.be.false;
});

it("does not let a listener hold a disabled picker open", async () => {
  const el = (await fixture(
    html`<lr-color-picker label="Accent" open></lr-color-picker>`
  )) as LyraColorPicker;
  await el.updateComplete;
  expect(el.open).to.be.true;
  el.addEventListener("lr-hide", (event) => event.preventDefault());
  el.disabled = true;
  await el.updateComplete;
  expect(el.open, "disablement closes the panel regardless of any veto").to.be
    .false;
});
...
it("makes lr-show/lr-hide cancelable and the after-events not", async () => {
  ...
  expect(seen.map((event) => event.cancelable)).to.deep.equal([
    true,
    false,
    true,
    false,
