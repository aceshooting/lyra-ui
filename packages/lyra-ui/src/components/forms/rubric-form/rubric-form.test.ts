...
function createRealmFrame(): {
  ...
  if (!frameDocument || !frameWindow) {
    iframe.remove();
    throw new Error(
      "Could not create an iframe realm for the rubric-form test."
    );
  }
  return { iframe, frameDocument, frameWindow: frameWindow as WindowWithCss };
}
...
describe("lr-rubric-form", () => {
  it("renders one control per key, in array order", async () => {
    ...
    const fields = [...el.shadowRoot!.querySelectorAll('[part="field"]')].map(
      (f) => f.getAttribute("data-key")
    );
    expect(fields).to.deep.equal(["accuracy", "category", "comment"]);
    expect(el.shadowRoot!.querySelector('[data-key="accuracy"] lr-segmented'))
      .to.exist;
    expect(el.shadowRoot!.querySelector('[data-key="category"] lr-select')).to
      .exist;
    expect(el.shadowRoot!.querySelector('[data-key="comment"] lr-textarea')).to
      .exist;
  });
  ...
  it("uses one shared required marker for owned and child labels", async () => {
    ...
    for (const key of ["score", "unsupported"]) {
      ...
      expect(field.hasAttribute("data-required"), `${key} field owns its label`)
        .to.be.true;
      expect(
        label.querySelectorAll("span[aria-hidden]").length,
        `${key} has no literal marker`
      ).to.equal(0);
      expect(
    ...
    for (const key of ["category", "categories"]) {
      const categoryField = el.shadowRoot!.querySelector(
        `[data-key="${key}"]`
      ) as HTMLElement;
      const category = categoryField.querySelector(
        "lr-select, lr-checkbox-group"
      ) as HTMLElement & {
        updateComplete: Promise<boolean>;
      };
      await category.updateComplete;
      const categoryLabel = category.shadowRoot!.querySelector(
        '[part~="form-control-label"]'
      ) as HTMLElement;
      expect(
        categoryField.hasAttribute("data-required"),
        `${key} label stays child-owned`
      ).to.be.false;
      expect(
        category.querySelectorAll("span[aria-hidden]").length,
        `${key} has no literal marker`
      ).to.equal(0);
      expect(
        getComputedStyle(categoryLabel, "::after").content,
        `${key} shared marker`
      ).to.contain("required");
    }
    ...
    const comment = commentField.querySelector("lr-textarea") as HTMLElement & {
      updateComplete: Promise<boolean>;
    };
    await comment.updateComplete;
    const commentLabel = comment.shadowRoot!.querySelector(
      '[part~="form-control-label"]'
    ) as HTMLElement;
    expect(
      commentField.hasAttribute("data-required"),
      "comment label stays child-owned"
    ).to.be.false;
    expect(
      comment.querySelectorAll("span[aria-hidden]").length,
      "comment has no literal marker"
    ).to.equal(0);
    expect(
      getComputedStyle(commentLabel, "::after").content,
      "comment shared marker"
    ).to.contain("required");
  });

  it("renders a score field with >10 discrete steps as lr-slider instead of lr-segmented", async () => {
    const keys: RubricKey[] = [
      { key: "score", type: "score", min: 0, max: 100, step: 1 },
    ];
    const el = (await fixture(
      html`<lr-rubric-form .keys=${keys}></lr-rubric-form>`
    )) as LyraRubricForm;
    await el.updateComplete;
    expect(el.shadowRoot!.querySelector('[data-key="score"] lr-slider')).to
      .exist;
    expect(
      el.shadowRoot!.querySelector('[data-key="score"] lr-segmented') == null
    ).to.be.true;
  });

  it("records the synthesized midpoint shown by an unanswered wide slider score", async () => {
    const keys: RubricKey[] = [
      {
        key: "quality",
        type: "score",
    ...
    >('[data-key="quality"] lr-slider')!;
    await slider.updateComplete;

    expect(slider.value, "the visible score").to.equal(50);
    expect(el.value["quality"], "the canonical recorded score").to.equal(50);
    expect(el.checkValidity(), "the displayed answer satisfies requiredness").to
      .be.true;
  });
  ...
  it("localizes visible segmented scores like slider scores while preserving raw values", async () => {
    ...
    const el = (await fixture(html`
      <lr-rubric-form
        locale="ar-EG"
        .keys=${keys}
        .value=${{ compact: 1, wide: 1234 }}
      ></lr-rubric-form>
    `)) as LyraRubricForm;
    await el.updateComplete;
    const segmented = el.shadowRoot!.querySelector(
      '[data-key="compact"] lr-segmented'
    ) as HTMLElement & {
      items: Array<{ value: string; label: string }>;
      updateComplete: Promise<boolean>;
    };
    const slider = el.shadowRoot!.querySelector(
      '[data-key="wide"] lr-slider'
    ) as HTMLElement & {
    ...
    expect(segmented.items.map((item) => item.value)).to.deep.equal([
      "0",
      "1",
      "2",
    ]);
    expect(segmented.items.map((item) => item.label)).to.deep.equal(
      [0, 1, 2].map((value) => formatter.format(value))
    );
    expect(
      slider.shadowRoot.querySelector('[part="value"]')!.textContent
    ).to.equal(formatter.format(1234));
  ...
  it("enumerates a single-point extreme score domain without a non-advancing loop", async () => {
    ...
    ) as (HTMLElement & { items: Array<{ value: string }> }) | null;
  ...
  it("renders a multiple category field as lr-checkbox-group with slotted lr-checkbox options", async () => {
    ...
    const group = el.shadowRoot!.querySelector(
      '[data-key="tags"] lr-checkbox-group'
    ) as HTMLElement;
  ...
  it("emits lr-input with the full value object when a score control changes", async () => {
    ...
    setTimeout(() =>
      segmented.dispatchEvent(
        new CustomEvent("lr-change", { detail: { value: "4" } })
      )
    );
  ...
  it("emits lr-input with the full value object when a multiple category control changes", async () => {
    ...
    setTimeout(() =>
      group.dispatchEvent(
        new CustomEvent("lr-change", { detail: { value: ["a", "b"] } })
      )
    );
    const ev = await oneEvent(el, "lr-input");
  ...
  it("does not emit lr-submit when validity fails, and reveals the error", async () => {
    ...
    const score = el.shadowRoot!.querySelector(
      '[data-key="accuracy"] .control'
    )!;
    expect(
      score.getAttribute("label") ?? score.getAttribute("aria-label")
    ).to.contain(error.textContent?.trim());
  });

  it("switches the submit label between rubricSubmit and rubricSubmitAndNext based on has-next", async () => {
    const el = (await fixture(
      html`<lr-rubric-form .keys=${KEYS} has-next></lr-rubric-form>`
    )) as LyraRubricForm;
    await el.updateComplete;
    expect(
      el.shadowRoot!.querySelector('[part="submit"]')!.textContent!.trim()
    ).to.equal("Submit and next");
  });
  ...
  it("resets touched/error-reveal state when itemId changes", async () => {
    ...
    el.reportValidity();
    await el.updateComplete;
    expect(el.shadowRoot!.querySelector('[data-key="accuracy"] [part="error"]'))
      .to.exist;
    el.itemId = "item-2";
    await el.updateComplete;
    expect(
      el.shadowRoot!.querySelector('[data-key="accuracy"] [part="error"]') ==
        null
    ).to.be.true;
  });
  ...
  it("forwards required to lr-select, lr-checkbox-group, and lr-textarea, matching each sibling’s own required-driven aria state", async () => {
    ...
    const select = el.shadowRoot!.querySelector(
      '[data-key="category"] lr-select'
    ) as HTMLElement & {
      required: boolean;
    };
    const optionalSelect = el.shadowRoot!.querySelector(
      '[data-key="optionalCategory"] lr-select'
    ) as HTMLElement & {
      required: boolean;
    };
    const group = el.shadowRoot!.querySelector(
  ...
  it("does not arm the mount-time auto-focus for a property set before the component has ever updated (regression)", () => {
    ...
    try {
      ...
      const el = container.querySelector("lr-rubric-form") as unknown as {
        pendingFocusFirst: boolean;
      };
  ...
  it("uses the adopted owner realm CSS escape to resolve a special-key validity anchor", async () => {
    ...
    try {
      ...
      expect(anchor?.closest("[data-key]")?.getAttribute("data-key")).to.equal(
        specialKey
      );
    } finally {
      restoreAmbient();
  ...
  it("renders a .strings override for the empty-keys message", async () => {
    const el = (await fixture(
      html`<lr-rubric-form
        .strings=${{ noData: "Rien à évaluer" }}
      ></lr-rubric-form>`
    )) as LyraRubricForm;
    await el.updateComplete;
    expect(
      el.shadowRoot!.querySelector('[part="empty"]')!.textContent
    ).to.equal("Rien à évaluer");
  });
  ...
  it("localizes the required-field and unsupported-field-type messages via .strings, reaching the DOM", async () => {
    ...
    expect(
      el.shadowRoot!.querySelector('[part="unsupported"]')!.textContent
    ).to.equal('Type de champ non pris en charge : "bogus".');
  });
  ...
  it("localizes the submit/submit-and-next/skip button labels via .strings", async () => {
    ...
    expect(
      el.shadowRoot!.querySelector('[part="skip"]')!.textContent!.trim()
    ).to.equal("Passer");
    el.hasNext = true;
    await el.updateComplete;
    expect(
      el.shadowRoot!.querySelector('[part="submit"]')!.textContent!.trim()
    ).to.equal("Envoyer et suivant");
  });
  ...
  it("renders aggregate label, hint, and error chrome with same-shadow ARIA links", async () => {
    ...
    expect(
      label.querySelector("slot")!.assignedElements()[0]?.textContent
    ).to.contain("for this response");
    expect(hint.textContent).to.contain("Score every dimension");
    expect(
      hint
        .querySelector<HTMLSlotElement>('slot[name="hint"]')!
        .assignedElements()[0]?.textContent
    ).to.contain("before submitting");
    expect(error.textContent).to.contain("Review could not be saved");
    expect(
      error.querySelector("slot")!.assignedElements()[0]?.textContent
    ).to.contain("Try again");
    expect(group.getAttribute("aria-labelledby")).to.equal(label.id);
    expect(group.getAttribute("aria-describedby")?.split(/\s+/)).to.deep.equal([
      hint.id,
      error.id,
    ]);
  ...
  it("renders a blocking aggregate custom error when no errorText or error slot is supplied", async () => {
    ...
    expect(group.getAttribute("aria-describedby")?.split(/\s+/)).to.include(
      error.id
    );
  ...
  it("renders a slider (not segmented) for score keys with a non-positive step, a fractional bound, a fractional step count, or a negative step count", async () => {
    ...
    for (const k of keys) {
      expect(
        el.shadowRoot!.querySelector(`[data-key="${k.key}"] lr-slider`),
        `${k.key} should be lr-slider`
      ).to.exist;
      expect(
        el.shadowRoot!.querySelector(`[data-key="${k.key}"] lr-segmented`) ==
          null,
        `${k.key} should not be lr-segmented`
      ).to.be.true;
    }
  });
  ...
  it("defaults to an empty option list for a category field with no options, and reflects a preset string value in a single-select", async () => {
    const keys: RubricKey[] = [
      { key: "catNoOpts", type: "category" },
      {
        key: "category",
        type: "category",
        options: [{ value: "hallucination" }, { value: "tone" }],
      },
    ];
    const el = (await fixture(
      html`<lr-rubric-form
    ...
    await el.updateComplete;
    const emptySelect = el.shadowRoot!.querySelector(
      '[data-key="catNoOpts"] lr-select'
    ) as HTMLElement;
  ...
  it("renders the description paragraph for a key that has one", async () => {
    ...
    await el.updateComplete;
    const desc = el.shadowRoot!.querySelector(
      '[data-key="accuracy"] [part="description"]'
    );
    expect(desc != null).to.equal(true);
    expect(desc!.textContent).to.equal(
  ...
  it("does not recreate the touched-fields set when a field is already touched (markTouched guard)", async () => {
    ...
    await el.updateComplete;
    const touchedAfterFirst = (el as unknown as { touchedFields: Set<string> })
      .touchedFields;
    field.dispatchEvent(new FocusEvent("focusout", { bubbles: true }));
    await el.updateComplete;
    const touchedAfterSecond = (el as unknown as { touchedFields: Set<string> })
      .touchedFields;
  ...
  it("focusFirstControl no-ops (no throw) when keys is empty during an itemId transition", async () => {
    const el = (await fixture(
      html`<lr-rubric-form .keys=${[]}></lr-rubric-form>`
    )) as LyraRubricForm;
    await el.updateComplete;
    el.itemId = "item-1";
    await el.updateComplete;
  ...
  it("dims the disabled submit/skip buttons through the shared disabled-opacity token", async () => {
    ...
    const skip = el.shadowRoot!.querySelector(
      '[part="skip"]'
    ) as HTMLButtonElement;
  ...
  describe("lifecycle: attachInternals guard", () => {
    it("degrades gracefully instead of throwing when ElementInternals is unavailable", async () => {
      ...
      try {
        ...
        await el.updateComplete;
        expect(
          el.shadowRoot!.querySelectorAll('[part="field"]').length
        ).to.equal(KEYS.length);
...
it("forwards field labels, descriptions, errors, and option descriptions to composed semantic owners", async () => {
      ...
      options: [
        {
          value: "unsafe",
          label: "Unsafe",
          description: "Contains unsafe material",
        },
      ],
    },
    {
  ...
  expect(sliderThumb.getAttribute("aria-label")).to.contain(
    "Use the full confidence range"
  );
  ...
  const selectLabel = select.shadowRoot!.querySelector(
    `label[for="${selectTrigger.id}"]`
  ) as HTMLElement;
  const assignedLabel = selectLabel
    .querySelector("slot")!
  ...
  expect(select.querySelector("lr-option")!.sub).to.equal(
...
it("keeps each keyed composed control with its logical field across reorder", async () => {
  ...
  const el = (await fixture(
    html`<lr-rubric-form .keys=${[a, b]}></lr-rubric-form>`
  )) as LyraRubricForm;
  ...
  expect(
    el.shadowRoot!.querySelector('[data-key="a"] lr-textarea') === originalA
  ).to.be.true;
  expect(
    el.shadowRoot!.querySelector('[data-key="b"] lr-textarea') === originalB
  ).to.be.true;
});
...
it("reportValidity() reveals current field errors and answers overall validity", async () => {
  ...
  await el.updateComplete;
  expect(typeof initial, "reportValidity answers a boolean").to.equal(
    "boolean"
  );
...
describe("validity custom states", () => {
  it("publishes required/optional from the keys, and valid/invalid from the answers", async function () {
    ...
    const el = (await fixture(
      html`<lr-rubric-form .keys=${KEYS} name="rubric"></lr-rubric-form>`
    )) as LyraRubricForm;
    await el.updateComplete;
    ...
    await el.updateComplete;
    expect(
      el.matches(":state(valid)"),
      "valid once the required key is answered"
    ).to.be.true;
    expect(
      el.matches(":state(invalid)"),
      "invalid once the required key is answered"
    ).to.be.false;

    el.keys = KEYS.map((key) => ({ ...key, required: false }));
    await el.updateComplete;
    expect(el.matches(":state(optional)"), "optional once no key is required")
      .to.be.true;
    expect(el.matches(":state(required)"), "required once no key is required")
      .to.be.false;
  });
  ...
  it("keeps user-valid/user-invalid off a pristine rubric until reportValidity() reveals the errors", async function () {
    ...
    expect(el.matches(":state(user-invalid)"), "user-invalid after reset").to.be
      .false;
    expect(el.matches(":state(user-valid)"), "user-valid after reset").to.be
      .false;
  ...
  it("does not turn a disabled field focusout into user interaction after re-enabling", async function () {
    ...
    expect(
      ...
      "a disable-forced focusout leaves the rubric pristine"
    ).to.be.false;
  });
  ...
  it("publishes neither invalid nor user-invalid while disabled", async function () {
    ...
    expect(
      el.matches(":state(valid)"),
      "barred matches neither half of the pair"
    ).to.be.false;
    expect(
      el.matches(":state(required)"),
      "requiredness describes the keys, not the outcome"
    ).to.be.true;
    el.reportValidity();
    await el.updateComplete;
    ...
    el.disabled = false;
    await el.updateComplete;
    expect(el.checkValidity(), "the constraint returns with the control").to.be
      .false;
    expect(el.matches(":state(invalid)"), "invalid once enabled again").to.be
      .true;
  });
  ...
  it("publishes neither invalid nor user-invalid inside a disabled fieldset", async function () {
    ...
    const form = (await fixture(html`
      <form>
        <fieldset disabled>
          <lr-rubric-form .keys=${KEYS} name="rubric"></lr-rubric-form>
        </fieldset>
      </form>
    `)) as HTMLFormElement;
    const el = form.querySelector("lr-rubric-form") as LyraRubricForm;
    await el.updateComplete;
    expect(el.disabled, "a fieldset never mutates the control own disabled").to
      .be.false;
    expect(
      el.validity.valueMissing,
      "fieldset-disabled bars validation exactly like own disabled"
    ).to.be.false;
...
describe("lr-rubric-form setCustomValidity()", () => {
  const optionalKeys: RubricKey[] = [
    { key: "comment", type: "comment", label: "Comment" },
  ];
  ...
  it("blocks form submission and becomes the validationMessage", async () => {
    ...
    expect(el.validationMessage).to.equal(
      "This item was already annotated by someone else"
    );
...
describe("canonical rubric schema/value model", () => {
  ...
  it("normalizes every external write once before render, validity, readout, and FormData", async () => {
    ...
    >('[data-key="bounded"] lr-slider')!;
    const select = el.shadowRoot!.querySelector<
      HTMLElement & { value: string }
    >('[data-key="category"] lr-select')!;
    expect(slider.value).to.equal(100);
    expect(select.value).to.equal("");
    expect(new FormData(form).get("rubric")).to.equal(
      '{"bounded":100,"tags":["a","b"]}'
    );

    const snapshot = el.value;
    expect(() => (snapshot["tags"] as string[]).push("mutated")).to.throw(
      TypeError
    );
  ...
  it("renormalizes live/default/restored values when the schema changes and resets explicitly", async () => {
    ...
    el.formStateRestoreCallback(
      '{"bounded":37,"category":"unknown","extra":1}',
      "restore"
    );
  ...
  it("republishes user validity atomically when item identity clears interaction", async function () {
    ...
    expect(
      el.matches(":state(user-invalid)"),
      "new item is pristine synchronously"
    ).to.equal(false);
    expect(
      el.matches(":state(invalid)"),
      "intrinsic requiredness remains"
    ).to.equal(true);
  });
  ...
  it("falls back to an empty type string for a runtime key with no type property at all", async () => {
    const el = (await fixture(
      html`<lr-rubric-form
        .strings=${{ unsupportedFieldType: 'Type: "{type}".' }}
      ></lr-rubric-form>`
    )) as LyraRubricForm;
  ...
  it("retains no selections for a multiple-category value when the key declares no options", async () => {
    const el = (await fixture(html`
      <lr-rubric-form
        .keys=${[
          { key: "tags", type: "category", multiple: true },
        ] as RubricKey[]}
        .value=${{ tags: ["a", "b"] }}
      ></lr-rubric-form>
    `)) as LyraRubricForm;
  ...
  it("drops a multiple-category value that is not an array", async () => {
    ...
    const el = (await fixture(html`
      <lr-rubric-form
        .keys=${keys}
        .value=${{ tags: "oops" } as unknown as Record<string, unknown>}
      ></lr-rubric-form>
    `)) as LyraRubricForm;
    await el.updateComplete;
    expect(el.value).to.deep.equal({});
  ...
  it("drops non-string candidates from a multiple-category array value", async () => {
    ...
    const el = (await fixture(html`
      <lr-rubric-form
        .keys=${keys}
        .value=${{ tags: ["a", 42, null, "b"] } as unknown as Record<
          string,
          unknown
        >}
      ></lr-rubric-form>
  ...
  it("set defaultValue does not overwrite an already-dirtied live value", async () => {
    ...
    await el.updateComplete;
    expect(el.value, "live value stays at the explicit write").to.deep.equal({
      accuracy: 5,
    });
    expect(el.defaultValue, "defaultValue itself still updates").to.deep.equal({
      accuracy: 3,
    });
  });
  ...
  it("normalizes a nullish defaultValue assignment to the safe empty value", async () => {
    ...
    expect(
      el.value,
      "a pristine live value follows the reset default"
    ).to.deep.equal({});
  });
  ...
  it("setCustomValidity treats a nullish message the same as an empty string (runtime safety)", async () => {
    ...
    expect(el.validity.customError).to.be.true;
    (el.setCustomValidity as (message: string) => void)(
...
describe("lr-rubric-form remaining public accessors and interaction paths", () => {
  ...
  it("emits lr-input with the full value object when a non-segmented (slider) score control changes", async () => {
    const keys: RubricKey[] = [
      { key: "wide", type: "score", min: 0, max: 100, step: 1 },
    ];
    const el = (await fixture(
      html`<lr-rubric-form .keys=${keys}></lr-rubric-form>`
    )) as LyraRubricForm;
    ...
    setTimeout(() =>
      slider.dispatchEvent(
        new CustomEvent("lr-change", { detail: { value: 42 } })
      )
    );
    const ev = await oneEvent(el, "lr-input");
  ...
  it("contains a slider child lr-input alias instead of leaking its scalar detail", async () => {
    const keys: RubricKey[] = [
      { key: "wide", type: "score", min: 0, max: 100, step: 1 },
    ];
    const el = (await fixture(
    ...
    el.addEventListener("lr-input", (event) => {
      details.push((event as CustomEvent<{ value: unknown }>).detail.value);
    });
    ...
    expect(
      details,
      "no child-shaped alias crosses the aggregate host"
    ).to.deep.equal([]);
  });
  ...
  it("contains a select child lr-input alias instead of leaking its scalar detail", async () => {
    const keys: RubricKey[] = [
      {
        key: "category",
        type: "category",
  ...
  it("renders the inline error span for a required comment field once revealed", async () => {
    const keys: RubricKey[] = [
      { key: "comment", type: "comment", label: "Notes", required: true },
    ];
    const el = (await fixture(
      html`<lr-rubric-form .keys=${keys}></lr-rubric-form>`
    )) as LyraRubricForm;
    ...
    const errorSpan = el.shadowRoot!.querySelector(
      '[data-key="comment"] lr-textarea span[slot="error"]'
    );
