...
it("bars constraint validation while disabled, fieldset-disabled or readonly", async () => {
  ...
  expect(
    ...
    "disabled must not be :state(user-invalid)"
  ).to.be.false;
  ...
  expect(nested.disabled, "a fieldset never mutates the control own disabled")
    .to.be.false;
  expect(nested.validity.valueMissing, "fieldset-disabled + required").to.be
    .false;
...
it("suppresses host click/focus in the same task that fieldset disablement starts", async () => {
  const form = (await fixture(html`
    <form>
      <fieldset><lr-textarea></lr-textarea></fieldset>
    </form>
  `)) as HTMLFormElement;
...
it("reflects rows/placeholder onto the native textarea", async () => {
  const el = (await fixture(
    html`<lr-textarea rows="6" placeholder="Notes"></lr-textarea>`
  )) as LyraTextarea;
  const textarea = el.shadowRoot!.querySelector(
    "textarea"
  ) as HTMLTextAreaElement;
...
it("applies resize onto the native textarea", async () => {
  const el = (await fixture(
    html`<lr-textarea resize="none"></lr-textarea>`
  )) as LyraTextarea;
  const textarea = el.shadowRoot!.querySelector(
    "textarea"
  ) as HTMLTextAreaElement;
...
it("participates in native form validation via required", async () => {
  const el = (await fixture(
    html`<lr-textarea required name="notes"></lr-textarea>`
  )) as LyraTextarea;
  expect(el.checkValidity()).to.be.false;
  el.value = "filled in";
...
describe("readonly", () => {
  ...
  it("keeps programmatic range edits form-synchronized and silent while readonly", async () => {
    const form = (await fixture(html`
      <form>
        <lr-textarea name="notes" value="hello world" readonly></lr-textarea>
      </form>
    `)) as HTMLFormElement;
    const el = form.querySelector("lr-textarea") as LyraTextarea;
    const seen: string[] = [];
...
describe("length constraints", () => {
  it("forwards minlength/maxlength onto the native textarea", async () => {
    const el = (await fixture(
      html`<lr-textarea minlength="2" maxlength="8"></lr-textarea>`
    )) as LyraTextarea;
  ...
  it("bridges tooLong to the host validity", async () => {
    const el = (await fixture(
      html`<lr-textarea maxlength="3"></lr-textarea>`
    )) as LyraTextarea;
    const native = el.shadowRoot!.querySelector(
      "textarea"
    ) as HTMLTextAreaElement;
  ...
  it("bridges tooShort to the host validity", async () => {
    const el = (await fixture(
      html`<lr-textarea minlength="5"></lr-textarea>`
    )) as LyraTextarea;
    const native = el.shadowRoot!.querySelector(
      "textarea"
    ) as HTMLTextAreaElement;
  ...
  it("recomputes validity when maxlength narrows below the current value without a value write", async () => {
    const el = (await fixture(
      html`<lr-textarea value="abcdef"></lr-textarea>`
    )) as LyraTextarea;
    expect(el.checkValidity()).to.equal(true);
    el.maxlength = 3;
    await el.updateComplete;
  ...
  it("keeps the localized required message and reports required + empty ahead of any length check", async () => {
    const el = (await fixture(
      html`<lr-textarea required minlength="5"></lr-textarea>`
    )) as LyraTextarea;
  ...
  it("unset regression: renders and validates exactly as before when minlength/maxlength are unset", async () => {
    ...
    el.value =
      "a value far longer than any plausible default limit would ever allow";
    expect(el.checkValidity()).to.equal(true);
    expect(el.validity.tooLong).to.equal(false);
    expect(el.validity.tooShort).to.equal(false);
  ...
  it("uses one native-effective limit for negative and fractional maxlength values", async () => {
    ...
    expect(
      el.shadowRoot!.querySelector('[part="count"]')!.textContent!.trim()
    ).to.equal("3 chars");
...
it("is accessible", async () => {
  const el = (await fixture(
    html`<lr-textarea placeholder="Notes"></lr-textarea>`
  )) as LyraTextarea;
...
describe("label/hint/error chrome", () => {
  ...
  it("shows a required asterisk on the label only when required", async () => {
    const el = (await fixture(
      html`<lr-textarea label="Notes" required></lr-textarea>`
    )) as LyraTextarea;
    const label = el.shadowRoot!.querySelector(
      '[part="form-control-label"]'
    ) as HTMLElement;
...
describe("accessibleLabel", () => {
  it("falls back to placeholder, then the localized default, when unset", async () => {
    const noLabel = (await fixture(
      html`<lr-textarea></lr-textarea>`
    )) as LyraTextarea;
    const withPlaceholder = (await fixture(
      html`<lr-textarea placeholder="Code"></lr-textarea>`
    )) as LyraTextarea;
    const ta1 = noLabel.shadowRoot!.querySelector(
      "textarea"
    ) as HTMLTextAreaElement;
    const ta2 = withPlaceholder.shadowRoot!.querySelector(
      "textarea"
    ) as HTMLTextAreaElement;
  ...
  it("routes the default textbox name through localization overrides", async () => {
    const el = (await fixture(html`
      <lr-textarea
        .strings=${{ textareaLabel: "Texte multiligne" }}
      ></lr-textarea>
    `)) as LyraTextarea;
    const ta = el.shadowRoot!.querySelector("textarea") as HTMLTextAreaElement;
...
describe('resize="auto"', () => {
  it('accepts "auto" as a resize value and sets native CSS resize to none (no manual handle)', async () => {
    const el = (await fixture(
      html`<lr-textarea resize="auto"></lr-textarea>`
    )) as LyraTextarea;
    const ta = el.shadowRoot!.querySelector("textarea") as HTMLTextAreaElement;
  ...
  it("grows the textarea block-size to fit typed content that exceeds the initial rows", async () => {
    const el = (await fixture(
      html`<lr-textarea resize="auto" rows="1"></lr-textarea>`
    )) as LyraTextarea;
    const ta = el.shadowRoot!.querySelector("textarea") as HTMLTextAreaElement;
...
describe("input / setRangeText()", () => {
  it("exposes the native textarea via the public input getter", async () => {
    ...
    expect(el.input === el.shadowRoot!.querySelector("textarea")).to.equal(
      true
    );
  });
...
describe("touched state", () => {
  ...
  it("does not mark touched from a blur the platform forces when the control becomes disabled while focused", async () => {
    const el = (await fixture(
      html`<lr-textarea required></lr-textarea>`
    )) as LyraTextarea;
    const ta = el.shadowRoot!.querySelector("textarea") as HTMLTextAreaElement;
    ta.focus();
...
describe("lr-textarea size", () => {
  ...
  it('defaults to size "m" and reflects the attribute', async () => {
    ...
    const sized = await fixture<LyraTextarea>(
      html`<lr-textarea size="s" aria-label="Notes"></lr-textarea>`
    );
  ...
  it('leaves the committed padding/font-size untouched at the default tier and tightens them at "xs"', async () => {
    const mEl = await fixture<LyraTextarea>(
      html`<lr-textarea aria-label="a"></lr-textarea>`
    );
    const xsEl = await fixture<LyraTextarea>(
      html`<lr-textarea size="xs" aria-label="b"></lr-textarea>`
    );
...
describe("lr-textarea appearance", () => {
  ...
  it("renders each other appearance distinctly", async () => {
    const outlined = await fixture<LyraTextarea>(
      html`<lr-textarea appearance="outlined" aria-label="a"></lr-textarea>`
    );
    const filled = await fixture<LyraTextarea>(
      html`<lr-textarea appearance="filled" aria-label="b"></lr-textarea>`
    );
    const plain = await fixture<LyraTextarea>(
      html`<lr-textarea appearance="plain" aria-label="c"></lr-textarea>`
    );
    const accent = await fixture<LyraTextarea>(
    ...
    expect(getComputedStyle(fieldOf(plain)).backgroundColor).to.equal(
      TRANSPARENT
    );
    expect(getComputedStyle(fieldOf(plain)).borderTopColor).to.equal(
      TRANSPARENT
    );
...
describe("lr-textarea wrapper parts", () => {
  it("wraps the native textarea in a textarea-wrapper part", async () => {
    ...
    expect(
      el.shadowRoot!.querySelectorAll('[part~="textarea-wrapper"]').length
    ).to.equal(1);
  ...
  it("keeps the footer part out of the layout until with-count is set", async () => {
    ...
    expect(el.shadowRoot!.querySelectorAll('[part="footer"]').length).to.equal(
      1
    );
...
describe("lr-textarea with-count", () => {
  const countOf = (el: LyraTextarea) =>
    el.shadowRoot!.querySelector('[part="count"]') as HTMLElement;

  it("renders no count element by default", async () => {
    const el = await fixture<LyraTextarea>(
      html`<lr-textarea value="abc" aria-label="Notes"></lr-textarea>`
    );
    expect(el.withCount).to.be.false;
    expect(el.shadowRoot!.querySelectorAll('[part="count"]').length).to.equal(
      0
    );
  });
  ...
  it("keeps both shadow count copies out of the accessibility tree and pre-mounts a polite sink", async () => {
    ...
    expect(
      el.shadowRoot!.querySelector('[aria-live], [role="status"]') === null
    ).to.be.true;
    const sink = document.querySelector<HTMLElement>(
      `[${ANNOUNCEMENT_SINK_ATTRIBUTE}="polite"]`
    )!;
    expect(Boolean(sink), "the sink is mounted before typing starts").to.be
      .true;
...
describe("lr-textarea with-count live announcement", () => {
  ...
  it("rebinds resize observation, animation frames, and count timers to an adopted realm", async () => {
    ...
    try {
      ...
      textarea.value = "abc";
      textarea.dispatchEvent(
        new frameWindow.InputEvent("input", { bubbles: true })
      );
...
describe("lr-textarea — the shared size ladder and pill", () => {
  ...
  it("renders the Web Awesome size spellings at the same geometry as the canonical steps", async () => {
    for (const [alias, step] of [
      ...
      expect(
        getComputedStyle(field(aliasEl)).fontSize,
        `size=${alias} font-size`
      ).to.equal(getComputedStyle(field(stepEl)).fontSize);
      expect(
        getComputedStyle(field(aliasEl)).paddingTop,
        `size=${alias} padding`
      ).to.equal(getComputedStyle(field(stepEl)).paddingTop);
    }
  });
  ...
  it("rounds the field to a pill, and leaves the corner radius alone when pill is unset", async () => {
    ...
    expect(getComputedStyle(field(pilled)).borderTopLeftRadius).to.equal(
      "999px"
    );
  });
});
...
describe("lr-textarea mapped Textarea parity surface", () => {
  ...
  it("accepts help-text slot and exports form-control-input/adjuster part aliases", async () => {
    ...
    `);
    expect(el.shadowRoot!.querySelector('[part~="form-control-input"]')).to
      .exist;
    expect(el.shadowRoot!.querySelector('[part~="textarea-adjuster"]')).to
      .exist;
    expect(el.shadowRoot!.querySelector('[part~="form-control-help-text"]')).to
      .exist;
    const help = el.shadowRoot!.querySelector(
      'slot[name="help-text"]'
    ) as HTMLSlotElement;
  ...
  it("keeps a boolean autocorrect read while accepting both upstream write vocabularies", async () => {
    ...
    expect(
      el.autocorrect,
      "attribute removal restores the true default"
    ).to.equal(true);
    expect(
      native.hasAttribute("autocorrect"),
      "the native control resumes its browser default"
    ).to.equal(false);
  });
...
it("mirrors the lowercase IDL aliases of the native input hints", async () => {
  const el = (await fixture(
    html`<lr-textarea label="Notes"></lr-textarea>`
  )) as LyraTextarea;
