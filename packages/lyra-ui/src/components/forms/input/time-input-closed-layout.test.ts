...
// `lr-time-input`'s picker popup defaults to `position: fixed`, so a bare `overflow: auto`
...
for (const phase of ['initial', 'closed after resize'] as const) {
  it(`keeps a ${phase} time-input popup out of the scrollable layout`, async () => {
    const container = await fixture<HTMLDivElement>(html`
      <div style="transform: translateZ(0); width: 320px; height: 200px; overflow: auto; display: flex; justify-content: end; align-items: start">
        <lr-time-input style="width: 120px; --lr-duration-fast: 0s"></lr-time-input>
      </div>
    `);
    const el = container.querySelector<LyraTimeInput>('lr-time-input')!;
...
it('preserves the time-input popup close transition while removing the settled closed layout', async () => {
  const el = await fixture<LyraTimeInput>(
    html`<lr-time-input style="--show-duration: 1s; --hide-duration: 1s"></lr-time-input>`
  );
  const panel = el.shadowRoot!.querySelector<HTMLElement>('[part="popup"]')!;
  const shown = el.show();
