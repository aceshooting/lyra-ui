...
it("accepts WA hint and Shoelace help-text spellings on one accessible hint surface", async () => {
  const wa = (await fixture(
    html`<lr-checkbox hint="WA hint">Choice</lr-checkbox>`
  )) as LyraCheckbox;
  const sl = (await fixture(
    html` <lr-checkbox help-text="Shoelace hint">Choice</lr-checkbox> `
  )) as LyraCheckbox & {
    helpText: string;
  };
...
it("renders static error text in the standard form-control frame and describes the checkbox", async () => {
  ...
  `)) as LyraCheckbox;
  const frame = el.shadowRoot!.querySelector<HTMLElement>(
    '[part="form-control"]'
  );
  const error = el.shadowRoot!.querySelector<HTMLElement>('[part="error"]')!;
  const control = el.shadowRoot!.querySelector<HTMLElement>(
    '[part~="base"]'
  ) as HTMLElement & {
    ariaDescribedByElements?: Element[] | null;
  };
  ...
  if ("ariaDescribedByElements" in control) {
    ...
    expect(control.getAttribute("aria-describedby")).to.equal(
...
it("tracks slotted error content added and removed after mount", async () => {
  const el = (await fixture(
    html`<lr-checkbox>Accept terms</lr-checkbox>`
  )) as LyraCheckbox;
  const error = el.shadowRoot!.querySelector<HTMLElement>('[part="error"]')!;
  ...
  if ("ariaDescribedByElements" in control) {
    ...
    expect(control.getAttribute("aria-describedby")).to.contain(
      "checkbox-error"
    );
  }
  ...
  if ("ariaDescribedByElements" in control) {
    ...
    expect(control.getAttribute("aria-describedby") ?? "").to.not.contain(
      "checkbox-error"
    );
  }
});
...
it("exports additive WA/Shoelace control and state part aliases", async () => {
  const checked = (await fixture(
    html`<lr-checkbox checked>Checked</lr-checkbox>`
  )) as LyraCheckbox;
  const control = checked.shadowRoot!.querySelector(
    '[part~="control"]'
  ) as HTMLElement;
  const checkedIcon = checked.shadowRoot!.querySelector(
    '[part~="checked-icon"]'
  ) as SVGElement;
  ...
  const mixedControl = checked.shadowRoot!.querySelector(
    '[part~="control"]'
  ) as HTMLElement;
  const mixedIcon = checked.shadowRoot!.querySelector(
    '[part~="indeterminate-icon"]'
  ) as SVGElement;
  expect(mixedControl.getAttribute("part")!.split(/\s+/)).to.include(
...
it("does not treat fictional default-checked as a reset-default alias", async () => {
  ...
  el.checked = true;
  form.reset();
  expect(el.checked, "only canonical checked/defaultChecked controls reset").to
    .be.false;
});
...
it("emits exactly one native Event pair and one prefixed alias pair for user toggles", async () => {
  ...
  expect(observed.map(({ name }) => name)).to.deep.equal([
    "input",
    "lr-input",
    "change",
    "lr-change",
...
it("sets aria-required when required", async () => {
  const el = (await fixture(
    html`<lr-checkbox required>Label</lr-checkbox>`
  )) as LyraCheckbox;
  const base = el.shadowRoot!.querySelector('[part~="base"]') as HTMLElement;
...
it("preserves an explicitly empty host aria-label on the internal checkbox role", async () => {
  const el = (await fixture(
    html`<lr-checkbox aria-label="">Visible label</lr-checkbox>`
  )) as LyraCheckbox;
  const base = el.shadowRoot!.querySelector('[part~="base"]') as HTMLElement;
...
describe("aria-describedby forwarding", () => {
  it('resolves host description ids onto the inner role="checkbox" element', async () => {
    const el = (await fixture(html`
      <div>
        <span id="description"
          >This option is unavailable during maintenance.</span
        >
    ...
    const base = checkbox.shadowRoot!.querySelector(
      '[part~="base"]'
    ) as HTMLElement & {
...
describe("indeterminate", () => {
  it('reflects aria-checked="mixed" regardless of the underlying checked value', async () => {
    ...
    await el.updateComplete;
    expect(
      base.getAttribute("aria-checked"),
      "indeterminate still wins over checked"
    ).to.equal("mixed");
  });
...
it("updates validity synchronously when required changes", async () => {
  const el = (await fixture(
    html` <lr-checkbox name="terms">Agree</lr-checkbox> `
  )) as LyraCheckbox;
...
it("does not force focus when the native invalid event is canceled", async () => {
  ...
  expect((document.activeElement as HTMLElement | null)?.id).to.equal(
    "cancel-sentinel"
  );
  expect(el.shadowRoot!.activeElement?.getAttribute("part")).to.equal(
    undefined
...
it("applies and removes explicit disabled form state synchronously", async () => {
  const form = (await fixture(html`
    <form>
      <lr-checkbox id="submitted" name="notify" value="yes" checked
        >Notify me</lr-checkbox
      >
...
it("restores the declared default checked state on form.reset()", async () => {
  const form = (await fixture(html`
    <form>
      <lr-checkbox name="notify" value="yes" checked required
        >Notify me</lr-checkbox
      >
    </form>
  `)) as HTMLFormElement;
...
it("temporarily disables through a fieldset without overwriting the author disabled state", async () => {
  const form = (await fixture(html`
    <form>
      <fieldset>
        <lr-checkbox name="notify" value="yes" checked>Notify me</lr-checkbox>
        <lr-checkbox name="always-disabled" value="yes" checked disabled
          >Always disabled</lr-checkbox
        >
      </fieldset>
    </form>
  `)) as HTMLFormElement;
...
it("does not mark touched/hasInteracted from a blur caused by the control itself becoming disabled", async () => {
  ...
  expect(
    ...
    "precondition: the control is focused"
  ).to.be.true;
  ...
  expect(isTouched(), "a disable-forced blur must not mark touched").to.be
    .false;
  expect(hasInteracted(), "a disable-forced blur must not mark hasInteracted")
    .to.be.false;
  await el.updateComplete;
  el.disabled = false;
  await el.updateComplete;
...
it("does not mark touched if an ancestor fieldset disabling a focused control also force-blurs it", async () => {
  ...
  expect(
    ...
    "fieldset disabling never sets the host attribute"
  ).to.be.false;
  expect(isTouched(), "a fieldset-disable-forced blur must not mark touched").to
    .be.false;
...
describe("validity styling", () => {
  ...
  it("reflects aria-invalid and data-invalid once a required, unchecked control is blurred", async () => {
    ...
    base.dispatchEvent(new FocusEvent("blur"));
    expect(el.hasAttribute("data-invalid"), "data-invalid is synchronous").to.be
      .true;
    await el.updateComplete;
    expect(base.getAttribute("aria-invalid")).to.equal("true");
  ...
  it("never reflects aria-invalid/data-invalid on a non-required, touched control", async () => {
    const el = (await fixture(
      html`<lr-checkbox>Agree</lr-checkbox>`
    )) as LyraCheckbox;
    const base = el.shadowRoot!.querySelector('[part~="base"]') as HTMLElement;
    base.dispatchEvent(new FocusEvent("blur"));
    await el.updateComplete;
...
it("tracks visual label presence through a forwarding slot without exposing its fallback", async () => {
  ...
  await settle();
  expect(label.hidden, "real assigned elements retain visual slot presence").to
    .be.false;
...
it("constructs its label observer in the adopted owner realm", async () => {
  ...
  const el = (await fixture(
    html`<lr-checkbox><span>Parent label</span></lr-checkbox>`
  )) as LyraCheckbox;
  adoptedTarget = el;
  el.remove();
  try {
    ...
    expect(
      labelHostObservations,
      "the adopted-realm label observer binds the checkbox host"
    ).to.be.greaterThan(0);
    expect(
...
it("is accessible in an indeterminate, labeled state", async () => {
  const el = (await fixture(
    html`<lr-checkbox indeterminate>Select all</lr-checkbox>`
  )) as LyraCheckbox;
  await expect(el).to.be.accessible();
});
...
it("keeps interactive label content outside the checkbox semantic owner", async () => {
  const el = (await fixture(
    html`<lr-checkbox
      >Accept the <button type="button">Terms of Service</button></lr-checkbox
    >`
  )) as LyraCheckbox;
  ...
  expect(el.checked, "the nested action does not activate the checkbox").to.be
    .false;

  const label = el.shadowRoot!.querySelector('[part="label"]') as HTMLElement;
  label.click();
  expect(el.checked, "ordinary label activation still toggles the checkbox").to
    .be.true;
});

it("inherits --lr-checkbox-label-indent and lets a direct host value remain authoritative", async () => {
  const wrapper = await fixture<HTMLElement>(html`
    <div style="--lr-checkbox-label-indent: 3rem">
...
describe("size", () => {
  ...
  it("keeps a label-less compact checkbox on the shared minimum target floor", async () => {
    for (const size of ["2xs", "xs", "s", "m", "l", "xl"] as const) {
      ...
      expect(
        boxRect.width,
        `${size} visible box remains tier-sized`
      ).to.be.lessThan(40);
      expect(
        boxRect.height,
        `${size} visible box remains tier-sized`
  ...
  it('renders "small"/"large" at the same geometry as "s"/"l"', async () => {
    ...
    const small = await boxOf(
      html`<lr-checkbox size="small">Label</lr-checkbox>`
    );
    const l = await boxOf(html`<lr-checkbox size="l">Label</lr-checkbox>`);
    const large = await boxOf(
      html`<lr-checkbox size="large">Label</lr-checkbox>`
    );
  ...
  it("keeps the rendered label indent in step with the box at every tier", async () => {
    ...
    for (const size of ["2xs", "xs", "s", "m", "l", "xl"] as const) {
      const el = (await fixture(
        html`<lr-checkbox size=${size}>Label</lr-checkbox>`
      )) as LyraCheckbox;
      await el.updateComplete;
      const base = el.shadowRoot!.querySelector(
        '[part~="base"]'
      ...
      const labelMargin = Number.parseFloat(
        getComputedStyle(label).marginInlineStart
      );
      expect(measured, `${size} indent`).to.be.closeTo(
        ownerWidth + labelMargin,
        0.5
      );
      expect(boxWidth, `${size} box grows with the tier`).to.be.greaterThan(
        previous
      );
      previous = boxWidth;
    }
  ...
  it("is accessible at a non-default tier", async () => {
    const el = (await fixture(
      html`<lr-checkbox size="l">Label</lr-checkbox>`
    )) as LyraCheckbox;
    await el.updateComplete;
    await expect(el).to.be.accessible();
  });
...
describe("lr-checkbox validity custom states", () => {
  it("publishes required/optional and valid/invalid from the first render", async function () {
    ...
    const optional = (await fixture(
      html`<lr-checkbox>Terms</lr-checkbox>`
    )) as LyraCheckbox;
...
describe("lr-checkbox setCustomValidity()", () => {
  ...
  it("keeps the custom error across a form reset", async () => {
    const form = (await fixture(
      html` <form><lr-checkbox name="terms">Agree</lr-checkbox></form> `
    )) as HTMLFormElement;
    const el = form.querySelector("lr-checkbox") as LyraCheckbox;
    el.setCustomValidity("Server says no");
    form.reset();
...
it("shows the hint region when hint content arrives only through the slot", async () => {
  const el = (await fixture(html`
    <lr-checkbox
      >Subscribe<span slot="hint">We never share it</span></lr-checkbox
    >
  `)) as LyraCheckbox;
  await el.updateComplete;
  const hint = el.shadowRoot!.querySelector('[part~="hint"]') as HTMLElement;
...
it("bars constraint validation while disabled, like a native disabled required control", async () => {
  const el = (await fixture(
    html`<lr-checkbox required disabled>Accept</lr-checkbox>`
  )) as LyraCheckbox;
  await el.updateComplete;
