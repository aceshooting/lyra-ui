...
describe("lr-button", () => {
  ...
  it("never fires click while disabled or loading (native disabled button semantics)", async () => {
    const disabledEl = (await fixture(
      html`<lr-button disabled>Save</lr-button>`
    )) as LyraButton;
    let calls = 0;
    disabledEl.addEventListener("click", () => calls++);
    (
      disabledEl.shadowRoot!.querySelector(
        'button[part~="base"]'
      ) as HTMLButtonElement
    ).click();

    const loadingEl = (await fixture(
      html`<lr-button .loading=${true}>Save</lr-button>`
    )) as LyraButton;
    loadingEl.addEventListener("click", () => calls++);
    (
      loadingEl.shadowRoot!.querySelector(
        'button[part~="base"]'
      ) as HTMLButtonElement
    ).click();
  ...
  it("forwards a host aria-label onto the internal button as a literal string", async () => {
    const el = (await fixture(
      html`<lr-button aria-label="Close dialog" appearance="plain"
        ><svg slot="start"></svg
      ></lr-button>`
    )) as LyraButton;
    const button = el.shadowRoot!.querySelector(
  ...
  it("keeps accessibleLabel reactive in anchor mode too", async () => {
    const el = (await fixture(
      html`<lr-button href="/settings" aria-label="Open settings"
        >Settings</lr-button
      >`
    )) as LyraButton;
  ...
  it('type="submit" requests submit on the closest ancestor form (a shadow-internal button cannot do this on its own)', async () => {
    ...
    (
      el.shadowRoot!.querySelector('button[part~="base"]') as HTMLButtonElement
    ).click();
  ...
  it("submits and resets through an external form owner, including submitter overrides", async () => {
    ...
    const seen: { action: FormDataEntryValue | null; method: string } = {
      action: null,
      method: "",
    };
    form.addEventListener("submit", (event) => {
    ...
    expect(
      form.querySelectorAll("button").length,
      "transient submitter is removed"
    ).to.equal(0);
  ...
  it("forwards host click() to the internal native button", async () => {
    const form = (await fixture(
      html`<form><lr-button type="submit">Save</lr-button></form>`
    )) as HTMLFormElement;
    const el = form.querySelector("lr-button") as LyraButton;
    let submitted = false;
    form.addEventListener("submit", (event) => {
  ...
  it('supports appearance="quiet": muted border/text tokens, transparent until hover', async () => {
    ...
    const el = (await fixture(
      html`<lr-button appearance="quiet">Save</lr-button>`
    )) as LyraButton;
    await el.updateComplete;
    const base = el.shadowRoot!.querySelector('[part~="base"]') as HTMLElement;
    const computed = getComputedStyle(base);
    expect(computed.backgroundColor, "quiet is transparent at rest").to.equal(
      "rgba(0, 0, 0, 0)"
    );
    expect(
      computed.color,
  ...
  it('keeps appearance="quiet"\'s text/border independent of variant (unlike outlined)', async () => {
    ...
    const neutralBase = neutralEl.shadowRoot!.querySelector(
      '[part~="base"]'
    ) as HTMLElement;
    const dangerBase = dangerEl.shadowRoot!.querySelector(
      '[part~="base"]'
    ) as HTMLElement;
    expect(getComputedStyle(neutralBase).color).to.equal(
      getComputedStyle(dangerBase).color
    );
    expect(getComputedStyle(neutralBase).borderColor).to.equal(
      getComputedStyle(dangerBase).borderColor
    );
  });
  ...
  it("cascades disabled state from an ancestor fieldset without mutating the disabled property", async () => {
    ...
    expect(
      el.hasAttribute("disabled"),
      "fieldset state must not mutate the host attribute either"
    ).to.be.false;
    expect(el.effectiveDisabled, "the button reflects inherited fieldset state")
      .to.be.true;
    ...
    expect(
      button.disabled,
      "the internal native button reflects the inherited state"
    ).to.be.true;
  ...
  it('supports appearance="accent" as a loud filled tier distinct from "filled" for variant="neutral"', async () => {
    ...
    const filledBase = filledEl.shadowRoot!.querySelector(
      '[part~="base"]'
    ) as HTMLElement;
    const accentBase = accentEl.shadowRoot!.querySelector(
      '[part~="base"]'
    ) as HTMLElement;
  ...
  it("matches lr-input's/lr-select's shared control height at every size tier so a button never sits shorter than its row neighbors", async () => {
    ...
    for (const [size, px] of Object.entries(expected)) {
      const el = (await fixture(
        html`<lr-button size=${size}>Go</lr-button>`
      )) as LyraButton;
      const baseEl = el.shadowRoot!.querySelector(
        '[part~="base"]'
      ) as HTMLElement;
      expect(getComputedStyle(baseEl).minBlockSize, `size=${size}`).to.equal(
        px
      );
    }
  });
  ...
  it("keeps compact circle and icon-only buttons on the shared minimum target floor", async () => {
    for (const size of ["2xs", "xs", "s", "m", "l", "xl"] as const) {
      const fixtures = [
        ...
        html`<lr-button size=${size} aria-label="Settings"
  ...
  it("honors inherited theme hooks while direct-host values remain authoritative", async () => {
    const wrapper = await fixture(html`
      ...
      >
        <lr-button id="inherited" appearance="filled" size="xs" pill
          >Inherited</lr-button
        >
        <lr-button
          id="direct"
          ...
          size="xs"
          pill
          style="--lr-button-width: 120px; --lr-button-min-height: 63px; --lr-button-radius: 11px; --lr-button-fill: rgb(7, 8, 9);"
          >Direct</lr-button
        >
      </div>
    `);
    const inherited = wrapper.querySelector("#inherited") as LyraButton;
    const direct = wrapper.querySelector("#direct") as LyraButton;
    await Promise.all([inherited.updateComplete, direct.updateComplete]);

    const inheritedBase = inherited.shadowRoot!.querySelector(
      '[part~="base"]'
    ) as HTMLElement;
    const directBase = direct.shadowRoot!.querySelector(
      '[part~="base"]'
    ) as HTMLElement;
    const inheritedStyle = getComputedStyle(inheritedBase);
    expect(inheritedStyle.inlineSize).to.equal("180px");
    expect(inheritedStyle.minBlockSize).to.equal("61px");
    expect(inheritedStyle.borderRadius).to.equal("9px");
    expect(inheritedStyle.backgroundColor).to.equal("rgb(1, 2, 3)");

    const directStyle = getComputedStyle(directBase);
    expect(directStyle.inlineSize).to.equal("120px");
    expect(directStyle.minBlockSize).to.equal("63px");
    expect(directStyle.borderRadius).to.equal("11px");
    expect(directStyle.backgroundColor).to.equal("rgb(7, 8, 9)");
  });
  ...
  it('colors appearance="link" from the same accent token appearance="plain" uses', async () => {
    ...
    const linkBase = linkEl.shadowRoot!.querySelector(
      '[part~="base"]'
    ) as HTMLElement;
    const plainBase = plainEl.shadowRoot!.querySelector(
      '[part~="base"]'
    ) as HTMLElement;
    expect(getComputedStyle(linkBase).color).to.equal(
      getComputedStyle(plainBase).color
    );
  });
  ...
  it('inherits the ambient font-size for appearance="link" instead of forcing a per-size font-size', async () => {
    ...
    const base = button.shadowRoot!.querySelector(
      '[part~="base"]'
    ) as HTMLElement;
  ...
  it('supports size="2xs": tighter than xs, with the ladder\'s tightest floor', async () => {
    const el = (await fixture(
      html`<lr-button size="2xs">Go</lr-button>`
    )) as LyraButton;
    const xsEl = (await fixture(
      html`<lr-button size="xs">Go</lr-button>`
    )) as LyraButton;
    const cs = getComputedStyle(
      el.shadowRoot!.querySelector('[part~="base"]') as HTMLElement
    );
    const xsCs = getComputedStyle(
      xsEl.shadowRoot!.querySelector('[part~="base"]') as HTMLElement
    );
    expect(cs.minBlockSize).to.equal("20px");
    expect(parseFloat(cs.fontSize)).to.be.lessThan(parseFloat(xsCs.fontSize));
    expect(parseFloat(cs.paddingInlineStart)).to.be.lessThan(
      parseFloat(xsCs.paddingInlineStart)
    );
  });
  ...
  describe("sizing custom properties", () => {
    ...
    it("renders the ladder's padding/font-size/min-height at all six tiers when the properties are untouched", async () => {
      for (const tier of tiers) {
        ...
        expect(cs.minHeight, `size=${tier.size} min-block-size`).to.equal(
          tier.minHeight
        );
      }
    });
    ...
    it('lets a consumer pin a size="s" button to a compact toolbar tier with no ::part(base) rule', async () => {
      const el = (await fixture(
        html`<lr-button size="s">Go</lr-button>`
      )) as LyraButton;
    ...
    it('keeps appearance="link" winning over --lr-button-radius (zero radius)', async () => {
      const el = (await fixture(
        html`<lr-button appearance="link">Retry</lr-button>`
      )) as LyraButton;
    ...
    it("keeps the gap constant across tiers while the radius follows the shared ladder", async () => {
      const mEl = (await fixture(
        html`<lr-button>Go</lr-button>`
      )) as LyraButton;
      const xsEl = (await fixture(
        html`<lr-button size="xs">Go</lr-button>`
      )) as LyraButton;
  ...
  describe("start/end adornment wrappers collapse when unslotted", () => {
    it("collapses both adornment wrappers (display:none) when nothing is slotted into start/end", async () => {
      const el = (await fixture(
        html`<lr-button>Label</lr-button>`
      )) as LyraButton;
      const startWrapper = el.shadowRoot!.querySelector(
        '[part~="start"]'
      ) as HTMLElement;
      const endWrapper = el.shadowRoot!.querySelector(
        '[part~="end"]'
      ) as HTMLElement;
  ...
  describe('appearance="outlined" fill', () => {
    it("stays transparent when --lr-button-outlined-fill is unset", async () => {
      const el = (await fixture(
        html`<lr-button appearance="outlined">Save</lr-button>`
      )) as LyraButton;
      const base = el.shadowRoot!.querySelector(
        '[part~="base"]'
      ) as HTMLElement;
  ...
  describe("anchor mode (href/target/download)", () => {
    it("renders a real <a> when href is set", async () => {
      ...
      expect(el.shadowRoot!.querySelector('button[part~="base"]') == null).to.be
        .true;
    });

    it("still renders the label/start/end/spinner content inside the anchor", async () => {
      const el = (await fixture(
        html`<lr-button href="https://example.com"
          ><span slot="start">*</span>Go</lr-button
        >`
      )) as LyraButton;
      const anchor = el.shadowRoot!.querySelector(
        'a[part~="base"]'
    ...
    it('derives rel="noopener noreferrer" when target is set on a link button', async () => {
      const el = (await fixture(
        html`<lr-button href="https://example.com" target="_blank"
          >Go</lr-button
        >`
      )) as LyraButton;
    ...
    it("forwards download to the anchor", async () => {
      const el = (await fixture(
        html`<lr-button href="https://example.com/file.zip" download="file.zip"
          >Go</lr-button
        >`
      )) as LyraButton;
      const anchor = el.shadowRoot!.querySelector("a") as HTMLAnchorElement;
    ...
    it("preserves an empty download attribute and applies the downloadable URL policy", async () => {
      const downloadable = (await fixture(html`
        <lr-button href="https://example.com/file.zip" download
          >Download</lr-button
        >
      `)) as LyraButton;
      const anchor = downloadable.shadowRoot!.querySelector(
        "a"
      ) as HTMLAnchorElement;
      ...
      expect(
        mail.shadowRoot!.querySelector("a")!.getAttribute("href")
      ).to.equal("mailto:hello@example.com");
      expect(mail.shadowRoot!.querySelector("a")!.hasAttribute("download")).to
        .be.false;
    });
    ...
    it("rejects a mailto: href when download is set, falling back to the native button", async () => {
      const el = (await fixture(
        html`<lr-button href="mailto:hello@example.com" download="contact"
          >Email</lr-button
        >`
      )) as LyraButton;
      ...
      expect(
        el.shadowRoot!.querySelector('[part~="base"]')!.localName
      ).to.equal("button");
    });
    ...
    it("ignores an unsafe href scheme, falling back to the native button", async () => {
      ...
      expect(el.shadowRoot!.querySelector('a[part~="base"]') == null).to.be
        .true;
    ...
    it("forwards a host aria-label onto the internal anchor as a literal string", async () => {
      const el = (await fixture(
        html`<lr-button href="https://example.com" aria-label="Open the site"
          >Go</lr-button
        >`
      )) as LyraButton;
      const anchor = el.shadowRoot!.querySelector(
    ...
    it("forwards host click()/focus()/blur() to the internal anchor", async () => {
      ...
      expect(
        el.shadowRoot!.activeElement === anchor,
        "focus() should focus the anchor"
      ).to.be.true;
      el.blur();
      expect(
        el.shadowRoot!.activeElement === anchor,
        "blur() should unfocus the anchor"
      ).to.be.false;
    });
    ...
    describe("a disabled link button omits href and cannot navigate", () => {
      ...
      it("dims the disabled anchor and shows not-allowed, exactly like the native button path", async () => {
        ...
        expect(
          Number(anchorStyle.opacity) < 1,
          "disabled link must be dimmed"
        ).to.equal(true);
        expect(anchorStyle.cursor).to.equal("not-allowed");

        const native = (await fixture(
          html`<lr-button disabled>Go</lr-button>`
        )) as LyraButton;
        const button = native.shadowRoot!.querySelector(
          'button[part~="base"]'
        ) as HTMLButtonElement;
      ...
      it("dims a link disabled purely by an ancestor fieldset", async () => {
        ...
        expect(
          ...
          "fieldset-disabled link must be dimmed"
        ).to.equal(true);
    ...
    describe("unset-regression: href unset renders the native button unchanged", () => {
      it("still renders a native <button> and honors variant/appearance/size when href is unset", async () => {
        const el = (await fixture(
          html`<lr-button variant="brand" appearance="outlined" size="l"
            >Go</lr-button
          >`
        )) as LyraButton;
        expect(el.shadowRoot!.querySelector('button[part~="base"]')).to.exist;
...
describe("lr-button: pill", () => {
  it("rounds the base to the pill radius token", async () => {
    const el = (await fixture(
      html`<lr-button pill>Save</lr-button>`
    )) as LyraButton;
  ...
  it("is accessible as a pill button", async () => {
    const el = await fixture(
      html`<lr-button pill variant="brand">Save</lr-button>`
    );
    await expect(el).to.be.accessible();
  });
});
...
describe("lr-button: with-caret", () => {
  it("renders no caret part by default (regression)", async () => {
    ...
    expect(el.shadowRoot!.querySelectorAll('[part="caret"]').length).to.equal(
      0
    );
  });
  ...
  it("renders a decorative caret glyph when with-caret is set", async () => {
    const el = (await fixture(
      html`<lr-button with-caret>Menu</lr-button>`
    )) as LyraButton;
    ...
    expect(
      el.shadowRoot!.querySelectorAll('[part="caret"] svg').length
    ).to.equal(1);
  });
  ...
  it("renders the caret in anchor mode too", async () => {
    ...
    expect(
      el.shadowRoot!.querySelectorAll('a[part~="base"] [part="caret"]').length
    ).to.equal(1);
  });
  ...
  it("is accessible as a caret-bearing dropdown trigger", async () => {
    const el = await fixture(
      html`<lr-button with-caret aria-haspopup="menu" aria-expanded="false"
        >Actions</lr-button
      >`
    );
    await expect(el).to.be.accessible();
...
describe('lr-button: appearance="filled-outlined"', () => {
  it("combines the filled fill with the outlined border color", async () => {
    ...
    const bothEl = (await fixture(
      html`<lr-button appearance="filled-outlined" variant="brand"
        >Save</lr-button
      >`
    )) as LyraButton;
    ...
    const filled = getComputedStyle(
      filledEl.shadowRoot!.querySelector('[part~="base"]')!
    );
    const outlined = getComputedStyle(
      outlinedEl.shadowRoot!.querySelector('[part~="base"]')!
    );
    const both = getComputedStyle(
      bothEl.shadowRoot!.querySelector('[part~="base"]')!
    );
...
describe("lr-button: named submitter and form-submission overrides", () => {
  ...
  it("leaves the submitted FormData and event.submitter untouched when nothing is named (regression)", async () => {
    ...
    `)) as HTMLFormElement;
    const el = form.querySelector("lr-button") as LyraButton;
    const seen: { submitterIsNull: boolean; keys: string[] } = {
      submitterIsNull: false,
      keys: [],
    };
    form.addEventListener("submit", (event) => {
      event.preventDefault();
  ...
  it("skips constraint validation when formnovalidate is set", async () => {
    const form = (await fixture(html`
      <form>
        <input name="q" required />
        <lr-button type="submit" name="action" value="save" formnovalidate
          >Save</lr-button
        >
      </form>
    `)) as HTMLFormElement;
  ...
  it("distinguishes absent submitter overrides from explicit empty property values", async () => {
    ...
    expect(
      (submitters[1] as HTMLButtonElement).getAttribute("formtarget")
    ).to.equal("");
  });
  ...
  it("reflects name synchronously on assignment, with no await", async () => {
    const el = (await fixture(
      html`<lr-button type="submit">Save</lr-button>`
    )) as LyraButton;
    el.name = "action";
...
describe("lr-button — the shared styling vocabulary", () => {
  ...
  it("renders the Web Awesome size spellings at the same height as the canonical steps", async () => {
    for (const [alias, step] of [
      ...
      const aliasEl = (await fixture(
        html`<lr-button size=${alias}>Go</lr-button>`
      )) as LyraButton;
      const stepEl = (await fixture(
        html`<lr-button size=${step}>Go</lr-button>`
      )) as LyraButton;
      expect(height(aliasEl), `size=${alias} height`).to.equal(height(stepEl));
      expect(
        getComputedStyle(base(aliasEl)).fontSize,
        `size=${alias} font-size`
      ).to.equal(getComputedStyle(base(stepEl)).fontSize);
      expect(
        getComputedStyle(base(aliasEl)).paddingLeft,
        `size=${alias} padding-inline`
      ).to.equal(getComputedStyle(base(stepEl)).paddingLeft);
    }
  });
  ...
  it('paints appearance="accent" differently from appearance="filled" for every variant', async () => {
    for (const variant of [
      ...
      ).to.not.equal(filled.backgroundColor);
      expect(accent.color, `variant=${variant} foreground`).to.not.equal(
        filled.color
      );
      // ...and neither tier may fall back to "no fill at all" on the page surface.
      expect(
        filled.backgroundColor,
        `variant=${variant} filled must not be transparent`
  ...
  it("keeps accent and filled apart in dark mode too", async () => {
    for (const variant of ["neutral", "brand", "danger"] as const) {
      ...
      `)) as HTMLElement;
      const [filledEl, accentEl] = Array.from(
        wrapper.querySelectorAll("lr-button")
      ) as LyraButton[];
      const filled = getComputedStyle(base(filledEl!));
      const accent = getComputedStyle(base(accentEl!));
      expect(
        accent.backgroundColor,
        `dark variant=${variant} background`
      ).to.not.equal(filled.backgroundColor);
      expect(
        filled.backgroundColor,
        `dark variant=${variant} filled must not be transparent`
      ).to.not.equal("rgba(0, 0, 0, 0)");
    }
  });
  ...
  it("is accessible in its new default appearance, and as a pill textarea-adjacent control", async () => {
    await expect(
      await fixture(html`<lr-button>Save</lr-button>`)
    ).to.be.accessible();
    await expect(
      await fixture(
        html`<lr-button pill variant="danger" appearance="filled"
          >Delete</lr-button
        >`
      )
    ).to.be.accessible();
  ...
  for (const appearance of ["quiet", "plain"] as const) {
    it(`paints a hovered appearance="${appearance}" button something other than the page surface`, async () => {
      ...
      try {
        ...
        expect(hovered, `${appearance} hover vs page surface`).to.not.equal(
          surface
        );
...
describe("lr-button — mapped Shoelace and Web Awesome surface", () => {
  it("keeps Lyra defaults while accepting Shoelace variant spellings", async () => {
    const defaultEl = (await fixture(
      html`<lr-button variant="default">Default</lr-button>`
    )) as LyraButton;
    const primaryEl = (await fixture(
      html`<lr-button variant="primary">Primary</lr-button>`
    )) as LyraButton;
    const textEl = (await fixture(
      html`<lr-button variant="text">Text</lr-button>`
    )) as LyraButton;
  ...
  it("maps caret, outline, and circle without replacing their Lyra counterparts", async () => {
    const caret = (await fixture(
      html`<lr-button caret>Menu</lr-button>`
    )) as LyraButton;
    expect(caret.caret).to.be.true;
    expect(caret.withCaret).to.be.true;
    expect(
      caret.shadowRoot!.querySelectorAll('[part="caret"]').length
    ).to.equal(1);

    const outlined = (await fixture(
      html`<lr-button outline>Outlined</lr-button>`
    )) as LyraButton;
    const outlinedBase = outlined.shadowRoot!.querySelector(
      '[part~="base"]'
    ) as HTMLElement;
    expect(outlined.outline).to.be.true;
    expect(getComputedStyle(outlinedBase).backgroundColor).to.equal(
      "rgba(0, 0, 0, 0)"
    );

    const circle = (await fixture(
      html`<lr-button circle aria-label="Settings"
        ><svg aria-hidden="true"></svg
      ></lr-button>`
    )) as LyraButton;
    const circleBase = circle.shadowRoot!.querySelector(
      '[part~="base"]'
    ) as HTMLElement;
    ...
    expect(
      Number.parseFloat(getComputedStyle(circleBase).borderRadius)
    ).to.be.at.least(box.height / 2);
  });
  ...
  it("honors with-start/with-end as SSR presence hints without requiring assigned content", async () => {
    const el = (await fixture(
      html`<lr-button with-start with-end>Save</lr-button>`
    )) as LyraButton;
    ...
    expect(
      (el.shadowRoot!.querySelector('[part~="end"]') as HTMLElement).hidden
    ).to.be.false;
  });
  ...
  it("exposes synchronous required/custom validity and state restoration without changing submitter semantics", async () => {
    const el = (await fixture(
      html`<lr-button required>Submit</lr-button>`
    )) as LyraButton;
  ...
  it("publishes disabled/loading/link/icon-button custom states as live rendered facts", async () => {
    const el = (await fixture(html`
      <lr-button href="https://example.com" aria-label="Settings"
        ><svg aria-hidden="true"></svg
      ></lr-button>
    `)) as LyraButton;
    const states = (el as unknown as { internals: ElementInternals }).internals
      .states;
  ...
  it("strips opener and force-adds the guard whenever target is set, even against an unsafe author rel", async () => {
    const el = (await fixture(html`
      <lr-button href="https://example.com" target="_blank" rel="opener"
        >Open</lr-button
      >
    `)) as LyraButton;
    const anchor = el.shadowRoot!.querySelector(
  ...
  it("preserves author rel tokens a migrating wa-/sl- consumer actually uses", async () => {
    ...
    const el = (await fixture(html`
      <lr-button href="https://example.com" rel="nofollow me"
        >Profile</lr-button
      >
    `)) as LyraButton;
    const anchor = el.shadowRoot!.querySelector(
  ...
  it("merges author tokens with the guard rather than replacing either", async () => {
    const el = (await fixture(html`
      <lr-button href="https://example.com" target="_blank" rel="nofollow"
        >Go</lr-button
      >
    `)) as LyraButton;
    const anchor = el.shadowRoot!.querySelector(
...
it("exposes the native validation surface of its form association", async () => {
  const form = (await fixture(html`
    <form>
      ...
      <lr-button id="send" name="action" value="send" type="submit"
        >Send</lr-button
      >
    </form>
  `)) as HTMLFormElement;
...
it("contains unbroken labels and end adornments in a 320px LTR or RTL allocation", async () => {
  ...
  for (const direction of ["ltr", "rtl"] as const) {
    const wrapper = await fixture<HTMLElement>(html`
      ...
      >
        <lr-button> ${unbroken}<span slot="end">${adornment}</span> </lr-button>
      </div>
    `);
    const el = wrapper.querySelector("lr-button") as LyraButton;
    ...
    expect(base.scrollWidth, `${direction} base scroll width`).to.be.at.most(
      base.clientWidth
    );
  }
});
