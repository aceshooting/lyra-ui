...
export const RetintedRunStates: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'The shared flow-status palette retints node cards, minimap rectangles, and summary counts together without changing the library-wide semantic tokens.',
      },
    },
  },
  render: () => html`
...
export const PlainInToolbar: Story = {
  ...
  render: () => html`
    ...
    >
      ...
      <lr-flow-run-status frame="plain" .decorations=${decorations}></lr-flow-run-status>
    </div>
  `,
};
