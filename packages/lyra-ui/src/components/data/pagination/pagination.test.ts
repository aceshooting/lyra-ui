...
it("moves focus to the applied current-page control after navigation is accepted", async () => {
  ...
  const nextPage = await pagination(
    html` <lr-pagination total="200" page-size="10" page="10"></lr-pagination> `
  );
  ...
  const nextCurrent = nextPage.shadowRoot!.querySelector(
    '[part~="page-current"]'
  ) as HTMLElement;
  ...
  const trailingGap =
    skippedRange.shadowRoot!.querySelectorAll<HTMLButtonElement>(
      '[part~="ellipsis"]'
    )[1]!;
  trailingGap.click();
  await skippedRange.updateComplete;
  const gapCurrent = skippedRange.shadowRoot!.querySelector(
    '[part~="page-current"]'
  ) as HTMLElement;
  ...
  (
    compact.shadowRoot!.querySelector(
      '[part~="next-button"]'
    ) as HTMLButtonElement
  ).click();
  await compact.updateComplete;
  expect(
    compact.shadowRoot!.activeElement ===
      compact.shadowRoot!.querySelector('[part="page-input"]')
  ).to.equal(true);
});
...
it("ref-counts the shared sink away once the last pagination disconnects", async () => {
  ...
  expect(
    ...
    "a connected pagination holds the sink"
  ).to.be.true;
  first.remove();
  expect(
    sinkElement("polite") !== null,
    "a still-connected pagination keeps it mounted"
  ).to.be.true;
  second.remove();
...
it("relays exactly one native FocusEvent from button, link, and compact controls", async () => {
  ...
  for (const testCase of cases) {
    ...
    for (const type of ["focus", "blur"] as const) {
      ...
      expect(hostEvents.length, `${testCase.name} ${type} host count`).to.equal(
        1
      );
      expect(
        delegatedEvents.length,
        `${testCase.name} ${type} delegated count`
      ).to.equal(1);
      for (const seen of [hostEvents[0]!, delegatedEvents[0]!]) {
        expect(
          seen.event.constructor,
          `${testCase.name} ${type} constructor`
        ).to.equal(FocusEvent);
        expect(
          seen.event instanceof CustomEvent,
          `${testCase.name} ${type} is not custom`
        ).to.equal(false);
        expect(
          seen.event.target === el,
          `${testCase.name} ${type} target`
        ).to.equal(true);
        expect(
          seen.origin === el,
          `${testCase.name} ${type} shadow original stopped`
        ).to.equal(true);
        expect(seen.event.bubbles, `${testCase.name} ${type} bubbles`).to.equal(
          true
        );
        expect(
          seen.event.composed,
          `${testCase.name} ${type} composed`
        ).to.equal(true);
        expect(
        ...
        expect(
          (seen.event as FocusEvent).view === window,
          `${testCase.name} ${type} view`
        ).to.equal(true);
        expect(
          (seen.event as FocusEvent).detail,
          `${testCase.name} ${type} detail`
        ).to.equal(7);
      }
    }
  }
...
it("disables every control for empty data, disabled, and loading states", async () => {
  const el = await pagination(
    html`<lr-pagination with-summary></lr-pagination>`
  );
  const controls = () => [
...
it("distinguishes omitted pagination labels from explicit English and empty overrides", async () => {
  ...
  const labels = (el: LyraPagination): string[] => [
    el
      .shadowRoot!.querySelector('[part="page-input"]')!
    ...
    el
      ...
      .getAttribute("aria-label") ?? "missing",
    el
      .shadowRoot!.querySelector('[part~="first-button"]')!
      .getAttribute("aria-label") ?? "missing",
    el
      .shadowRoot!.querySelector('[part~="last-button"]')!
      .getAttribute("aria-label") ?? "missing",
  ];
  ...
  const explicitEnglish = await pagination(html`
    ...
    ></lr-pagination>
  `);
  expect(labels(explicitEnglish)).to.deep.equal([
    "Page",
    "Previous",
    "Next",
    "First page",
...
it("localizes the empty summary as one interpolated message", async () => {
  ...
  expect(
    el.shadowRoot!.querySelector('[part="summary"]')!.textContent!.trim()
  ).to.equal("Aucun contenu (0 résultats)");
});
...
it("omits the built-in summary by default, without removing the controls", async () => {
  ...
  // migrated pager.
  const el = await pagination(
    html` <lr-pagination total="30"></lr-pagination> `
  );
...
it("renders the built-in summary when with-summary is set", async () => {
  const el = await pagination(
    html` <lr-pagination total="30" with-summary></lr-pagination> `
  );
...
it("mirrors the directional icons under RTL", async () => {
  ...
  const rtl = await pagination(
    html` <lr-pagination dir="rtl" total="95" page-size="10"></lr-pagination> `
  );
  const ltrPrevious = ltr.shadowRoot!.querySelector(
    '[part="previous-icon"]'
  ) as HTMLElement;
  const rtlPrevious = rtl.shadowRoot!.querySelector(
    '[part="previous-icon"]'
  ) as HTMLElement;
...
it("stacks its summary and controls in a narrow allocation", async () => {
  ...
  expect(
    getComputedStyle(el.shadowRoot!.querySelector('[part~="base"]')!)
      .flexDirection
  ).to.equal("column");
});
...
describe("control padding knob (--lr-pagination-control-padding)", () => {
  ...
  it("defaults the control padding to var(--lr-space-xs) (4px) identically at every tier", async () => {
    ...
    for (const size of ["xs", "s", "m", "l", "xl"] as const) {
      ...
      expect(
        getComputedStyle(nextButton(el)).paddingTop,
        `${size} button`
      ).to.equal("4px");
      const input = pageInput(el);
      if (input)
        expect(getComputedStyle(input).paddingTop, `${size} input`).to.equal(
          "4px"
        );
    }
  });
...
describe("layout gap hooks", () => {
  async function themedPagination(
...
describe("state color hooks", () => {
  it("inherits resting and current-page color hooks from an ancestor", async () => {
    ...
    expect(getComputedStyle(current).borderTopColor).to.equal(
      "rgb(13, 14, 15)"
    );
      ...
      try {
        ...
        await waitUntil(() => {
          ...
          return (
            ...
            style.borderTopColor === hoverBorder
          );
        ...
        await waitUntil(() => {
          ...
          return (
            ...
            style.borderTopColor === activeBorder
          );
...
describe("numbered page list", () => {
  ...
  it("renders every page as its own control when they all fit", async () => {
    ...
    expect(
      el.shadowRoot!.querySelectorAll('[part~="ellipsis"]').length
    ).to.equal(0);
    expect(
      el.shadowRoot!.querySelectorAll('[part="page-input"]').length
    ).to.equal(0);
  });
  ...
  it("elides long runs and keeps the item count constant as the page moves", async () => {
    ...
    expect(
      itemCount(el),
      "the control must not resize as the page changes"
    ).to.equal(middleCount);
    el.page = 20;
    await el.updateComplete;
  ...
  it("widens the window with sibling-count and pins more pages with boundary-count", async () => {
    ...
    expect(pageLabels(el)).to.deep.equal([
      "1",
      "2",
      "8",
      "9",
  ...
  it("renders no first/last controls until with-edges is set", async () => {
    const el = await pagination(
      html` <lr-pagination total="50" page-size="10" page="3"></lr-pagination> `
    );
    expect(el.withEdges).to.equal(false);
    expect(
      el.shadowRoot!.querySelectorAll('[part~="first-button"]').length
    ).to.equal(0);
    expect(
      el.shadowRoot!.querySelectorAll('[part~="last-button"]').length
    ).to.equal(0);
  });
  ...
  it("jumps to the first and last page through the edge controls", async () => {
    ...
    const last = el.shadowRoot!.querySelector(
      '[part~="last-button"]'
    ) as HTMLButtonElement;
    ...
    expect(
      first.disabled,
      "the first-page control is spent on page 1"
    ).to.equal(true);
  ...
  it("renders pages as links from an href-template string, current page excepted", async () => {
    ...
    expect(nodes.map((node) => node.localName)).to.deep.equal([
      "a",
      "a",
      "a",
      "a",
  ...
  it("removes every link-mode tab stop while disabled or loading", async () => {
    ...
    for (const testCase of inertCases) {
      ...
      expect(anchors.length, `${testCase.name} link branch`).to.be.greaterThan(
        0
      );
      expect(
      ...
      expect(
        el
          .shadowRoot!.querySelector('[part~="page-current"]')!
  ...
  it("calls an href-template function only for active valid page targets", async () => {
    const seen: number[] = [];
    const el = await pagination(
      html` <lr-pagination total="50" page="1" with-edges></lr-pagination> `
    );
    ...
    expect(
      seen.every(
        (page) => page >= 1 && page <= el.totalPages && page !== el.page
      ),
      "first-page render"
    ).to.equal(true);
    ...
    for (const testCase of [
      ...
      expect(
        anchors.length,
        `${testCase.name} configured link branch`
      ).to.be.greaterThan(0);
      expect(
  ...
  it("disables every page control while disabled or loading", async () => {
    ...
    expect(pageNodes(el).some((node) => node.hasAttribute("href"))).to.equal(
      false
    );
  });
  ...
  it("mirrors the edge-control glyphs under RTL", async () => {
    ...
    expect(
      getComputedStyle(
        ltr.shadowRoot!.querySelector('[part="first-icon"]') as HTMLElement
      ).transform
    ).to.not.equal(
      getComputedStyle(
        rtl.shadowRoot!.querySelector('[part="first-icon"]') as HTMLElement
      ).transform
    );
  });
  ...
  it("renders the compact layout as previous/next around the page-jump field", async () => {
    ...
    expect(el.shadowRoot!.querySelectorAll('[part="pages"]').length).to.equal(
      0
    );
    expect(el.shadowRoot!.querySelectorAll('[part~="page"]').length).to.equal(
      0
    );
    expect(
  ...
  it("keeps compact previous/next and edge controls in link mode, including inactive boundaries", async () => {
    const firstPage = await pagination(html`
      ...
      ></lr-pagination>
    `);
    const first = firstPage.shadowRoot!.querySelector(
      '[part~="first-button"]'
    ) as HTMLAnchorElement;
    const previous = firstPage.shadowRoot!.querySelector(
      '[part~="previous-button"]'
    ) as HTMLAnchorElement;
    const next = firstPage.shadowRoot!.querySelector(
      '[part~="next-button"]'
    ) as HTMLAnchorElement;
    const last = firstPage.shadowRoot!.querySelector(
      '[part~="last-button"]'
    ) as HTMLAnchorElement;
    ...
    expect(firstPage.shadowRoot!.querySelector('[part="page-input"]') !== null)
      .to.be.true;
    expect(firstPage.shadowRoot!.querySelector('[part="pages"]') === null).to.be
      .true;
  ...
  it("normalizes foreign format values to the standard behavior and reflected property value", async () => {
    const el = await pagination(
      html`<lr-pagination format="unknown" total="50"></lr-pagination>`
    );

    expect(el.format).to.equal("standard");
    expect(el.shadowRoot!.querySelector('[part="pages"]') !== null).to.be.true;
    expect(el.shadowRoot!.querySelector('[part="page-input"]') === null).to.be
      .true;
  ...
  it("is accessible as a populated page list, as links, and in the compact layout", async () => {
    ...
    `);
    expect(
      standard.shadowRoot!.querySelectorAll('[part~="ellipsis"]').length
    ).to.be.greaterThan(0);
    ...
    it("keeps the current page a solid brand chip in every appearance, accent included", async () => {
      for (const appearance of [
        ...
        expect(
          ...
          appearance
        ).to.not.equal(getComputedStyle(other).backgroundColor);
      }
    });
...
it("wraps a long page list into a narrow allocation instead of overflowing it", async () => {
  ...
  expect(
    el.shadowRoot!.querySelectorAll('[part~="page"]').length
  ).to.be.greaterThan(4);
...
describe("the shared size ladder", () => {
  ...
  it("keeps interactive ellipses and the compact page field at the shared hit-area floor", async () => {
    const paged = await pagination(html`
      ...
      ></lr-pagination>
    `);
    for (const ellipsis of paged.shadowRoot!.querySelectorAll<HTMLElement>(
      '[part~="ellipsis"]'
    )) {
    ...
    const compact = await pagination(html`
      ...
      ></lr-pagination>
    `);
    const field = compact.shadowRoot!.querySelector<HTMLElement>(
      '[part="page-input"]'
    )!;
...
describe("Web Awesome navigation surface", () => {
  ...
  it("without-nav removes only previous/next and hide-single-page renders nothing", async () => {
    const withoutNav = await pagination(
      html` <lr-pagination total="50" without-nav with-edges></lr-pagination> `
    );
    expect(
      withoutNav.shadowRoot!.querySelector('[part~="previous-button"]') == null
    ).to.be.true;
    expect(
      withoutNav.shadowRoot!.querySelector('[part~="next-button"]') == null
    ).to.be.true;
    expect(withoutNav.shadowRoot!.querySelector('[part~="first-button"]')).to
      .exist;
    expect(withoutNav.shadowRoot!.querySelector('[part~="page"]')).to.exist;

    const single = await pagination(
      html` <lr-pagination total="5" hide-single-page></lr-pagination> `
    );
  ...
  it("projects all four custom icon slots without replacing accessible button names", async () => {
    ...
    for (const name of ["first", "previous", "next", "last"]) {
      ...
      expect(
        el
          .shadowRoot!.querySelector(`[part~="${name}-button"]`)!
          .getAttribute("aria-label")
      )
        .to.be.a("string")
        .and.not.equal("");
  ...
  it("puts the shared button part on pages, ellipses, and navigation and label on compact output", async () => {
    const standard = await pagination(
      html` <lr-pagination total="200" page="10" with-edges></lr-pagination> `
    );
    const controls = [
      ...standard.shadowRoot!.querySelectorAll<HTMLElement>('[part~="button"]'),
    ];
    expect(controls.length).to.be.greaterThan(8);
    expect(controls.some((control) => control.part.contains("ellipsis"))).to.be
      .true;
    expect(controls.every((control) => control.matches("button, a"))).to.be
      .true;

    const compact = await compactPagination();
    const label = compact.shadowRoot!.querySelector(
      '[part~="label"]'
    ) as HTMLElement;
