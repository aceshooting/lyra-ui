...
export interface LyraDataGridEventMap<T = unknown> { 'lr-row-click': CustomEvent<{ row: T }>; 'lr-cell-focus': CustomEvent<{ row: T; column: string }>; 'lr-sort': CustomEvent<{ key: string; direction: 'ascending' | 'descending' }>; 'lr-selection-change': CustomEvent<{ row: T | null }>; }
...
export class LyraDataGrid<T = Record<string, unknown>> extends LyraElement<LyraDataGridEventMap<T>> {
  ...
  private activate(row: T): void { this.selectedKey = this.rowKey(row, this.rows.indexOf(row)); this.emit('lr-row-click', { row }); this.emit('lr-selection-change', { row }); }
  private sort(column: DataGridColumn<T>): void { if (!column.sortable) return; if (this.sortKey === column.key) this.sortDirection = this.sortDirection === 'ascending' ? 'descending' : 'ascending'; else { this.sortKey = column.key; this.sortDirection = 'ascending'; } this.emit('lr-sort', { key: column.key, direction: this.sortDirection }); }
  ...
  render(): TemplateResult {
    ...
    return html`<div part="viewport"><table part="grid" role="grid" aria-label=${label} aria-rowcount=${this.rows.length + 1} aria-colcount=${this.columns.length}>
      ...
      <tbody>${this.loading ? html`<tr><td role="gridcell" colspan=${Math.max(1, this.columns.length)} part="empty">${this.localize('loading')}</td></tr>` : this.rows.length ? this.rows.map((row, rowIndex) => html`<tr part="row" role="row" data-selected=${this.rowKey(row, rowIndex) === this.selectedKey ? 'true' : 'false'} aria-selected=${this.rowKey(row, rowIndex) === this.selectedKey ? 'true' : 'false'} @click=${() => this.activate(row)}>${this.columns.map((column, columnIndex) => html`<td part="cell" role="gridcell" tabindex=${rowIndex === this.focusRow && columnIndex === this.focusColumn ? '0' : '-1'} data-row=${rowIndex} data-column=${columnIndex} aria-colindex=${columnIndex + 1} aria-rowindex=${rowIndex + 2} @keydown=${(event: KeyboardEvent) => this.onCellKeyDown(event, rowIndex, columnIndex)} @focus=${() => { this.focusRow = rowIndex; this.focusColumn = columnIndex; this.emit('lr-cell-focus', { row, column: column.key }); }}>${this.cell(row, column)}</td>`)}</tr>`) : html`<tr><td role="gridcell" colspan=${Math.max(1, this.columns.length)} part="empty">${this.emptyText || this.localize('noData')}</td></tr>`}</tbody>
    </table></div>`;
  }
}
declare global { interface HTMLElementTagNameMap { 'lr-data-grid': LyraDataGrid; } }
