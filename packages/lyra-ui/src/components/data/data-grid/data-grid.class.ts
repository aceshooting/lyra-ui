...
export class LyraDataGrid<Row = Record<string, unknown>> extends LyraElement<LyraDataGridEventMap<Row>> {
  ...
  @property({ attribute: 'child-rows' }) childRows: string | ((row: Row) => Row[] | undefined) | null = null;
  ...
  @property({ attribute: false }) dataSource: ((request: DataGridRequest) => Promise<DataGridResponse<Row>>) | null = null;
  ...
  @property({ type: Number, attribute: 'filter-debounce' }) filterDebounce = 250;
  ...
  @property({ type: Boolean, attribute: 'filter-from-leaf-rows' }) filterFromLeafRows = false;
  ...
  @property({ attribute: false }) rowClass: ((row: Row) => string | null | undefined) | null = null;
  /** Callback rendering expandable detail content. */
  @property({ attribute: false }) rowDetail: ((row: Row) => string | TemplateResult | Node) | null = null;
  /** Dot path used as the stable row identity. */
  @property({ attribute: 'row-key' }) rowKey: string | null = null;
  /** Optional global-search matcher. */
  @property({ attribute: false }) searchFn: ((value: unknown, term: string, row: Row) => boolean) | null = null;
  /** Controlled global search term. */
  @property({ attribute: false }) searchTerm = '';
  /** Row-selection behavior. A bare attribute means `multiple`. */
  @property({ reflect: true, converter: selectableConverter }) selectable: DataGridSelectable = 'none';
  ...
  @property({ type: Boolean, attribute: 'sort-desc-first' }) sortDescFirst = false;
  ...
  @property({ type: Boolean, attribute: 'with-column-menu', reflect: true }) withColumnMenu = false;
  /** Shows the all-columns visibility menu. */
  @property({ type: Boolean, attribute: 'with-columns-menu', reflect: true }) withColumnsMenu = false;
  /** Keeps a sorted column in ascending/descending states instead of removing its sort. */
  @property({ type: Boolean, attribute: 'without-sort-removal', reflect: true }) withoutSortRemoval = false;
  /** Shows the global search field. */
  @property({ type: Boolean, attribute: 'with-search', reflect: true }) withSearch = false;
  ...
  protected override willUpdate(changed: PropertyValues<this>): void {
    ...
    if (
      !this.usesServerData &&
      (changed.has('data') || changed.has('rowKey') || changed.has('childRows') ||
        changed.has('server') || changed.has('dataSource'))
    ) {
      const validKeys = this.allSourceRows.map((row, index) => this.keyForRow(row, index));
      ...
      if (
        ...
        selectedKeys.some((key, index) => !keysEqual(key, this.selectedKeys[index]!))
      ) {
    ...
    if (
      this.paginate &&
      (changed.has('data') || changed.has('filters') || changed.has('searchTerm') ||
        changed.has('groupBy') || changed.has('pageSize') || changed.has('total') ||
        changed.has('server') || changed.has('dataSource'))
    ) {
    ...
    if (
      changed.has('data') || changed.has('columns') || changed.has('filters') ||
      changed.has('searchTerm') || changed.has('sort') || changed.has('expandedKeys') ||
      changed.has('groupBy') || changed.has('page') || changed.has('pageSize') ||
      changed.has('paginate') || changed.has('childRows')
    ) {
  ...
  protected override updated(changed: PropertyValues<this>): void {
    if (changed.has('dataSource') && this.dataSource) this.scheduleServerRequest(false);
    const requestRelevant =
      changed.has('sort') || changed.has('filters') || changed.has('searchTerm') ||
      changed.has('page') || changed.has('pageSize') || changed.has('server');
  ...
  private get orderedColumns(): Array<{ column: DataGridColumn<Row>; id: string; naturalIndex: number }> {
    ...
    return entries.sort((left, right) => {
      ...
      if (leftRank === undefined && rightRank === undefined) return left.naturalIndex - right.naturalIndex;
  ...
  private get visibleColumns(): Array<{ column: DataGridColumn<Row>; id: string; naturalIndex: number }> {
    const entries = this.orderedColumns.filter(({ column, id }) =>
      this.columnVisibility.get(id) ?? column.hidden !== true,
    );
    const rank = (side: 'left' | 'right', id: string): number => {
      const ordered = this.orderedColumns.filter((entry) => this.getColumnPin(entry.id) === side);
  ...
  private get processedClientRows(): Row[] {
    ...
    if (this.childRows) {
      const included = this.data.filter((row) =>
        this.rowMatches(row) || (this.filterFromLeafRows && this.hasMatchingDescendant(row)),
      );
      return sortRows(included, this.columns, this.sort, this.effectiveLocale);
    }
    const filtered = filterRows(this.data, this.columns, this.filters, this.effectiveLocale);
    const searched = searchRows(filtered, this.columns, this.searchTerm, this.effectiveLocale, this.searchFn);
    return sortRows(searched, this.columns, this.sort, this.effectiveLocale);
  }

  private childrenFor(row: Row): Row[] {
    const children = typeof this.childRows === 'function'
      ? this.childRows(row)
      : this.childRows
  ...
  autoSizeColumn(columnId: string): void {
    ...
    const maximum = finiteRange(entry.column.maxWidth ?? Number.MAX_SAFE_INTEGER, Number.MAX_SAFE_INTEGER, minimum);
    this.setColumnWidth(columnId, finiteRange(width, minimum, minimum, maximum), false);
  }
  ...
  collapseRow(key: string | number): void {
    this.expandedKeys = this.expandedKeys.filter((candidate) => !keysEqual(candidate, key));
  }
  ...
  copySelectedRows(options: {
    ...
    const rowsToCopy = this.selectedRows.length > 0 ? this.selectedRows : this.getProcessedRows();
  ...
  expandAllRows(): void {
    ...
    this.allSourceRows.forEach((row, index) => {
      if (this.childrenFor(row).length > 0 || this.rowDetail) keys.push(this.keyForRow(row, index));
    });
    for (const group of this.groupItems(this.processedClientRows)) keys.push(group.key);
    this.expandedKeys = keys;
  }

  /** Expands one tree/detail/group key without emitting a user event. */
  expandRow(key: string | number): void {
    if (!arrayHasKey(this.expandedKeys, key)) this.expandedKeys = [...this.expandedKeys, key];
  }
  ...
  override focus(options: FocusOptions = {}): void {
    const selector = this.focusedRow < 0
    ...
    (this.renderRoot.querySelector(selector) as HTMLElement | null)?.focus(options);
  }
  ...
  getColumnFacets(columnId: string): DataGridFacets {
    ...
    const otherFilters = this.filters.filter((filter) => filter.id !== columnId);
  ...
  getColumnPin(columnId: string): DataGridPinSide {
    if (this.columnPinning.has(columnId)) return this.columnPinning.get(columnId) ?? false;
    return this.orderedColumns.find(({ id }) => id === columnId)?.column.pinned ?? false;
  }
  ...
  getVisibleRows(): Row[] {
    ...
    if (normalizedGroupBy(this.groupBy).length > 0) {
      return this.topLevelGroupBuckets(rows).slice(start, start + size).flatMap((bucket) => bucket.rows);
    }
  ...
  handleColumnsChange(): void {
    const valid = new Set(this.columns.map((column, index) => columnId(column, index)));
    const natural = [...valid];
    const order = this.columnOrder.filter((id) => valid.has(id));
    for (const id of natural) if (!order.includes(id)) order.push(id);
    if (this.columnOrder.length > 0 && order.join('\u0000') !== this.columnOrder.join('\u0000')) {
      this.columnOrder = order;
    }
    const widths = [...this.columnWidths].filter(([id]) => valid.has(id));
    const visibility = [...this.columnVisibility].filter(([id]) => valid.has(id));
    const pinning = [...this.columnPinning].filter(([id]) => valid.has(id));
    if (widths.length !== this.columnWidths.size) this.columnWidths = new Map(widths);
    if (visibility.length !== this.columnVisibility.size) this.columnVisibility = new Map(visibility);
    if (pinning.length !== this.columnPinning.size) this.columnPinning = new Map(pinning);
  }
  ...
  setState(state: DataGridState): void {
    const known = new Set(this.columns.map((column, index) => columnId(column, index)));
    if (state.order) this.columnOrder = state.order.filter((id) => known.has(id));
    if (state.widths) this.columnWidths = this.validColumnMap(state.widths, known, (width) =>
      finiteRange(width, 0, 0),
    );
    if (state.visibility) this.columnVisibility = this.validColumnMap(state.visibility, known, Boolean);
    if (state.pinning) this.columnPinning = this.validColumnMap(
      state.pinning,
      known,
      (side) => side === 'left' || side === 'right' ? side : false,
    );
    if (state.sort) this.sort = state.sort.filter((item) => known.has(item.id)).map((item) => ({ id: item.id, desc: Boolean(item.desc) }));
    if (state.filters) this.filters = state.filters.filter((item) => known.has(item.id)).map((item) => ({ ...item }));
    if (state.search !== undefined) this.searchTerm = state.search;
    if (state.selectedKeys) this.selectedKeys = state.selectedKeys.filter(isKey);
    if (state.expandedKeys) this.expandedKeys = state.expandedKeys.filter(isKey);
    if (state.page !== undefined) this.page = finiteCount(state.page);
    if (state.pageSize !== undefined) this.pageSize = finiteCount(state.pageSize);
  }
  ...
  sizeColumnsToFit(): void {
    ...
    const totalFlex = flexible.reduce((sum, { column }) => sum + finiteRange(column.flex ?? 1, 1, 0), 0) || flexible.length;
    const next = new Map(this.columnWidths);
    for (const { column, id } of flexible) {
      const share = available * (finiteRange(column.flex ?? 1, 1, 0) / totalFlex);
      const minimum = finiteRange(column.minWidth ?? 0, 0, 0);
      const maximum = finiteRange(column.maxWidth ?? Number.MAX_SAFE_INTEGER, Number.MAX_SAFE_INTEGER, minimum);
  ...
  private validColumnMap<Value, Input>(
    ...
    for (const [id, value] of Object.entries(record)) if (known.has(id)) result.set(id, convert(value));
    return result;
  }

  private delimitedRows(rows: readonly Row[], options: DataGridCsvOptions): string {
  ...
  private rowMatches(row: Row): boolean {
    return filterRows([row], this.columns, this.filters, this.effectiveLocale).length > 0 &&
      searchRows([row], this.columns, this.searchTerm, this.effectiveLocale, this.searchFn).length > 0;
  }
  ...
  private hasMatchingDescendant(row: Row): boolean {
    return this.childrenFor(row).some((child) => this.rowMatches(child) || this.hasMatchingDescendant(child));
  }
  ...
  private get currentPageSelectableRows(): Array<{ row: Row; key: DataGridKey }> {
    ...
    const visit = (pageRows: readonly Row[]): void => {
      for (const row of pageRows) {
        ...
        const key = this.keyForRow(row, sourceIndex < 0 ? result.length : sourceIndex);
  ...
  private topLevelGroupBuckets(rows: readonly Row[]): Array<{ value: unknown; rows: Row[] }> {
    const field = normalizedGroupBy(this.groupBy)[0];
    if (!field) return [];
    const entry = this.orderedColumns.find(({ id, column }) => id === field || column.field === field);
    if (!entry) return [];
    ...
    return [...buckets].map(([value, bucketRows]) => ({ value, rows: bucketRows }));
  }
  ...
  private groupItems(rows: readonly Row[]): DataDisplayGroup<Row>[] {
    ...
    const visit = (groupRows: readonly Row[], depth: number, parent: string): void => {
      ...
      for (const [value, bucket] of buckets) {
        ...
        result.push({ kind: 'group', key, value, columnId: entry.id, rows: bucket, depth });
  ...
  private get displayItems(): DisplayItem<Row>[] {
    ...
    if (groups.length > 0 && !this.usesServerData) return this.groupedDisplayItems(this.pageRows, groups, 0, 'root');
    ...
    const visit = (rows: readonly Row[], depth: number): void => {
      for (const row of rows) {
        ...
        if (arrayHasKey(this.expandedKeys, key)) visit(this.processedChildrenFor(row), depth + 1);
      }
    };
  ...
  private groupedDisplayItems(
    ...
    if (!field) return [];
    const entry = this.orderedColumns.find(({ id, column }) => id === field || column.field === field);
    if (!entry) return rows.map((row, index) => ({
      kind: 'row', row, key: this.keyForRow(row, index), depth, sourceIndex: index,
    }));
    const buckets = new Map<unknown, Row[]>();
    for (const row of rows) {
      const value = columnValue(entry.column, row);
    ...
    for (const [value, bucket] of buckets) {
      ...
      if (depth + 1 < fields.length) result.push(...this.groupedDisplayItems(bucket, fields, depth + 1, key));
      else {
        for (const row of bucket) {
          const sourceIndex = this.allSourceRows.indexOf(row);
          result.push({
            kind: 'row', row, key: this.keyForRow(row, Math.max(0, sourceIndex)),
            depth: depth + 1, sourceIndex: Math.max(0, sourceIndex),
          });
        }
      }
    }
  ...
  private get virtualWindow(): { items: DisplayItem<Row>[]; start: number; end: number } {
    ...
    const end = Math.min(items.length, start + visibleCount + VIRTUAL_OVERSCAN * 2);
  ...
  private get gridTemplate(): string {
    const columns = this.visibleColumns.map(({ column, id }) => {
      ...
      if (width !== undefined && Number.isFinite(width) && width > 0) return `${width}px`;
      const minimum = Number.isFinite(column.minWidth) && (column.minWidth ?? 0) > 0
        ? `${column.minWidth}px`
        : 'var(--lr-size-7rem)';
      const maximum = Number.isFinite(column.maxWidth) && (column.maxWidth ?? 0) > 0
        ? `${column.maxWidth}px`
    ...
    return columns.length > 0 ? columns.join(' ') : 'minmax(var(--lr-size-7rem), 1fr)';
  }

  private columnStyle(column: DataGridColumn<Row>, id: string): Record<string, string> {
  ...
  private pinOffset(id: string, side: 'left' | 'right'): number {
    ...
    if (index < 0) return 0;
    const subset = side === 'left' ? entries.slice(0, index) : entries.slice(index + 1);
    return subset
      .filter((entry) => this.getColumnPin(entry.id) === side)
      .reduce((sum, entry) => sum + this.estimatedColumnWidth(entry.column, entry.id), 0);
  }

  private columnLabel(column: DataGridColumn<Row>, id: string): string {
    return column.label ?? humanizeIdentifier(column.field ?? id, this.effectiveLocale);
  }
  ...
  private sortFor(id: string): { state: DataGridSortingState[number] | undefined; index: number } {
  ...
  private activateSort(id: string, additive: boolean): void {
    ...
    else if (existing.desc === descendingFirst) replacement = { id, desc: !descendingFirst };
    else if (this.withoutSortRemoval) replacement = { id, desc: descendingFirst };
    ...
    this.sort = next;
    this.page = 0;
    this.emit('lr-sort-change', { sort: this.sort.map((item) => ({ ...item })) });
  }
  ...
  private onFilterInput(event: Event, id: string): void {
    ...
    this.filters = next;
    this.page = 0;
    this.emit('lr-filter-change', { filters: next.map((filter) => ({ ...filter })) });
  }
  ...
  private emitCellContextMenu(
    ...
    const emitted = this.emit('lr-cell-contextmenu', detail, { cancelable: true });
  ...
  private setRowSelected(item: DataDisplayRow<Row>, selected: boolean, event?: MouseEvent): void {
    if (!this.rowIsSelectable(item.row)) return;
    const selectableItems = this.displayItems.filter(
      (candidate): candidate is DataDisplayRow<Row> => candidate.kind === 'row' && this.rowIsSelectable(candidate.row),
    );
    const position = selectableItems.findIndex((candidate) => keysEqual(candidate.key, item.key));
    let affected = [item];
    if (event?.shiftKey && this.selectionMode === 'multiple' && this.lastSelectedIndex >= 0) {
    ...
    const descendants = this.selectionMode === 'multiple'
  ...
  private selectCurrentPage(selected: boolean): void {
    ...
    let next = this.selectedKeys.filter((key) => !pageKeys.some((pageKey) => keysEqual(pageKey, key)));
  ...
  private rowSelectionState(item: DataDisplayRow<Row>): { checked: boolean; indeterminate: boolean } {
    ...
    const selected = descendants.filter((row, index) => {
      const sourceIndex = this.allSourceRows.indexOf(row);
      return arrayHasKey(this.selectedKeys, this.keyForRow(row, sourceIndex < 0 ? index : sourceIndex));
    }).length;
  ...
  private toggleRowExpanded(item: DataDisplayRow<Row>, emitUserEvent: boolean): void {
    const expanded = arrayHasKey(this.expandedKeys, item.key);
    if (expanded) this.collapseRow(item.key);
    else this.expandRow(item.key);
    if (emitUserEvent) {
      this.emit(expanded ? 'lr-row-collapse' : 'lr-row-expand', { key: item.key, row: item.row });
    }
  }
  ...
  private moveColumn(id: string, delta: number, emitUserEvent: boolean): void {
    ...
    this.columnOrder = order;
    if (emitUserEvent) this.emit('lr-column-move', { columnOrder: [...order], columnId: id, finished: true });
  }
  ...
  private onDrop(event: DragEvent, targetId: string): void {
    ...
    this.columnOrder = order;
    this.emit('lr-column-move', { columnOrder: [...order], columnId: sourceId, finished: true });
  }
  ...
  private onResizeStart(event: PointerEvent, id: string): void {
    ...
    this.resizeSession = {
      ...
      startWidth: header?.getBoundingClientRect().width ?? this.estimatedColumnWidth(entry.column, id),
  ...
  private onResizeMove(event: PointerEvent): void {
    ...
    this.setColumnWidth(session.columnId, session.startWidth + (event.clientX - session.startClientX) * direction, true, false);
  }
  ...
  private onResizeEnd(event: PointerEvent): void {
    ...
    this.emit('lr-column-resize', { columnId: session.columnId, width, finished: true });
  }
  ...
  private onResizeKey(event: KeyboardEvent, id: string): void {
    ...
    const entry = this.orderedColumns.find((item) => item.id === id);
    if (!entry) return;
    const logical = (event.key === 'ArrowRight' ? 1 : -1) * (this.effectiveDirection === 'rtl' ? -1 : 1);
    const current = this.columnWidths.get(id) ?? this.estimatedColumnWidth(entry.column, id);
  ...
  private onGridKeyDown(
    ...
    const headerEntry = columnIdValue
      ...
      : undefined;
    if (rowPosition < 0 && columnIdValue && headerEntry &&
      (this.reorderable || headerEntry.column.movable) && event.shiftKey && !event.altKey &&
    ...
    if (rowPosition < 0 && columnIdValue && headerEntry &&
      (this.resizable || headerEntry.column.resizable) && event.altKey) {
    ...
    if (event.key === 'ArrowLeft' || event.key === 'ArrowRight') {
      ...
      this.focusCell(event.ctrlKey ? this.displayItems.length - 1 : rowPosition, this.visibleColumns.length - 1);
      event.preventDefault();
    } else if (event.key === 'PageUp' || event.key === 'PageDown') {
      const delta = event.key === 'PageDown' ? this.safePageSize || 10 : -(this.safePageSize || 10);
      this.focusCell(rowPosition + delta, columnPosition);
      event.preventDefault();
    } else if (event.key === 'Enter') {
      if (rowPosition < 0 && columnIdValue) this.activateSort(columnIdValue, event.shiftKey);
      else if (item && column) this.emit('lr-cell-click', {
        column, value: columnValue(column, item.row), row: item.row, index: rowPosition,
      });
      event.preventDefault();
  ...
  private setGroupSelected(group: DataDisplayGroup<Row>, selected: boolean): void {
    ...
    return html`
      <div part="toolbar">
        ${this.withSearch ? html`
          ...
          />
        ` : nothing}
        ${this.withColumnsMenu ? html`
          <div part="columns-menu">
            <button
              type="button"
              aria-expanded=${this.columnsMenuOpen ? 'true' : 'false'}
              @click=${() => { this.columnsMenuOpen = !this.columnsMenuOpen; }}
            >${this.localize('showAllColumns')}</button>
            ${this.columnsMenuOpen ? html`
              <div role="group" aria-label=${this.localize('showAllColumns')}>
  ...
  private renderSelectAllHeader(): TemplateResult | typeof nothing {
    ...
    const selected = items.filter((item) => arrayHasKey(this.selectedKeys, item.key)).length;
    return html`
  ...
  private renderColumnMenu(column: DataGridColumn<Row>, id: string): TemplateResult | typeof nothing {
    ...
    return html`
      <div part="column-menu">
        <button
          ...
          @click=${() => { this.activeColumnMenu = open ? null : id; }}
        ><span aria-hidden="true">⋮</span></button>
        ${open ? html`
          <div role="menu" aria-label=${this.localize('menuLabel')}>
            ${pinAllowed ? html`
              <button type="button" role="menuitem" @click=${() => this.userPinColumn(id, 'left')}>${this.localize('previous')}</button>
              <button type="button" role="menuitem" @click=${() => this.userPinColumn(id, 'right')}>${this.localize('next')}</button>
              <button type="button" role="menuitem" @click=${() => this.userPinColumn(id, false)}>${this.localize('clear')}</button>
            ` : nothing}
            ${column.hideable === false ? nothing : html`
              <label role="menuitemcheckbox" aria-checked=${visible ? 'true' : 'false'}>
                <input
                  type="checkbox"
                  .checked=${visible}
                  @change=${(event: Event) => {
  ...
  private renderHeader(): TemplateResult {
    ...
    return html`
      <div part="header" role="row" style=${styleMap({ '--data-grid-columns': this.gridTemplate })}>
        ${this.renderSelectAllHeader()}
        ${this.visibleColumns.length === 0 && !this.selectionEnabled ? html`
          <div role="columnheader" aria-colindex="1">${this.localize('noColumns')}</div>
        ` : nothing}
        ${this.visibleColumns.map(({ column, id }, position) => {
          ...
          return html`
            <div
              ...
              .draggable=${canMove}
              @focus=${() => { this.focusedRow = -1; this.focusedColumn = position; }}
              @click=${(event: MouseEvent) => this.onHeaderClick(event, id)}
              @keydown=${(event: KeyboardEvent) => this.onGridKeyDown(event, -1, position, undefined, column, id)}
              @dragstart=${(event: DragEvent) => this.onDragStart(event, id)}
              @dragend=${() => { this.dragGhost = ''; }}
              @dragover=${(event: DragEvent) => event.preventDefault()}
              @drop=${(event: DragEvent) => this.onDrop(event, id)}
            >
              <span>${this.columnLabel(column, id)}</span>
              ${sorting.state ? html`
                <span part="sort-indicator" data-direction=${direction}>${chevronIcon()}</span>
                ${this.sort.length > 1 ? html`<span part="sort-number">${getNumberFormat(this.effectiveLocale).format(sorting.index + 1)}</span>` : nothing}
              ` : nothing}
              ${column.filterable ? html`
                <button
                  ...
                  aria-expanded=${this.activeFilterColumn === id ? 'true' : 'false'}
                  @click=${() => { this.activeFilterColumn = this.activeFilterColumn === id ? null : id; }}
                ><span aria-hidden="true">⌕</span></button>
              ` : nothing}
              ${this.renderColumnMenu(column, id)}
              ${this.getColumnPin(id) ? html`<span part="pin-indicator" aria-hidden="true"></span>` : nothing}
              ${canResize ? html`
                <span
                  part="resize-handle"
                  role="separator"
                  aria-orientation="vertical"
                  aria-label=${this.localize('resizeColumn', undefined, { label: this.columnLabel(column, id) })}
                  tabindex="-1"
                  @pointerdown=${(event: PointerEvent) => this.onResizeStart(event, id)}
                  @pointermove=${this.onResizeMove}
                  @pointerup=${this.onResizeEnd}
                  @pointercancel=${this.onResizeEnd}
                  @dblclick=${() => this.autoSizeColumn(id)}
                  @keydown=${(event: KeyboardEvent) => this.onResizeKey(event, id)}
                ></span>
              ` : nothing}
              ${this.activeFilterColumn === id ? html`
                <div part="filter-panel">
                  <input
                    type="search"
                    aria-label=${this.localize('tableFilterLabel')}
                    .value=${safeText(this.filters.find((filter) => filter.id === id)?.value)}
                    @input=${(event: Event) => this.onFilterInput(event, id)}
                  />
                </div>
              ` : nothing}
            </div>
  ...
  private renderExpandButton(item: DataDisplayRow<Row>): TemplateResult | typeof nothing {
    const expandable = this.childrenFor(item.row).length > 0 || this.rowDetail !== null;
    ...
    return html`
      ...
      ><span data-expanded=${expanded ? 'true' : 'false'}>${chevronIcon()}</span></button>
    `;
  }

  private renderDataRow(item: DataDisplayRow<Row>, rowPosition: number): TemplateResult {
    ...
    return html`
      <div
        ...
        aria-selected=${this.selectionEnabled ? (selection.checked ? 'true' : 'false') : nothing}
        aria-expanded=${this.childRows && expandable ? (expanded ? 'true' : 'false') : nothing}
      ...
      >
        ...
        ` : nothing}
        ${this.visibleColumns.map(({ column, id }, columnPosition) => html`
          <div
            part="cell"
            role="gridcell"
            aria-colindex=${columnPosition + 1 + selectionOffset}
            tabindex=${this.focusedRow === rowPosition && this.focusedColumn === columnPosition ? '0' : '-1'}
            data-focus-cell
            ...
            data-pin=${this.getColumnPin(id) || nothing}
            style=${styleMap(this.columnStyle(column, id))}
            @focus=${() => { this.focusedRow = rowPosition; this.focusedColumn = columnPosition; }}
            @click=${(event: MouseEvent) => this.onCellClick(event, item, column, rowPosition)}
            @contextmenu=${(event: MouseEvent) => this.emitCellContextMenu(event, item, column, rowPosition)}
            @keydown=${(event: KeyboardEvent) => this.onGridKeyDown(event, rowPosition, columnPosition, item, column, id)}
          >
      ...
      ${expanded && this.rowDetail ? html`
        <div role="row">
          <div
            part="row-detail"
            role="gridcell"
            aria-colspan=${Math.max(1, this.visibleColumns.length + selectionOffset)}
          >${this.rowDetail(item.row)}</div>
        </div>
      ` : nothing}
    `;
  }

  private renderGroupRow(item: DataDisplayGroup<Row>, rowPosition: number): TemplateResult {
    const expanded = arrayHasKey(this.expandedKeys, item.key);
    ...
    return html`
      ...
      >
        <div
          part="group-value"
          role="gridcell"
          aria-colspan=${Math.max(1, this.visibleColumns.length + (this.selectionEnabled ? 1 : 0))}
          tabindex=${this.focusedRow === rowPosition ? '0' : '-1'}
          data-focus-cell
          data-row-position=${rowPosition}
          data-column-position="0"
          @focus=${() => { this.focusedRow = rowPosition; this.focusedColumn = 0; }}
          @keydown=${(event: KeyboardEvent) => this.onGridKeyDown(event, rowPosition, 0)}
        >
          ${this.selectionMode === 'multiple' ? html`
            <input
              type="checkbox"
          ...
          <span part="group-count">${getNumberFormat(this.effectiveLocale).format(item.rows.length)}</span>
          ${this.visibleColumns.map(({ column }) => {
            ...
            return html`<span>${column.aggregatedFormatter
  ...
  private renderBodyContent(): TemplateResult {
    if (this.columns.length === 0 || this.visibleColumns.length === 0) {
      return html`
        <div role="row" aria-rowindex="2">
          <div part="empty" role="gridcell" aria-colindex="1"><slot name="empty">${this.localize('noColumns')}</slot></div>
        </div>
      `;
    }
    if (this.data.length === 0) {
      return html`
        <div role="row" aria-rowindex="2">
          <div part="empty" role="gridcell" aria-colspan=${Math.max(1, this.visibleColumns.length)}><slot name="empty">${this.localize('noData')}</slot></div>
        </div>
      `;
    }
    if (this.displayItems.length === 0) {
      return html`
        <div role="row" aria-rowindex="2">
          <div part="no-results" role="gridcell" aria-colspan=${Math.max(1, this.visibleColumns.length)}><slot name="no-results">${this.localize('noMatches')}</slot></div>
        </div>
      `;
    }
    ...
    const height = this.resolvedRowHeight;
    return html`
      ${window.start > 0 ? html`<div aria-hidden="true" style=${styleMap({ height: `${window.start * height}px` })}></div>` : nothing}
      ${repeat(
        window.items,
        (item) => item.kind === 'group' ? item.key : `row:${typeof item.key}:${item.key}`,
        (item, localIndex) => {
      ...
      ${window.end < this.displayItems.length ? html`
        <div aria-hidden="true" style=${styleMap({ height: `${(this.displayItems.length - window.end) * height}px` })}></div>
      ` : nothing}
    `;
  }

  private renderFooter(): TemplateResult | typeof nothing {
    const hasFooter = this.visibleColumns.some(({ column }) => column.footer !== undefined);
    if (!hasFooter) return nothing;
    const rows = this.getProcessedRows();
    return html`
      <div part="footer">
        <div part="footer-row" role="row" style=${styleMap({ '--data-grid-columns': this.gridTemplate })}>
          ${this.selectionEnabled ? html`<div part="footer-cell" role="gridcell"></div>` : nothing}
          ${this.visibleColumns.map(({ column }) => html`
            <div part="footer-cell" role="gridcell">
              ${typeof column.footer === 'function' ? column.footer(rows) : column.footer ?? nothing}
            </div>
          `)}
        </div>
      </div>
  ...
  private renderPager(): TemplateResult | typeof nothing {
    ...
    const format = getNumberFormat(this.effectiveLocale, { maximumFractionDigits: 0 });
    ...
    return html`
      <nav part="pager" aria-label=${this.localize('paginationLabel')}>
        ...
        >
          ${[...new Set(this.pageSizeOptions.map((value) => finiteCount(value)).filter((value) => value > 0))]
            .map((value) => html`<option value=${value}>${format.format(value)}</option>`)}
        </select>
        <button
        ...
        ><span aria-hidden="true">‹</span></button>
        ${numbers[0] !== undefined && numbers[0] > 0 ? html`<span part="ellipsis" aria-hidden="true">…</span>` : nothing}
        ${numbers.map((page) => page === current ? html`
          <button
            part="pager-button page page-current"
            type="button"
            aria-current="page"
            aria-label=${this.localize('paginationJumpToPage', undefined, { page: format.format(page + 1) })}
            @click=${() => this.applyPageChange(page)}
          >${format.format(page + 1)}</button>
        ` : html`
          <button
          ...
          >${format.format(page + 1)}</button>
        `)}
        ${numbers.at(-1) !== undefined && numbers.at(-1)! < count - 1 ? html`<span part="ellipsis" aria-hidden="true">…</span>` : nothing}
        <button
  ...
  protected override render(): TemplateResult {
    const accessibleName = this.getAttribute('aria-label') || this.label || undefined;
    ...
    const columnCount = Math.max(1, this.visibleColumns.length + (this.selectionEnabled ? 1 : 0));
    const role = this.childRows || normalizedGroupBy(this.groupBy).length > 0 ? 'treegrid' : 'grid';
    return html`
      <div part="data-grid">
        ${this.renderToolbar()}
        <div
        ...
        >
          ${this.renderHeader()}
          <div part="body" role="rowgroup" tabindex="0" @scroll=${this.onBodyScroll}>
