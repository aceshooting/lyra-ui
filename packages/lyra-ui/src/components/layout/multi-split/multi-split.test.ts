...
function installOwnerResizeObserverStub(owner: Window): {
  ...
  (
    owner as unknown as { ResizeObserver: typeof ResizeObserver }
 ...
 *  `finally`. */
function installMatchMediaStub(initialWidth: number): {
    ...
    setWidth(next: number): void {
      ...
      lists.forEach((entry, i) => {
        ...
        for (const fn of [...entry.listeners])
          fn({ matches, media: entry.media } as MediaQueryListEvent);
      });
    },
    restore(): void {
...
it('does not swap ArrowUp/ArrowDown for vertical orientation under dir="rtl" (direction only affects the horizontal inline axis)', async () => {
  const el = (await fixture(
    html`<lr-multi-split dir="rtl" orientation="vertical"
      ><div>A</div>
      <div>B</div></lr-multi-split
    >`
  )) as LyraMultiSplit;
...
it("ignores secondary mouse and pen button drags while keeping concurrent touch drags active", async () => {
  ...
  dividers.forEach(
    (divider) =>
      (divider.setPointerCapture = (pointerId: number) =>
...
it("falls back to observed base geometry when a ResizeObserver entry omits contentBoxSize", async () => {
  ...
  try {
    const el = (await fixture(
      html`<lr-multi-split
        collapse="start"
        style="display: block; inline-size: 800px;"
        ><div>A</div>
        <div>B</div></lr-multi-split
      >`
    )) as LyraMultiSplit;
...
it("applies flex styles and interleaving order to panels", async () => {
  ...
  const [panelA, panelB, panelC] = [...el.children] as [HTMLElement, HTMLElement, HTMLElement];
...
for (const direction of ["ltr", "rtl"] as const) {
  it(`reserves a contained horizontal divider gutter without stealing ${direction.toUpperCase()} panel input`, async () => {
    const el = (await fixture(html`
      ...
      >
        <button data-side="a" style="inline-size:100%; block-size:100%">
          A
        </button>
        <button data-side="b" style="inline-size:100%; block-size:100%">
          B
        </button>
      </lr-multi-split>
    `)) as LyraMultiSplit;
    ...
    const overlapA =
      Math.min(dividerRect.right, aRect.right) -
      Math.max(dividerRect.left, aRect.left);
    const overlapB =
      Math.min(dividerRect.right, bRect.right) -
      Math.max(dividerRect.left, bRect.left);
    ...
    for (const [panel, expected] of [
      ...
      const edgeX =
        Math.abs(rect.right - dividerRect.left) < 1
          ? rect.right - 1
          : rect.left + 1;
      const hit = panel.ownerDocument.elementFromPoint(
        edgeX,
...
it("reserves a contained vertical divider gutter and paints only its center", async () => {
  const el = (await fixture(html`
    <lr-multi-split
      ...
      style="inline-size: 320px; block-size: 240px"
    >
      <button data-side="a" style="inline-size:100%; block-size:100%">A</button>
      <button data-side="b" style="inline-size:100%; block-size:100%">B</button>
    </lr-multi-split>
  `)) as LyraMultiSplit;
  await elementUpdated(el);
  const base = el.shadowRoot!.querySelector('[part="base"]') as HTMLElement;
  ...
  for (const panel of [panelA, panelB]) {
    const rect = panel.getBoundingClientRect();
    const overlap =
      Math.min(dividerRect.bottom, rect.bottom) -
      Math.max(dividerRect.top, rect.top);
  ...
  const hit = panelA.ownerDocument.elementFromPoint(
...
it("preserves and restores custom panel proportions across an appended-then-removed panel", async () => {
  ...
  const ratios = el.sizes
    .slice(0, 4)
    .map((s) => Math.round((s / existingTotal) * 1000));
...
it("keeps sizes summing to 100 when two adjacent dividers are dragged concurrently", async () => {
  ...
  await el.updateComplete;
  const dividers = el.shadowRoot!.querySelectorAll(
    '[part="divider"]'
  ...
  (
    dividers[0]! as unknown as { setPointerCapture(id: number): void }
  ).setPointerCapture = () => {};
  (
    dividers[1]! as unknown as { setPointerCapture(id: number): void }
  ).setPointerCapture = () => {};
...
it("clears inline flex/order styles from a panel removed from the slot", async () => {
  const el = (await fixture(
    html`<lr-multi-split
      ><div id="p1">a</div>
      <div id="p2">b</div></lr-multi-split
    >`
  )) as LyraMultiSplit;
  await el.updateComplete;
...
it("routes an adopted divider drag through its owner window and removes that realm's listeners on adoption", async () => {
  ...
  frameWindow.removeEventListener = ((
    ...
    if (listener) originalRemoveEventListener.call(frameWindow, type, listener, options);
...
it("commits sizes to localStorage exactly once on pointerup, not just via keyboard commit", async () => {
  ...
  await elementUpdated(el);
  const persistenceOwner = el as unknown as { persist: () => void };
  const originalPersist = persistenceOwner.persist.bind(el);
  let persistenceCommits = 0;
  persistenceOwner.persist = (): void => {
...
it("suppresses resize events and persistence for clamped keyboard and pointer no-ops", async () => {
  ...
  divider.dispatchEvent(boundaryKey);
  pointerDown(divider, 811, 100);
  pointerMove(811, 100);
  pointerMove(811, 0);
  pointerMove(811, -100);
  ...
  expect(el.sizes).to.deep.equal([10, 90]);
...
it("redistributes an unconstrained sibling into space freed by a maxPx-clamped panel", async () => {
  ...
  expect(
    match,
    `unexpected flex-basis shape: ${panelB.style.flex}`
  ).to.not.equal(null);
...
it("falls back to sentinel px bounds when a constraint only specifies one side", async () => {
  ...
  const match = /^0 1 clamp\(40px, 30%, ([\d.e+]+)px\)$/.exec(
    panelA.style.flex
  );
  expect(
    match,
    `unexpected flex-basis shape: ${panelA.style.flex}`
  ).to.not.equal(null);
  ...
  it(`keeps long direct panels contained, horizontally unoverflowing, and independently scrollable in a fixed 320px block allocation (${direction})`, async () => {
    const wrapper = await fixture(html`
      <div style="inline-size: 320px; block-size: 200px">
        <lr-multi-split dir=${direction} style="block-size: 100%">
          <div data-panel="first">
            <div style="block-size: 40rem" data-token>
              ${unbrokenPanelToken}
            </div>
          </div>
    ...
    expect(Math.round(base.scrollHeight)).to.equal(
      Math.round(base.clientHeight)
    );
...
it("splits :hover and :focus-visible into separate divider rules with a token-driven outline", () => {
  ...
  expect(css).to.not.match(
    /\[part=['"]?divider['"]?]:hover\s*,\s*\[part=['"]?divider['"]?]:focus-visible/
  );

  const hoverBlock = /\[part=['"]?divider['"]?]:hover::before\s*{([^}]*)}/.exec(
    css
  );
  expect(
    hoverBlock,
  ...
  expect(
    focusVisibleBlock,
    'expected a standalone [part="divider"]:focus-visible rule'
  ).to.not.equal(null);
...
it("replaces the responsive observer with the destination owner constructor after adoption and ignores stale callbacks", async () => {
  ...
  try {
    ...
    expect(
      destination.records[0]!.observed[0]!.ownerDocument === frameDocument
    ).to.equal(true);
...
it('resolves collapse="start"/"end" to the same physical panel indices (0 / last) under a simulated RTL host, since panel 0 already renders at the logical inline-start edge regardless of direction (same as the existing pointer-drag RTL behavior above)', async () => {
  ...
  try {
    ...
    fireCollapseResize(spy.callbacks[0]!, 500);
    await elementUpdated(el);
    const [panelA, , panelC] = [...el.children] as [HTMLElement, HTMLElement, HTMLElement];
    expect(panelA.dataset["collapseState"]).to.equal("rail");
...
it("reconciles a directly-assigned sizes array of the wrong length from willUpdate(), not updated(), avoiding the same redundant-render pattern", async () => {
  ...
  expect((el as unknown as { isUpdatePending: boolean }).isUpdatePending).to.be
    .false;
});
...
it('anchors the floating overlay to inset-inline-end for collapse="end" (vs. inset-inline-start for collapse="start")', async () => {
  ...
  try {
    ...
    await elementUpdated(el);
    fireCollapseResize(spy.callbacks[0]!, 300); // floating
    // Reveal the hidden-by-default drawer (see multi-split.class.ts's class doc) so its
...
it('moves a divider directly to both achievable extremes with Home and End', async () => {
  const el = (await fixture(html`
    <lr-multi-split min="10"><div>A</div><div>B</div></lr-multi-split>
  `)) as LyraMultiSplit;
  await elementUpdated(el);
  el.sizes = [50, 50];
  await elementUpdated(el);
  ...
  await elementUpdated(el);
  expect(el.sizes.map(Math.round)).to.deep.equal([10, 90]);

  divider.dispatchEvent(new KeyboardEvent('keydown', { key: 'End', bubbles: true }));
  await elementUpdated(el);
  expect(el.sizes.map(Math.round)).to.deep.equal([90, 10]);
});
...
it('inerts sibling panes behind an open floating drawer while leaving the drawer interactive', async () => {
  const spy = installResizeObserverSpy();
  try {
    const el = (await fixture(html`
      <lr-multi-split
        collapse="start"
        style="inline-size: 300px; block-size: 200px"
      ><div><button>Drawer action</button></div><div><button>Behind action</button></div></lr-multi-split>
    `)) as LyraMultiSplit;
    await elementUpdated(el);
    fireCollapseResize(spy.callbacks[0]!, 300);
    await elementUpdated(el);
    ...
    const [drawer, behind] = [...el.children] as [HTMLElement, HTMLElement];
    expect(drawer.inert).to.equal(false);
    expect(behind.inert).to.equal(true);

    el.open = false;
    await elementUpdated(el);
    expect(behind.inert).to.equal(false);
  } finally {
    spy.restore();
  }
});
  ...
  try {
    ...
    const el = (await fixture(
      html`<lr-multi-split
        collapse="start"
        style="inline-size: 300px; block-size: 200px"
        ><div><button>first</button><button>last</button></div>
        <div>B</div></lr-multi-split
      >`
    )) as LyraMultiSplit;
    await elementUpdated(el);
...
it('closes the floating drawer when a real pointer targets its inert backdrop', async () => {
  const spy = installResizeObserverSpy();
  try {
    const el = (await fixture(html`
      <lr-multi-split
...
describe("dividerLabel", () => {
  it("overrides the divider aria-label template when set", async () => {
    ...
    `)) as LyraMultiSplit;
    el.dividerLabel = (index: number, panelCount: number) =>
  ...
  it("crosses at the same width for a rem rail breakpoint as for its px equivalent", async () => {
    ...
    try {
      const el = (await fixture(html`
        <lr-multi-split
          collapse="start"
          rail-breakpoint="68.75rem"
          float-breakpoint="200"
        >
          <div>A</div>
          <div>B</div>
        </lr-multi-split>
  ...
  it("crosses at the same width for a rem float breakpoint as for its px equivalent", async () => {
    ...
    try {
      const el = (await fixture(html`
        <lr-multi-split
          collapse="start"
          rail-breakpoint="1200px"
          float-breakpoint="20rem"
        >
          <div>A</div>
          <div>B</div>
        </lr-multi-split>
  ...
  it("falls back to the documented 640/400 defaults for unparseable lengths", async () => {
    ...
    try {
      ...
      const el = (await fixture(html`
        <lr-multi-split
          collapse="start"
          rail-breakpoint="80vw"
          float-breakpoint="calc(100% - 2rem)"
        >
          <div>A</div>
          <div>B</div>
        </lr-multi-split>
  ...
  it("releases and reacquires exact panel state across disconnect/reconnect", async () => {
    const el = (await fixture(html`
      <lr-multi-split .sizes=${[40, 60]}>
        <div
          style="flex: 7 7 7px !important; order: 9 !important"
          data-collapse-state="author"
        >
          A
        </div>
        <div>B</div>
  ...
  it("keeps forced collapse/open inert while disabled, then emits only real enable/disable transitions", async () => {
    const wrapper = await fixture<HTMLDivElement>(html`
      <div>
        ...
        <lr-multi-split style="inline-size:300px; block-size:160px">
          <div><button id="multi-split-inside">Inside</button></div>
          <div>Main</div>
        </lr-multi-split>
      </div>
    `);
  ...
  it("normalizes and cleans up when the active collapsing pane is removed", async () => {
    const wrapper = await fixture<HTMLDivElement>(html`
      <div>
        ...
        <lr-multi-split
          collapse="start"
          style="inline-size:300px; block-size:160px"
        >
          <div><button id="multi-split-remove-inside">Inside</button></div>
          <div>Main</div>
        </lr-multi-split>
      </div>
    `);
