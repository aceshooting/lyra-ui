...
it("renders label and sublabel in the header", async () => {
  ...
  expect(
    el.shadowRoot!.querySelector('[part="sublabel"]')!.textContent
  ).to.equal("Last 7 days");
});

describe("icon slot", () => {
  it("renders a leading icon from the icon slot", async () => {
    const el = (await fixture(
      html`<lr-widget label="Load"
        ><svg slot="icon" width="16" height="16"></svg>content</lr-widget
      >`
    )) as LyraWidget;
...
describe("collapse-icon slot override", () => {
  ...
  it("renders a custom icon slotted into collapse-icon instead of the default chevron", async () => {
    ...
    expect((assigned[0] as HTMLElement).classList.contains("my-collapse-icon"))
      .to.be.true;
  });
});
...
describe("fullscreen-icon slot override", () => {
  ...
  it("renders a custom icon slotted into fullscreen-icon instead of the default expand/close glyph", async () => {
    ...
    expect(assigned.length).to.equal(1);
    expect(
      (assigned[0] as HTMLElement).classList.contains("my-fullscreen-icon")
    ).to.be.true;
    ...
    const slotAfter = btn.querySelector(
      'slot[name="fullscreen-icon"]'
    ) as HTMLSlotElement;
...
describe("rich label/sublabel", () => {
  it("lets the label slot override the label attribute instead of concatenating both", async () => {
    const el = (await fixture(
      html`<lr-widget label="attr"
        ><span slot="label">rich</span>content</lr-widget
      >`
    )) as LyraWidget;
    const slot = el.shadowRoot!.querySelector(
      'slot[name="label"]'
  ...
  it("renders label/sublabel as plain strings when no slot content is provided (unchanged default)", async () => {
    const el = (await fixture(
      html`<lr-widget label="Load" sublabel="Last 7 days">content</lr-widget>`
    )) as LyraWidget;
    expect(
  ...
  it('detects a slot="label" child appended after connect even when label/sublabel start empty', async () => {
    ...
    const el = (await fixture(
      html`<lr-widget>content</lr-widget>`
    )) as LyraWidget;
    const labelPart = el.shadowRoot!.querySelector(
      '[part="label"]'
    ) as HTMLElement;
    const sublabelPart = el.shadowRoot!.querySelector(
      '[part="sublabel"]'
    ) as HTMLElement;
  ...
  it("updates the fullscreen dialog name when text inside the assigned label mutates", async () => {
    const el = (await fixture(html`
      <lr-widget expandable
        ><span slot="label">Original</span>content</lr-widget
      >
    `)) as LyraWidget;
    (
    ...
    await el.updateComplete;
    const base = el.shadowRoot!.querySelector('[part="base"]') as HTMLElement;
    expect(base.getAttribute("aria-label")).to.equal("Original");
...
describe("views", () => {
  ...
  it("hides arbitrary view icons from the toggle accessible name", async () => {
    const el = (await fixture(html`
      <lr-widget
        label="Usage"
        .views=${[
  ...
  it("applies a first-occurrence policy to duplicate view IDs", async () => {
    ...
    expect(toggles).to.have.length(2);
    expect(toggles.map((toggle) => toggle.textContent?.trim())).to.deep.equal([
      "First",
      "Other",
    ]);
    expect(
...
it("truncates long label/sublabel text instead of wrapping and growing the header", async () => {
  ...
  const el = (await fixture(
    html`<lr-widget
      label=${longText}
      sublabel=${longText}
      style="max-inline-size: 8rem;"
      >content</lr-widget
    >`
  )) as LyraWidget;
  const label = el.shadowRoot!.querySelector('[part="label"]') as HTMLElement;
  const sublabel = el.shadowRoot!.querySelector(
    '[part="sublabel"]'
  ) as HTMLElement;
  const header = el.shadowRoot!.querySelector('[part="header"]') as HTMLElement;
  ...
  expect(
    label.scrollWidth,
    "sanity check: label text actually overflows its box"
  ).to.be.greaterThan(label.clientWidth);
  expect(
    sublabel.scrollWidth,
    "sanity check: sublabel text actually overflows its box"
  ).to.be.greaterThan(sublabel.clientWidth);
...
it("renders the actions wrapper visible on first paint when actions content is present before upgrade", async () => {
  const el = (await fixture(
    html`<lr-widget label="x"
      ><button slot="actions">Refresh</button>content</lr-widget
    >`
  )) as LyraWidget;
  const actions = el.shadowRoot!.querySelector(
    '[part="actions"]'
...
it("a registered locale supplies widgetCollapse/widgetExpand for the collapse-button aria-label", async () => {
  registerLyraLocale("fr-test-widget-collapse", {
    widgetCollapse: "Réduire",
    widgetExpand: "Développer",
  });
  const el = (await fixture(
    html`<lr-widget label="x" collapsible locale="fr-test-widget-collapse"
      >content</lr-widget
    >`
  )) as LyraWidget;
  await el.updateComplete;
...
it("localizes the collapse/fullscreen/view-group aria-labels and the fullscreen dialog fallback via .strings", async () => {
  ...
  expect(
    el
      .shadowRoot!.querySelector('[part="view-toggles"]')!
      .getAttribute("aria-label")
  ).to.equal("Vue du panneau");
  ...
  const fullscreenBtn = el.shadowRoot!.querySelector(
    '[part="fullscreen-button"]'
  ) as HTMLButtonElement;
  expect(fullscreenBtn.getAttribute("aria-label")).to.equal(
...
it("is accessible in both default and fullscreen-expandable states", async () => {
  const el = (await fixture(
    html`<lr-widget label="Load profile" collapsible expandable
      >content</lr-widget
    >`
  )) as LyraWidget;
  await expect(el).to.be.accessible();
...
it("falls back to a generic aria-label for the fullscreen dialog when no label is set", async () => {
  const el = (await fixture(
    html`<lr-widget expandable>content</lr-widget>`
  )) as LyraWidget;
  const base = el.shadowRoot!.querySelector('[part="base"]') as HTMLElement;
...
it("names the fullscreen dialog from a slotted label, not just the label attribute", async () => {
  const el = (await fixture(html`
    <lr-widget expandable
      ><span slot="label">Energy production</span>content</lr-widget
    >
  `)) as LyraWidget;
  const base = el.shadowRoot!.querySelector('[part="base"]') as HTMLElement;
...
it("lets a host aria-label override both label and a slotted label for the fullscreen dialog name", async () => {
  const el = (await fixture(html`
    <lr-widget expandable aria-label="Override" label="attr"
      ><span slot="label">rich</span>content</lr-widget
    >
  `)) as LyraWidget;
  const base = el.shadowRoot!.querySelector('[part="base"]') as HTMLElement;
...
it("traps Tab/Shift+Tab at a slotted element whose focusable target lives in its own shadow root", async () => {
  ...
  const shadowHost = el.querySelector(
    "widget-test-shadow-input"
  ) as WidgetTestShadowInput;
...
it("excludes a collapsed body's slotted focusable content from the fullscreen tab trap", async () => {
  const el = (await fixture(
    html`<lr-widget label="x" collapsible collapsed expandable
      ><button>inner</button></lr-widget
    >`
  )) as LyraWidget;
  const collapseBtn = el.shadowRoot!.querySelector(
    '[part="collapse-button"]'
...
it("reclaims focus inside the fullscreen panel when collapsing hides the currently focused body content", async () => {
  const el = (await fixture(
    html`<lr-widget label="x" collapsible expandable
      ><button id="inner">inner</button></lr-widget
    >`
  )) as LyraWidget;
  const fullscreenBtn = el.shadowRoot!.querySelector(
    '[part="fullscreen-button"]'
  ...
  inner.focus();
  expect((el.shadowRoot!.activeElement ?? document.activeElement) === inner).to
    .be.true;
...
it("applies a custom fullscreen-inset while fullscreen", async () => {
  const el = (await fixture(
    html`<lr-widget expandable fullscreen fullscreen-inset="0 0 0 240px"
      ><p>Body</p></lr-widget
    >`
  )) as LyraWidget;
  const base = el.shadowRoot!.querySelector('[part="base"]') as HTMLElement;
...
describe("backdrop-inset", () => {
  ...
  it("decouples backdrop inset from fullscreen-inset when backdrop-inset is set", async () => {
    ...
    const base = el.shadowRoot!.querySelector('[part="base"]') as HTMLElement;
    expect(getComputedStyle(backdrop).inset).to.not.equal(
...
describe("storage-key persistence", () => {
  it("persists and restores collapsed via storage-key", async () => {
    ...
    const restored = await fixture<LyraWidget>(
      html`<lr-widget storage-key=${key} collapsible></lr-widget>`
    );
  ...
  it("writes nothing to storage when storage-key is unset (unset-regression)", async () => {
    ...
    const el = await fixture<LyraWidget>(
      html`<lr-widget collapsible></lr-widget>`
    );
    el.collapsed = true;
    await el.updateComplete;
...
// deprecation record. It was restored in 11.2.0 as a deprecated alias that SEEDS the canonical
// property, with `removalNotBefore: '13.0.0'`. That version is long past, so 16.0.0 removes it
// outright -- `activeView` is now a plain, unobserved expando: Lit no longer manages it as a
// reactive property, and setting it via a property binding no longer seeds `activeViewId`.
describe("activeView (removed 16.0.0; no longer an alias for activeViewId)", () => {
  const VIEWS = [
    { viewId: "chart", label: "Chart" },
    { viewId: "table", label: "Table" },
  ];
  ...
  it("is not registered as a Lit reactive property", async () => {
    const el = (await fixture(html`<lr-widget label="Usage"></lr-widget>`)) as LyraWidget;
    const ctor = el.constructor as unknown as { elementProperties: Map<string, unknown> };
    expect(ctor.elementProperties.has("activeView")).to.be.false;
  });

  it("does not seed activeViewId when set via property binding", async () => {
    const el = (await fixture(html`
