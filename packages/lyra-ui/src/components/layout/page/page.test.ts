...
const access = (page: LyraPage): PageTestAccess =>
  page as unknown as PageTestAccess;
...
it('renders all 15 slots and all 22 documented parts, with base/page on one node', async () => {
  const page = (await fixture(html`<lr-page></lr-page>`)) as LyraPage;
  const slots = [...page.shadowRoot!.querySelectorAll('slot')].map(
    (slot) => slot.name
  );
  ...
  const parts = new Set(
    [...page.shadowRoot!.querySelectorAll<HTMLElement>('[part]')].flatMap(
      (node) => (node.getAttribute('part') ?? '').split(/\s+/).filter(Boolean)
    )
  );
  for (const part of [
    'aside',
...
it('uses semantic landmarks and per-instance skip/navigation targets', async () => {
  const wrapper = (await fixture(
    html`<div><lr-page></lr-page><lr-page></lr-page></div>`
  )) as HTMLElement;
  const [first, second] = [
    ...wrapper.querySelectorAll('lr-page'),
  ] as LyraPage[];
  ...
  expect(byPart(first!, 'skip-to-content').getAttribute('href')).to.equal(
    `#${first!.id}`
  );
  expect(byPart(second!, 'skip-to-content').getAttribute('href')).to.equal(
    `#${second!.id}`
  );
});
...
it('localizes the skip link, navigation landmark, and open/close toggle names', async () => {
  ...
  expect(navigation.getAttribute('aria-label')).to.equal(
...
it('uses rich skip-to-content slot content in place of the localized fallback', async () => {
  const page = (await fixture(html`
    <lr-page
      ><strong slot="skip-to-content"
        >Jump straight to the report</strong
      ></lr-page
    >
  `)) as LyraPage;
  const slot = page.shadowRoot!.querySelector<HTMLSlotElement>(
    'slot[name="skip-to-content"]'
  )!;
  expect(slot.assignedElements().map((node) => node.localName)).to.deep.equal([
    'strong',
  ]);
...
it('moves focus to its own main landmark when the skip link is activated', async () => {
  const page = (await fixture(
    html`<lr-page><p>Report</p></lr-page>`
  )) as LyraPage;
...
it('emits a cancelable lr-nav-toggle before mutating navOpen, honoring a prevented request', async () => {
  ...
  page.addEventListener('lr-nav-toggle', (event) => {
    ...
    order.push({
      type: event.type,
      open: detail.open,
      navOpenAtDispatch: page.navOpen,
    });
...
it('gives the default toggle an exact controls/expanded contract and supports its icon slot', async () => {
  const page = (await fixture(html`
    <lr-page style="inline-size:320px"
      ><span slot="navigation-toggle-icon">Menu</span></lr-page
    >
  `)) as LyraPage;
  access(page).applyMeasuredInlineSize(320);
  await page.updateComplete;
...
it('keeps slotted skip content and toggle glyph controls decorative while preserving the outer actions', async () => {
  const root = await fixture(html`
    <div>
      ...
      <lr-page style="inline-size:320px">
        <button id="skip-control" slot="skip-to-content" type="button">
          Jump to report
        </button>
        <button
          id="toggle-icon-control"
  ...
  const toggleIconControl = page.querySelector<HTMLButtonElement>(
    '#toggle-icon-control'
  )!;
  const skip = byPart(page, 'skip-to-content');
  const main = byPart(page, 'main');
  ...
  const skipVisual = skipSlot.closest<HTMLElement>(
    '[inert][aria-hidden="true"]'
  );
  const iconVisual = iconSlot.closest<HTMLElement>(
    '[inert][aria-hidden="true"]'
  );

  expect(
    skipVisual?.id ?? '',
    'the skip visual layer has a stable accessible-name target'
  ).to.not.equal('');
...
it('keeps a custom navigation-toggle operable when the default is disabled and avoids double data-toggle activation', async () => {
  ...
  access(page).applyMeasuredInlineSize(320);
  await page.updateComplete;
  const custom = page.querySelector<HTMLButtonElement>(
    '[slot="navigation-toggle"]'
  )!;
...
it('synchronizes every assigned navigation-toggle and lets each available control own the drawer action', async () => {
  const page = (await fixture(html`
    <lr-page style="inline-size:320px" disable-navigation-toggle>
      <button id="primary-toggle" slot="navigation-toggle">
        Primary sections
      </button>
      <button
        id="secondary-toggle"
  ...
  access(page).applyMeasuredInlineSize(320);
  await page.updateComplete;
  const primary = page.querySelector<HTMLButtonElement>('#primary-toggle')!;
  const secondary = page.querySelector<HTMLButtonElement>('#secondary-toggle')!;

  for (const toggle of [primary, secondary]) {
    expect(toggle.getAttribute('aria-haspopup')).to.equal('dialog');
    expect(toggle.getAttribute('aria-expanded')).to.equal('false');
    expect(toggle.getAttribute('aria-controls')?.split(/\s+/)).to.include(
      page.id
    );
  }
  ...
  await page.updateComplete;
  expect(page.navOpen).to.equal(true);
  expect(primary.getAttribute('aria-expanded')).to.equal('true');
  expect(secondary.getAttribute('aria-expanded')).to.equal('true');

  page.hideNavigation();
  await page.updateComplete;
  expect(page.navOpen).to.equal(false);
  expect(primary.getAttribute('aria-expanded')).to.equal('false');
  expect(secondary.getAttribute('aria-expanded')).to.equal('false');
...
it('does not activate disabled, aria-disabled, or inert secondary navigation toggles', async () => {
  const page = (await fixture(html`
    <lr-page style="inline-size:320px" disable-navigation-toggle>
      <button id="available-toggle" slot="navigation-toggle">Available</button>
      <button id="disabled-toggle" slot="navigation-toggle" disabled>
        Disabled
      </button>
      <button
        id="aria-disabled-toggle"
        slot="navigation-toggle"
        aria-disabled=" TRUE "
      >
        ARIA disabled
      </button>
      <button id="inert-toggle" slot="navigation-toggle" inert>Inert</button>
      <button slot="navigation">Inside navigation</button>
    </lr-page>
  `)) as LyraPage;
  access(page).applyMeasuredInlineSize(320);
  ...
  for (const id of [
    ...
    await page.updateComplete;
    expect(page.navOpen, id).to.equal(false);
  }

  page.querySelector<HTMLButtonElement>('#available-toggle')!.click();
  await page.updateComplete;
  expect(page.navOpen).to.equal(true);
});
...
it('owns the real custom-toggle control and retargets focus restoration after replacement', async () => {
  ...
  expect(replacement.shadowRoot!.activeElement === replacementControl).to.equal(
    true
  );
});
...
it('honors data-toggle-nav controls in any slotted page region', async () => {
  const page = (await fixture(html`
    <lr-page style="inline-size:320px"
      ><button slot="header" data-toggle-nav>Sections</button></lr-page
    >
  `)) as LyraPage;
  access(page).applyMeasuredInlineSize(320);
  await page.updateComplete;
...
it('falls back to the documented breakpoint for an invalid CSS length', async () => {
  const page = (await fixture(
    html`<lr-page mobile-breakpoint="calc(100% - 1px)"></lr-page>`
  )) as LyraPage;
...
it('keeps the exact slotted navigation node and focus while crossing desktop/mobile views', async () => {
  const page = (await fixture(html`
    <lr-page style="inline-size:900px"
      ><button slot="navigation">Focused section</button></lr-page
    >
  `)) as LyraPage;
  const navigationButton = page.querySelector<HTMLButtonElement>(
    '[slot="navigation"]'
  )!;
  ...
  access(page).applyMeasuredInlineSize(320);
  await page.updateComplete;
  expect(page.querySelector('[slot="navigation"]')?.textContent).to.equal(
    'Focused section'
  );
  expect(document.activeElement === navigationButton).to.equal(true);
...
it('dismisses the open mobile drawer from its backdrop wrapper', async () => {
  const page = (await fixture(html`
    <lr-page style="inline-size:320px"
      ><button slot="navigation">Inside</button></lr-page
    >
  `)) as LyraPage;
  access(page).applyMeasuredInlineSize(320);
  page.showNavigation();
...
it('places navigation on the logical start/end edge in LTR and RTL', async () => {
  const page = (await fixture(html`
    <lr-page style="inline-size:320px" nav-open
      ><button slot="navigation">Inside</button></lr-page
    >
  `)) as LyraPage;
  access(page).applyMeasuredInlineSize(320);
...
it('backs all six Web Awesome sizing aliases with canonical --lr-page-* properties', async () => {
  ...
  expect(getComputedStyle(byPart(page, 'banner')).minBlockSize).to.equal(
    '17px'
  );
  expect(getComputedStyle(byPart(page, 'header')).minBlockSize).to.equal(
    '19px'
  );
  expect(getComputedStyle(byPart(page, 'subheader')).minBlockSize).to.equal(
    '23px'
  );
...
it('uses token-driven drawer motion and renders the reduced-motion branch with no transition', async () => {
  ...
  expect(getComputedStyle(drawer).transitionDuration === '0s').to.equal(
    reduced
  );

  const reducedRule = page
      ...
      (rule): rule is CSSMediaRule =>
        ...
        [...rule.cssRules].some(
          (nested) =>
            ...
            nested.selectorText.includes('drawer')
        )
    );
...
it('treats an element in a detached document with no viewport as not visible', async () => {
  ...
  const detachedDocument =
    document.implementation.createHTMLDocument('Detached page');
...
it('constructs safely while detached and retains pre-upgrade light-DOM identity on connection', async () => {
  ...
  expect(page.firstElementChild?.textContent).to.equal(
    'Server-rendered navigation'
  );
});

it('preserves authored fragment identity and focusability from construction through connection', async () => {
  const page = document.createElement('lr-page') as LyraPage;
  page.id = 'authored-page';
  page.tabIndex = 2;
  const host = (await fixture(html`<div></div>`)) as HTMLElement;
  host.append(page);
  await page.updateComplete;
...
it('reclassifies its allocation when the window resizes', async () => {
  const el = (await fixture(html`
    <lr-page style="display: block; inline-size: 240px"
      ><div>Body</div></lr-page
    >
  `)) as LyraPage;
  ...
  expect(el.getAttribute('data-allocation') ?? el.className).to.not.equal(
    undefined
  );
...
it('operates foreign-realm navigation-toggle and delegated data-toggle controls', async () => {
  ...
  try {
    ...
    expect(
      customToggle instanceof HTMLElement,
...
describe('Page state cssprops', () => {
  ...
  it('keeps pre-cssprop skip, navigation-toggle, backdrop, and drawer paint when props are unset', async () => {
    ...
    try {
      ...
      expect(page.navOpen).to.equal(true);
      expect(
        getComputedStyle(byPart(page, 'dialog-wrapper')).backgroundColor
      ).to.equal(
        resolvedInShadow(
          page,
  ...
  it('inherits independent Page state properties from ancestors, including a slotted navigation toggle', async () => {
    ...
    try {
      ...
      expect(
        ...
        'state props do not recolor the Page surface'
      ).to.equal(pageSurfaceColor);
    } finally {
      await resetMouse();
      page.hideNavigation();
      await page.updateComplete;
    ...
    const customToggle = customPage.querySelector<HTMLElement>(
      '[slot="navigation-toggle"]'
    )!;
    ...
    expect(
      customToggle.getBoundingClientRect().width,
      'the slotted toggle has pointer geometry'
    ).to.be.greaterThan(0);
    ...
    try {
      ...
      expect(getComputedStyle(customToggle).color).to.equal(
        'rgb(255, 255, 255)'
      );
      await sendMouse({ type: 'down' });
      expect(
