...
it('does not swap ArrowUp/ArrowDown for vertical orientation under dir="rtl" (direction only affects the horizontal inline axis)', async () => {
  const el = (await fixture(
    html`<lr-split dir="rtl" orientation="vertical"
      ><div>A</div>
      <div>B</div></lr-split
    >`
  )) as LyraSplit;
...
it("uses defaultSizes only for initialization, below valid persistence and above equal distribution", async () => {
  ...
  const firstVisit = (await fixture(
    html`<lr-split storage-key=${firstVisitKey} .defaultSizes=${[20, 80]}
      ><div>A</div>
      <div>B</div></lr-split
    >`
  )) as LyraSplit;
  ...
  const restored = (await fixture(
    html`<lr-split storage-key=${restoredKey} .defaultSizes=${[20, 80]}
      ><div>A</div>
      <div>B</div></lr-split
    >`
  )) as LyraSplit;
...
it("falls back from invalid persisted sizes to defaultSizes through the initialization path", async () => {
  ...
  const el = (await fixture(
    html`<lr-split storage-key=${storageKey} min="10" .defaultSizes=${[25, 75]}
      ><div>A</div>
      <div>B</div></lr-split
    >`
  )) as LyraSplit;
...
it("switches the resize axis from its own inline-size breakpoint and reports the effective orientation", async () => {
  ...
  try {
    const el = (await fixture(
      html`<lr-split orientation-breakpoint="500" narrow-orientation="vertical"
        ><div>A</div>
        <div>B</div></lr-split
      >`
    )) as LyraSplit;
...
it("exposes the resolved container-basis orientation breakpoint in pixels via the private accessor", async () => {
  const el = (await fixture(
    html`<lr-split orientation-breakpoint="500"
      ><div>A</div>
      <div>B</div></lr-split
    >`
  )) as LyraSplit;
...
it("keeps the bare-number orientation breakpoint working from both the attribute and the property", async () => {
  ...
  try {
    const el = (await fixture(
      html`<lr-split orientation-breakpoint="900" narrow-orientation="vertical"
        ><div>A</div>
        <div>B</div></lr-split
      >`
    )) as LyraSplit;
...
it("treats an unparseable orientation breakpoint as unset (no observation, no effective marker)", async () => {
  ...
  try {
    const el = (await fixture(
      html`<lr-split orientation-breakpoint="abc" narrow-orientation="vertical"
        ><div>A</div>
        <div>B</div></lr-split
      >`
    )) as LyraSplit;
...
it("does not throw when localStorage.getItem/setItem are unavailable (e.g. blocked or quota-exceeded)", async () => {
  ...
  try {
    const el = (await fixture(
      html`<lr-split storage-key="blocked-test"
        ><div>A</div>
        <div>B</div></lr-split
      >`
    )) as LyraSplit;
...
it("rejects a stale persisted layout that violates a since-raised min, falling back to an equal split", async () => {
  ...
  const el = (await fixture(
    html`<lr-split storage-key=${storageKey} min="10"
      ><div>A</div>
      <div>B</div></lr-split
    >`
  )) as LyraSplit;
...
it("splits :hover and :focus-visible into separate divider rules with a token-driven outline", () => {
  ...
  expect(
    hoverBlock,
    'expected a standalone [part="divider"]:hover rule'
  ).to.not.equal(null);
...
it('resolves collapse="start"/"end" to the same physical panel indices (0 / last) under a simulated RTL host, since panel 0 already renders at the logical inline-start edge regardless of direction (same as the existing pointer-drag RTL behavior above)', async () => {
  ...
  try {
    const el = (await fixture(
      html`<lr-split dir="rtl" collapse="start"
        ><div>A</div>
        <div>B</div>
        <div>C</div></lr-split
      >`
  ...
  try {
    const el = (await fixture(
      html`<lr-split dir="rtl" collapse="end"
        ><div>A</div>
        <div>B</div>
        <div>C</div></lr-split
      >`
...
it("honors a custom rail-width for the rail state", async () => {
  ...
  try {
    const el = (await fixture(
      html`<lr-split collapse="start" rail-width="4rem"
        ><div>A</div>
        <div>B</div></lr-split
      >`
    )) as LyraSplit;
