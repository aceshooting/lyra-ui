...
export class LyraHeatmap extends LyraElement {
  ...
  protected willUpdate(changed: PropertyValues): void {
    ...
    // region, so unconditionally forcing role="img" here would tell
    // assistive tech to flatten the whole subtree into one image, hiding
    // both from the accessibility tree; it would also silently overwrite
    // any author-supplied role/aria-label on every update.
