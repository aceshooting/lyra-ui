 ...
 *   dialog body before it grows to fit content.
 * @cssprop [--lr-document-viewer-width=auto] - Assertive inline size for the nested dialog panel,
 *   forwarded to `<lr-dialog>`'s own `--lr-dialog-width`. Left at `auto` the panel shrink-wraps to
 *   content, capped by the dialog's own `--lr-dialog-max-width` tier -- set this instead of
 *   reaching through to `--lr-dialog-width` directly, which remains an internal implementation
 *   detail of the nested dialog.
