...
export const styles = css`
  :host { display: block; min-inline-size: 0; --lr-spreadsheet-viewer-max-height: none; }
  [part='base'], [part='body'], [part='sheet'] { display: flex; flex-direction: column; min-inline-size: 0; }
  [part='body'] { box-sizing: border-box; overflow-y: auto; overflow-x: hidden; max-block-size: var(--lr-spreadsheet-viewer-max-height); }
  ...
  [part~='cell-highlight'], lr-virtual-list::part(cell-highlight) { outline: var(--lr-border-width-medium) solid var(--lr-spreadsheet-viewer-highlight-color, var(--lr-color-brand)); outline-offset: var(--lr-spreadsheet-viewer-highlight-outline-offset, calc(-1 * var(--lr-border-width-medium))); padding: 0; }
