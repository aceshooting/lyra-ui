...
export const styles = css`
  :host {
    ...
    /* Consumer-tunable scroll cap; 'none' grows with content until a caller opts into an internal
       scrollbar via the max-height attribute. */
    --_lr-stack-trace-max-height: none;
    --_lr-stack-trace-font: var(--lr-font-mono);
  }
  [part="base"] {
    display: block;
    max-block-size: var(
  ...
  /* MUST stay after :host([compact]) [part='base']: equal specificity, so source order decides
     ...
     hover/focus affordances never depended on this border. */
  ...
  [part="group"] {
    margin-block-end: var(--lr-space-s);
    font-family: var(--lr-stack-trace-font, var(--_lr-stack-trace-font));
  }
  ...
  /* Hover recolours the label only, leaving pressed nothing to deepen, so pressed tints the
     frame's transparent surface toward --lr-color-mix-partner, which follows the text colour:
