...
export const Untitled: Story = {
  ...
  render: () => html`
    <lr-result-card style="max-inline-size:20rem;">
      <lr-result-field label="Rows affected" value="1,204"></lr-result-field>
      <lr-result-field label="Query time" value="88ms"></lr-result-field>
    </lr-result-card>
  `,
};
...
export const WithHeaderActions: Story = {
  render: () => html`
    <lr-result-card title="run_query" style="max-inline-size:20rem;">
      ...
      <lr-result-field label="Status" value="Success"></lr-result-field>
      <lr-result-field label="Rows" value="42"></lr-result-field>
    </lr-result-card>
  `,
};

/** `<lr-result-field>`'s default slot overrides the plain `value` prop
 *  once it carries real content -- here a `<lr-chip>` status badge stands
...
/** `<lr-result-field>` also works outside a `<lr-result-card>`, e.g. a
 *  compact metadata strip. */
export const FieldStandalone: Story = {
  render: () => html`
    <div style="display:flex; flex-direction:column; gap:0.25rem; max-inline-size:16rem;">
      <lr-result-field label="Model" value="gpt-5.4"></lr-result-field>
      <lr-result-field label="Tokens" value="1,024 / 8,192"></lr-result-field>
      <lr-result-field value="No label, value only"></lr-result-field>
    </div>
  `,
};
...
export const AsToolResultViewRenderer: Story = {
  name: 'As a <lr-tool-result-view> renderer',
  render: () => html`
    <lr-tool-result-view
