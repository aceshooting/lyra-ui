...
it("prunes stale measuredHeights entries once items changes to a wholly different set of keys", async () => {
  ...
  const measuredHeights = (
    el as unknown as { measuredHeights: Map<string | number, number> }
  ).measuredHeights;
  ...
  expect(measuredHeights.has("a"), 'stale key "a" should have been pruned').to
    .be.false;
  expect(measuredHeights.has("b"), 'stale key "b" should have been pruned').to
    .be.false;
  expect(measuredHeights.has("c"), 'stale key "c" should have been pruned').to
    .be.false;
});
