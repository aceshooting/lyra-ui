...
it('renders a disabled button when no href is available', async () => {
  const el = (await fixture(html`<lr-app-rail-item disabled>Settings</lr-app-rail-item>`)) as LyraAppRailItem;
...
it('hardens links opened in a new browsing context', async () => {
  const el = (await fixture(html`<lr-app-rail-item href="https://example.test" target="_blank">Open</lr-app-rail-item>`)) as LyraAppRailItem;
...
it('marks the base part aria-current="page" when active', async () => {
  const el = (await fixture(html`<lr-app-rail-item href="/home" active>Home</lr-app-rail-item>`)) as LyraAppRailItem;
  const base = el.shadowRoot!.querySelector('[part="base"]')!;
  expect(base.getAttribute('aria-current')).to.equal('page');
});

it('omits aria-current when not active', async () => {
  const el = (await fixture(html`<lr-app-rail-item href="/home">Home</lr-app-rail-item>`)) as LyraAppRailItem;
  const base = el.shadowRoot!.querySelector('[part="base"]')!;
...
describe('active', () => {
  it('reflects aria-current="page" onto [part=base] when true', async () => {
    const el = (await fixture(html`<lr-app-rail-item href="/inbox" active>Inbox</lr-app-rail-item>`)) as LyraAppRailItem;
    const base = el.shadowRoot!.querySelector('[part="base"]')!;
    expect(base.getAttribute('aria-current')).to.equal('page');
  });

  it('defaults to false and omits aria-current entirely', async () => {
    const el = (await fixture(html`<lr-app-rail-item href="/inbox">Inbox</lr-app-rail-item>`)) as LyraAppRailItem;
  ...
  it('reflects on the button-rendering path too (no href)', async () => {
    const el = (await fixture(html`<lr-app-rail-item active>Settings</lr-app-rail-item>`)) as LyraAppRailItem;
    const base = el.shadowRoot!.querySelector('[part="base"]')!;
...
describe('tooltip', () => {
  it('shows a flyout with the label text on hover/focus when tooltip is set and icon-only is active', async () => {
    const el = (await fixture(html`<lr-app-rail-item tooltip icon-only>Dashboard</lr-app-rail-item>`)) as LyraAppRailItem;
  ...
  it('does not show a flyout when tooltip is unset (the default)', async () => {
    const el = (await fixture(html`<lr-app-rail-item icon-only>Dashboard</lr-app-rail-item>`)) as LyraAppRailItem;
    const base = el.shadowRoot!.querySelector('[part="base"]') as HTMLElement;
    base.dispatchEvent(new FocusEvent('focus', { bubbles: true }));
    await el.updateComplete;
    expect(el.shadowRoot!.querySelector('[part="tooltip"]')).to.not.exist;
  ...
  it('does not show a flyout when tooltip is set but icon-only is not active (label is already visible)', async () => {
    const el = (await fixture(html`<lr-app-rail-item tooltip>Dashboard</lr-app-rail-item>`)) as LyraAppRailItem;
    const base = el.shadowRoot!.querySelector('[part="base"]') as HTMLElement;
    base.dispatchEvent(new FocusEvent('focus', { bubbles: true }));
    await el.updateComplete;
    expect(el.shadowRoot!.querySelector('[part="tooltip"]')).to.not.exist;
