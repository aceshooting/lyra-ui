...
it("resizes a resizable column through its native pointer handle and emits live widths", async () => {
  ...
  expect(
    (el.shadowRoot!.querySelector("col") as HTMLElement).style.inlineSize
  ).to.equal(`${detail!.width}px`);
});
...
it("uses the themed minimum width when a resizable column has no explicit minimum", async () => {
  ...
  window.dispatchEvent(
    new PointerEvent("pointermove", { pointerId: 2, clientX: -10000 })
  );

  expect(
    (
      el as unknown as { resizedColumnWidths: Map<string, number> }
    ).resizedColumnWidths.get("name")
  ).to.equal(90);
  window.dispatchEvent(
    new PointerEvent("pointerup", { pointerId: 2, clientX: -10000 })
  );
});

it("reflects spellcheck=false when assigned as a property", async () => {
  const el = (await fixture(
    html`<lr-table filterable></lr-table>`
  )) as LyraTable<Row>;
  el.spellcheck = false;
  await el.updateComplete;
...
it("uses groupLabel to render custom group header content", async () => {
  ...
  el.rows = rows;
  el.groupBy = (r) => (r.score > 2 ? "Passing" : "Needs review");
  el.groupLabel = (key, groupedRows) =>
    html`<strong>${key}:${groupedRows.length}</strong>`;
  await el.updateComplete;

  expect(
    el.shadowRoot!.querySelector('[part="group-cell"]')!.textContent
  ).to.contain("Passing:1");
});
...
it("forwards spellcheck/autocapitalize/autocorrect to a text cell editor but not a number one", async () => {
  ...
  const cells = [
    ...el.shadowRoot!.querySelectorAll('[part="row"] [part="cell"]'),
  ] as HTMLElement[];
  ...
  await el.updateComplete;
  const textInput = cells[0].querySelector(
    '[part="cell-editor"]'
  ) as HTMLInputElement;
  ...
  await el.updateComplete;
  const numberInput = cells[1].querySelector(
    '[part="cell-editor"]'
  ) as HTMLInputElement;
...
it("paginates client-side rows and emits controlled page requests", async () => {
  const el = (await fixture(
    html`<lr-table page-size="1"></lr-table>`
  )) as LyraTable<Row>;
  el.columns = columns;
  el.rows = rows;
  el.rowKey = (r) => r.id;
...
it("renders a localized busy state before rows while loading", async () => {
  const el = (await fixture(
    html`<lr-table loading></lr-table>`
  )) as LyraTable<Row>;
  el.columns = columns;
  el.rows = rows;
  await el.updateComplete;
...
it("emits lr-sort when a sortable header is clicked", async () => {
  ...
  el.rows = rows;
  await el.updateComplete;
  const header = el.shadowRoot!.querySelectorAll(
    '[part="header-cell"]'
  )[1] as HTMLElement;
...
it("flips the sort-icon rotation data-dir when sortDir changes from desc to asc", async () => {
  ...
  el.sortDir = "asc";
  await el.updateComplete;
  const scoreHeader = el.shadowRoot!.querySelectorAll(
    '[part="header-cell"]'
  )[1];
  const icon = scoreHeader.querySelector('[part="sort-icon"]');
...
it("applies the shared focus-ring outline to a sortable header cell, a row, and the more-button on :focus-visible", async () => {
  ...
  const moreButton = el.shadowRoot!.querySelector(
    '[part="more-button"]'
  ) as HTMLElement;
  moreButton.focus();
...
it('renders [part="reveal-columns-button"] only when at least one column declares a priority and a priority column is actually hidden', async () => {
  ...
  el.rows = rows;
  await el.updateComplete;
  expect(el.shadowRoot!.querySelector('[part="reveal-columns-button"]')).to.not
    .exist;
  ...
  await waitUntil(
    () =>
      el.shadowRoot!.querySelector('[part="reveal-columns-button"]') !== null
  );
  expect(el.shadowRoot!.querySelector('[part="reveal-columns-button"]')).to
    .exist;
});
...
it("hides low- and medium-priority columns in a narrow container, and reveals them via the toggle button", async () => {
  ...
  const lowHeader = el.shadowRoot!.querySelector(
    '[part="header-cell"][data-priority="low"]'
  ) as HTMLElement;
  const mediumHeader = el.shadowRoot!.querySelector(
    '[part="header-cell"][data-priority="medium"]'
  ) as HTMLElement;
  const lowCell = el.shadowRoot!.querySelector(
    '[part="cell"][data-priority="low"]'
  ) as HTMLElement;
...
it('renders [part="reveal-columns-button"], sets columnsHidden=true, and fires lr-columns-hidden-change once when a priority column is actually hidden by a narrow container', async () => {
  ...
  el.addEventListener("lr-columns-hidden-change", (e) =>
    events.push((e as CustomEvent).detail.hidden)
  );
  el.columns = priorityColumns;
  el.rows = rows;
  await el.updateComplete;
...
it("emits lr-columns-revealed with the new state whenever the reveal button is toggled", async () => {
  ...
  const events: boolean[] = [];
  el.addEventListener("lr-columns-revealed", (e) =>
    events.push((e as CustomEvent).detail.revealed)
  );
  const revealButton = el.shadowRoot!.querySelector(
    '[part="reveal-columns-button"]'
  ) as HTMLElement;
...
it("gives a sticky column's header and cell the sticky positioning attribute and styles", async () => {
  ...
  const stickyHeader = el.shadowRoot!.querySelector(
    '[part="header-cell"]'
  ) as HTMLElement;
  const stickyCell = el.shadowRoot!.querySelector(
    '[part="cell"]'
  ) as HTMLElement;
  const nonStickyHeader = el.shadowRoot!.querySelectorAll(
    '[part="header-cell"]'
  )[1] as HTMLElement;
...
it("pins a sticky: 'end' column's header and cell to the inline-end edge instead of inline-start", async () => {
  ...
  const stickyHeader = el.shadowRoot!.querySelectorAll(
    '[part="header-cell"]'
  )[1] as HTMLElement;
  const stickyCell = el.shadowRoot!.querySelectorAll(
    '[part="cell"]'
  )[1] as HTMLElement;
...
it('sets aria-selected="true" only on the row matching selectedKey', async () => {
  ...
  const [firstRow, secondRow] = [
    ...el.shadowRoot!.querySelectorAll('[part="row"]'),
  ];
...
it('sets data-align="end" on the header cell and body cell for an end-aligned column, and "start" otherwise', async () => {
  ...
  const firstRowCells = el
    .shadowRoot!.querySelectorAll('[part="row"]')[0]
    .querySelectorAll('[part="cell"]');
...
it("does not emit lr-sort when a non-sortable header is clicked or activated via keyboard", async () => {
  ...
  const nameHeader = el.shadowRoot!.querySelectorAll(
    '[part="header-cell"]'
  )[0] as HTMLElement;
  nameHeader.click();
...
it("supports Home/End row navigation and ignores unknown keyboard commands", async () => {
  ...
  secondRow.dispatchEvent(
    new KeyboardEvent("keydown", { key: "Unrelated", bubbles: true })
  );
  await el.updateComplete;
...
it("rehomes the active column when its priority-hidden header is no longer visible", async () => {
  ...
  expect(
    (el as unknown as { activeColKey: string | null }).activeColKey
  ).to.equal("name");
});
...
it("stops observing removed sticky headers when sticky columns are replaced", async () => {
  ...
  el.columns = columns;
  await el.updateComplete;
  expect(el.shadowRoot!.querySelectorAll("th[data-col-key]").length).to.equal(
    2
  );
});
...
it("keeps a numeric-key row and a string-key row distinct instead of colliding", async () => {
  ...
  const keys = new Set(
    Array.from(rowEls).map((r) => r.getAttribute("data-row-key"))
  );
...
describe("cellStyle column hook", () => {
  ...
  it("coexists with sticky-column offset styling without clobbering it", async () => {
    ...
    expect(
      cell.style.getPropertyValue("--lr-table-sticky-offset")
    ).to.not.equal("");
  });
});
...
describe("headerCell", () => {
  it("renders col.label by default when headerCell is unset", async () => {
    ...
    const el = (await fixture(
      html`<lr-table .columns=${columns} .rows=${[{ id: 1 }]}></lr-table>`
    )) as LyraTable;
    ...
    const columns: TableColumn<{ id: number }>[] = [
      {
        ...
        label: "ID",
        headerCell: (col) =>
          html`<strong class="custom">${col.label}!</strong>`,
        cell: (row) => row.id,
      },
    ];
    const el = (await fixture(
...
describe("column width", () => {
  ...
  it("sets table-layout: fixed and applies <col> widths when a column defines width", async () => {
    ...
    expect(
      (cols[0] as HTMLElement).style.getPropertyValue("inline-size")
    ).to.equal("120px");
  });
});
...
describe("expandable rows", () => {
  ...
  it("renders no leading toggle cell when expandedContent is unset (unchanged default)", async () => {
    ...
    el.rows = rows;
    await el.updateComplete;
    expect(el.shadowRoot!.querySelector('[part="expand-toggle-cell"]')).to.not
      .exist;
    expect(el.shadowRoot!.querySelector("[data-row-expand-toggle]")).to.not
      .exist;
  });
  ...
  it("renders a leading toggle cell on the header and every row when expandedContent is set", async () => {
    ...
    const toggleCells = el.shadowRoot!.querySelectorAll(
  ...
  it("renders an empty, non-interactive toggle cell for a row that fails canExpand", async () => {
    ...
    await el.updateComplete;
    const toggleCells = [
      ...el.shadowRoot!.querySelectorAll('[part="expand-toggle-cell"]'),
    ];
  ...
  it("emits lr-row-expand-toggle with { row, key } when the chevron button is clicked, and does not also emit lr-row-click", async () => {
    ...
    const firstToggleButton = el.shadowRoot!.querySelector(
      '[part="expand-toggle-cell"] button'
    ) as HTMLButtonElement;
  ...
  it("still emits lr-row-click when clicking elsewhere in an expandable row", async () => {
    ...
    const nameCell = el.shadowRoot!.querySelector(
      '[part="cell"]'
    ) as HTMLElement;
  ...
  it("renders the expanded panel row with the correct colspan when a row key is in expandedKeys", async () => {
    ...
    expect(expandedCell.querySelector(".panel")!.textContent).to.equal(
      "Alpha details"
    );

    // Only one row is in expandedKeys — only one expanded-row renders.
    expect(
      el.shadowRoot!.querySelectorAll('[part="expanded-row"]').length
    ).to.equal(1);
  });
  ...
  it("grows a matching leading spacer cell in the footer row when combined with a footer column, keeping real footer cells aligned", async () => {
    ...
    expect(foot).to.exist;
    const footerCells = [
      ...foot!.querySelectorAll('[part="footer-cell"]'),
    ] as HTMLElement[];
...
describe("localization", () => {
  ...
  it("localizes the loading spinner label", async () => {
    ...
    expect(spinner.getAttribute("accessible-label")).to.equal(
  ...
  it("localizes the no-data empty-state heading (both the whole-table and filtered-to-empty variants)", async () => {
    ...
    expect(
      whole.shadowRoot!.querySelector("lr-empty")!.getAttribute("heading")
    ).to.equal("Aucune donnée");

    const filtered = (await fixture(
      html`<lr-table
        filterable
        .strings=${{ noData: "Aucune correspondance" }}
      ></lr-table>`
    )) as LyraTable<Row>;
    ...
    await filtered.updateComplete;
    expect(
      filtered.shadowRoot!.querySelector("lr-empty")!.getAttribute("heading")
    ).to.equal("Aucune correspondance");
  });
  ...
  it("localizes the row expand/collapse toggle aria-label", async () => {
    ...
    const el = (await fixture(
      html`<lr-table
        .strings=${{ expand: "Développer", collapse: "Réduire" }}
      ></lr-table>`
    )) as LyraTable<Row>;
    el.columns = expandableColumns;
    el.rows = rows;
  ...
  it("localizes the inline cell editor aria-label, interpolating the column label", async () => {
    const el = (await fixture(
      html`<lr-table
        .strings=${{ tableEditCell: "Modifier {column}" }}
      ></lr-table>`
    )) as LyraTable<Row>;
    el.columns = editableColumns;
    el.rows = rows;
  ...
  it("localizes the load-more button label", async () => {
    ...
    await el.updateComplete;
    expect(
      el.shadowRoot!.querySelector('[part="more-button"]')!.textContent
    ).to.contain("Charger plus");
  });
});
...
describe("heat-tint mode", () => {
  ...
  it("computes --lr-table-heat-t from the auto-derived min/max across all heatValue columns", async () => {
    ...
    await el.updateComplete;
    const scoreCells = [
      ...el.shadowRoot!.querySelectorAll('[part="cell"][data-col-key="score"]'),
    ] as HTMLElement[];
    expect(scoreCells.length).to.equal(2);
    expect(scoreCells[0].style.getPropertyValue("--lr-table-heat-t")).to.equal(
      "100.00%"
    ); // Alpha: (3-1)/(3-1)
    expect(scoreCells[1].style.getPropertyValue("--lr-table-heat-t")).to.equal(
      "0.00%"
    ); // Beta: (1-1)/2
    expect(scoreCells.every((c) => c.hasAttribute("data-heat"))).to.be.true;
    const nameCells = [
      ...el.shadowRoot!.querySelectorAll('[part="cell"][data-col-key="name"]'),
    ] as HTMLElement[];
...
describe("rowTotal / grandTotal", () => {
  ...
  it("renders no trailing column when rowTotal is unset (unchanged default)", async () => {
    ...
    el.rowKey = (r) => r.id;
    await el.updateComplete;
    expect(el.shadowRoot!.querySelector('[part="row-total-cell"]')).to.not
      .exist;
  ...
  it("renders a trailing row-total cell on the header and every row when rowTotal is set", async () => {
    ...
    const cells = [
      ...el.shadowRoot!.querySelectorAll('[part="row-total-cell"]'),
    ];
  ...
  it("extends the expanded-row and group-row colspan to include the new trailing column", async () => {
    ...
    el.expandedKeys = new Set(["a"]);
    await el.updateComplete;
    const expandedCell = el.shadowRoot!.querySelector(
      '[part="expanded-cell"]'
    ) as HTMLElement;
...
describe("matching-entries memoization", () => {
  it("does not re-run row filtering for an unrelated reactive update (roving focus move)", async () => {
    ...
    expect(el.shadowRoot!.activeElement?.getAttribute("data-row-key")).to.equal(
      "string:b"
    );
...
describe("sticky-offset observation across reconnect", () => {
  it("keeps tracking a header resize after the table is detached and re-attached", async () => {
    ...
    const initialOffset = headers()[1].style.getPropertyValue(
    ...
    await waitUntil(
      () => {
        const offset = headers()[1].style.getPropertyValue(
          "--lr-table-sticky-offset"
        );
