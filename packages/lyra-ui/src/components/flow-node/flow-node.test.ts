...
it('renders the heading text and reflects selected/status', async () => {
  const el = (await fixture(html`<lyra-flow-node heading="Fetch data" status="running" selected></lyra-flow-node>`)) as LyraFlowNode;
...
it('shows a visible status chip with text (never color-only) for each status', async () => {
  const el = (await fixture(html`<lyra-flow-node status="error"></lyra-flow-node>`)) as LyraFlowNode;
...
it('renders one handle per input/output with data-handle-id/data-handle-kind and a native title', async () => {
  const el = (await fixture(
    html`<lyra-flow-node .inputs=${[{ id: 'a' }, { id: 'b', label: 'Second input' }]} .outputs=${[{ id: 'out' }]}></lyra-flow-node>`,
  )) as LyraFlowNode;
...
it('renders icon/header/toolbar slots and the default body slot', async () => {
  const el = (await fixture(
    html`<lyra-flow-node
      ><span slot="icon">i</span><span slot="toolbar">t</span>body</lyra-flow-node
    >`,
  )) as LyraFlowNode;
...
it('is accessible with a status, progress, and handles', async () => {
  const el = (await fixture(
    html`<lyra-flow-node heading="Fetch" status="running" progress="40"></lyra-flow-node>`,
  )) as LyraFlowNode;
...
describe('duration composition', () => {
  it('composes flowStatusWithDuration when both status and durationMs are set', async () => {
    const el = (await fixture(html`<lyra-flow-node status="success" duration-ms="812"></lyra-flow-node>`)) as LyraFlowNode;
  ...
  it('formats a duration at or above 1000ms in seconds', async () => {
    const el = (await fixture(html`<lyra-flow-node status="running" duration-ms="2500"></lyra-flow-node>`)) as LyraFlowNode;
  ...
  it('falls back to the plain status label when durationMs is unset', async () => {
    const el = (await fixture(html`<lyra-flow-node status="pending"></lyra-flow-node>`)) as LyraFlowNode;
