...
const meta: Meta = {
  title: 'Conversation/Realtime Session',
  component: 'lr-realtime-session',
};
export default meta;
type Story = StoryObj;

/** The composed capture keeps its seven `lr-record-*`/`lr-level` events intact;
  ...
  render: () => html`<lr-realtime-session
    ...
    .entries=${[
      { id: '1', speaker: 'You', text: 'Summarize the evidence.' },
      {
        id: '2',
        speaker: 'Assistant',
        text: 'The sources agree on three findings.',
      },
    ]}
  ></lr-realtime-session>`,
};

/** The shell renders one localized, provider-neutral connection failure. */
export const ErrorState: Story = {
  render: () => html`<lr-realtime-session state="error"></lr-realtime-session>`,
};
