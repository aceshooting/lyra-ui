...
export const RightToLeftCodeStream: Story = {
  ...
  render: () => {
    ...
    return html`<div dir="rtl" style="display:grid; gap:0.75rem; inline-size:400px; max-inline-size:100%;">
      <lr-streaming-text content-mode="markdown" coalesce-ms="50"></lr-streaming-text>
      <div><button type="button" @click=${(event: Event) => {
        const el = (event.currentTarget as HTMLElement).closest('div[dir]')!.querySelector('lr-streaming-text') as HTMLElement & { content: string; streaming: boolean };
        el.content = '';
        el.streaming = true;
        let index = 0;
        const timer = setInterval(() => {
...
export const ForwardedTaskListParts: Story = {
  ...
  render: () => html`
    ...
    <lr-streaming-text class="forwarded-task-parts" content-mode="markdown"
      .content=${'- [x] Draft the answer\n- [ ] Cite the sources\n\n| Step | Owner |\n| --- | --- |\n| Review | Platform team |'}></lr-streaming-text>
  `,
};
