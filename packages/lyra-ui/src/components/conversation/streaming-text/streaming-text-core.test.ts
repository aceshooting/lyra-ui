...
describe('lr-streaming-text-core forwarded Markdown parts and code direction', () => {
  const innerOf = (el: LyraStreamingTextCore): HTMLElement => el.shadowRoot!.querySelector<HTMLElement>('lr-markdown-core')!;
  const innerRoot = (el: LyraStreamingTextCore): ShadowRoot => innerOf(el).shadowRoot!;

  async function mount(content: string, dir: 'ltr' | 'rtl' = 'ltr', mode = 'markdown'): Promise<LyraStreamingTextCore> {
    const host = await fixture<HTMLElement>(html`<div dir=${dir} style="inline-size: 400px"></div>`);
    const el = document.createElement('lr-streaming-text-core') as LyraStreamingTextCore;
  ...
  it('lets outer ::part() rules reach forwarded task-list and table-wrapper parts', async () => {
    ...
    style.textContent = `
      lr-streaming-text-core::part(task-list) { outline-style: dashed }
      lr-streaming-text-core::part(task-item) { outline-style: dotted }
      lr-streaming-text-core::part(task-item-checked) { text-decoration-line: line-through }
      lr-streaming-text-core::part(task-checkbox) { outline-style: solid }
      lr-streaming-text-core::part(table-wrapper) { outline-style: double }
    `;
  ...
  it('segments the streamed fallback and keeps settled code left-to-right under RTL', async () => {
    ...
    const style = document.createElement('style');
    style.textContent = 'lr-streaming-text-core::part(code-block), lr-streaming-text-core::part(inline-code) { outline-style: dotted }';
    document.head.append(style);
    try {
