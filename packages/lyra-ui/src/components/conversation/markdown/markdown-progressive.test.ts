...
it('recovers progressive output for both tags when the first optional-peer load settles', async () => {
  ...
  for (const root of roots) {
    ...
    const tail = root.querySelector<HTMLElement>('.streaming-tail');
...
it('keeps an open fence literal and LTR without highlight or chrome until its closing fence arrives', async () => {
  ...
  for (const tag of ['lr-markdown', 'lr-markdown-core'] as const) {
    ...
    await el.updateComplete;
    const root = await contentRoot(el);
    const open = root.querySelector<HTMLElement>('[part="code-block"][data-open-fence="true"]');
    expect(Boolean(open)).to.equal(true);
    expect(open?.getAttribute('dir')).to.equal('ltr');
    ...
    await el.updateComplete;
    await waitUntil(
      () => el.shadowRoot!.querySelector('[part="code-block"]:not([data-open-fence])') !== null,
      'closed streaming fence did not commit as a Markdown code block',
    );
    expect(el.shadowRoot!.querySelector('[part="code-block"][data-open-fence="true"]') === null).to.equal(true);
  }
});
...
it('keeps aria-busy during streaming and emits composed settle events after tail and completion updates', async () => {
  ...
  for (const tag of ['lr-markdown', 'lr-markdown-core'] as const) {
    ...
    expect(el.shadowRoot!.querySelector('.streaming-tail')?.textContent).to.equal('current tail extended');
    expect(el.getAttribute('aria-busy')).to.equal('true');
...
it('keeps settled nodes, selection, and repeated heading ids stable through async Shiki refresh', async function () {
  ...
  for (const tag of ['lr-markdown', 'lr-markdown-core'] as const) {
    ...
    expect(root.querySelectorAll('[part="code-block"] span').length).to.equal(0);
...
it('rebuilds the progressive buffer when public renderMarkdown refreshes the same source', async () => {
  await preloadMarkdown();
  ...
  for (const tag of ['lr-markdown', 'lr-markdown-core'] as const) {
    ...
    await el.updateComplete;
    await waitUntil(
      () => el.shadowRoot!.querySelector('[part="content"] strong')?.textContent === 'configured parser',
      `${tag} did not rebuild the progressive stream from its same source`,
    );
...
it('forwards progressive mode through both streaming-text variants and message text/reasoning parts', async () => {
  ...
  for (const [tag, markdownTag] of [
    ...
    expect(nested.hasAttribute('streaming-render')).to.equal(true);
    await waitUntil(
