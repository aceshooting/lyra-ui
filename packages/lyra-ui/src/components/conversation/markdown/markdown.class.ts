...
export class LyraMarkdown extends DocumentAnchorTarget(LyraMarkdownBase) {
  ...
  protected override willUpdate(changed: PropertyValues): void {
    ...
    if (
      ...
      if (changed.has('highlightCode') || changed.has('languages') || changed.has('languagesOnly')) {
        this.highlightToken++;
        this.failedHighlightKeys.clear();
        if (changed.has('languages') || changed.has('languagesOnly')) this.highlightCache.clear();
      }
