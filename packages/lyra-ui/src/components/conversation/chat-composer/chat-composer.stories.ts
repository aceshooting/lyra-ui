...
export const PlainInsidePanel: Story = {
  name: 'frame="plain" (docked in a bordered panel)',
  render: () => html`
    <div
    ...
    >
      ...
      <lr-chat-composer frame="plain" placeholder="Message the assistant…"></lr-chat-composer>
    </div>
  `,
  parameters: {
    docs: {
      description: {
        story:
          'With `frame="plain"` the composer drops its own border/background so it doesn\'t double the frame of the chat panel, dialog footer or toolbar it is docked in. Focus stays visible: with no border left to recolor, the input row underlines itself instead (click into the textarea to see it).',
      },
    },
  },
};
