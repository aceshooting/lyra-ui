...
export class LyraChatViewport extends LyraElement<LyraChatViewportEventMap> {
  ...
  /** Accessible name for the log region. Omitting it localizes the default `chatViewportLabel`
   *  message; an explicit empty string (`label=""` or `.label = ''`) suppresses that default and
   *  renders no label. A host `aria-label` (see `accessibleLabel`) wins over both. */
  ...
  override render(): TemplateResult {
    const label =
      ...
      (this.label == null ? this.localize('chatViewportLabel') : this.label);
