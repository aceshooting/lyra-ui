...
it("renders optional line numbers for plain code without changing the copied source", async () => {
  ...
  expect(
    [...el.shadowRoot!.querySelectorAll(".line-source")]
...
it("shows filename and language as visible header text when set", async () => {
  ...
  expect(
    el.shadowRoot!.querySelector('[part="language"]')!.textContent!.trim()
  ).to.equal("typescript");
});
...
describe("shiki highlighting (real peer)", () => {
  it("shows a loading skeleton and aria-busy while shiki loads for a set language, then swaps to highlighted output", async function () {
    ...
    expect(
      coldLoad.ariaBusy,
      "aria-busy while the shared shiki load is pending"
    ).to.equal("true");
    expect(
      coldLoad.bodyRole,
      "the loading code surface remains a named group"
    ).to.equal("group");
    expect(
      coldLoad.bodyLabel,
      "the loading code surface has an accessible name"
    ).to.be.a("string").and.not.be.empty;
    expect(
      coldLoad.hasSkeleton,
      "skeleton while the shared shiki load is pending"
    ).to.be.true;
    expect(
      coldLoad.skeletonAnnounces,
      "the nested loading placeholder is decorative"
    ).to.be.false;
    expect(
      coldLoad.activeLiveRegions,
      "the loading state does not create a nested live region"
    ).to.equal(0);
    expect(
      coldLoad.hasHighlighted,
      "no highlighted output before the load resolves"
    ).to.be.false;
  ...
  it("does not set highlighter/shikiReady when the element disconnects before loadShikiHighlighter() resolves", async () => {
    ...
    expect(
      internalsOf(el).highlighter,
      "must not be set on a disconnected instance"
    ).to.equal(undefined);
  });
  ...
  it("re-highlights when code changes for an already-loaded language", async () => {
    ...
    ).to.contain("totallyDifferentIdentifier");
  });
  ...
  it("lazily loads a second language on demand rather than bundling every grammar up front", async () => {
    ...
    expect(internalsOf(el).highlighter!.getLoadedLanguages()).to.include(
      "python"
    );
  });
  ...
  it("highlights GreyCat source through the built-in gcl grammar", async () => {
    const el = (await fixture(
      html`<lr-code-block
        language="greycat"
        .code=${'fn greet(name: String) { return "Hi " + name }'}
      ></lr-code-block>`
    )) as LyraCodeBlock;
    await waitUntil(
      () => el.shadowRoot!.querySelector(".shiki") !== null,
  ...
  it("does not let a superseded async grammar load overwrite a later synchronous highlight", async () => {
    ...
    await waitUntil(
      () => hl.getLoadedLanguages().includes("rust"),
      "rust grammar never finished loading",
      {
        timeout: 8000,
      }
    );
...
describe("loader route", () => {
  it("uses the default loader unconditionally; the core tag is the explicit-language lean route", async () => {
    const el = (await fixture(
      html`<lr-code-block language="bash" .code=${"echo hi"}></lr-code-block>`
    )) as LyraCodeBlock;
    await waitUntil(
      () => internalsOf(el).shikiReady,
      "shiki never finished loading",
    ...
    expect("languagesOnly" in el).to.be.false;
  });
});
...
describe("fallback matrix (reaching into private state to exercise both paths deterministically)", () => {
  it("falls back to plain text when the shiki peer failed to load, even though language is set", async () => {
    ...
    // activatable-lines addressing), so a bare `[part="pre"] span` selector no longer
    ...
    expect(
      el.shadowRoot!.querySelector('[part="pre"] .line-source span') == null
    ).to.be.true;
    expect(el.shadowRoot!.querySelector('[part="code"]')!.textContent).to.equal(
      jsSample
    );
  ...
  it("falls back to plain text for an unrecognized language even though the highlighter loaded fine", async () => {
    const el = (await fixture(
      html`<lr-code-block
        language="not-a-real-language-xyz"
        .code=${jsSample}
      ></lr-code-block>`
    )) as LyraCodeBlock;
    await waitUntil(
...
describe("copy button", () => {
  it("fires lr-copy with the raw code (not highlighted HTML) and writes it to the clipboard", async () => {
    ...
    try {
      ...
      expect(detail).to.deep.equal({ ok: true, text: jsSample });
  ...
  it("fires lr-copy-error, not lr-copy, when navigator.clipboard is unavailable", async () => {
    ...
    try {
      ...
      expect(button.getAttribute("aria-label")).to.equal("Copy failed");
    } finally {
      if (original) Object.defineProperty(navigator, "clipboard", original);
      else Reflect.deleteProperty(navigator, "clipboard");
    }
  ...
  it("classifies a rejected permission write and localizes its visible failure state", async () => {
    const original = Object.getOwnPropertyDescriptor(navigator, "clipboard");
    const denied = new DOMException("permission denied", "NotAllowedError");
    Object.defineProperty(navigator, "clipboard", {
      configurable: true,
      value: { writeText: () => Promise.reject(denied) },
    });
    ...
    try {
      ...
      await el.updateComplete;
      expect(button.textContent!.trim()).to.equal("Could not copy");
      expect(button.getAttribute("aria-label")).to.equal("Could not copy");
    } finally {
      if (original) Object.defineProperty(navigator, "clipboard", original);
      else Reflect.deleteProperty(navigator, "clipboard");
    }
...
describe("collapsible / collapsed", () => {
  ...
  it("gives every instance a collision-safe body id", async () => {
    const a = (await fixture(
      html`<lr-code-block collapsible></lr-code-block>`
    )) as LyraCodeBlock;
...
it("honors an inherited max-height hook without the max-height attribute", async () => {
  ...
  expect(getComputedStyle(body).maxBlockSize).to.equal("41px");
});
  ...
  it("localizes the code-region aria-label, with and without a language", async () => {
    const withLang = (await fixture(
      html`<lr-code-block
        language="typescript"
        .code=${jsSample}
        .strings=${{ codeRegionWithLanguage: "{language} — code" }}
      ></lr-code-block>`
    )) as LyraCodeBlock;
    expect(
      (
        withLang.shadowRoot!.querySelector('[part="body"]') as HTMLElement
      ).getAttribute("aria-label")
    ).to.equal("typescript — code");

    const withoutLang = (await fixture(
      html`<lr-code-block
        .code=${jsSample}
        .strings=${{ codeRegion: "Extrait de code" }}
      ></lr-code-block>`
    )) as LyraCodeBlock;
    expect(
      (
        withoutLang.shadowRoot!.querySelector('[part="body"]') as HTMLElement
      ).getAttribute("aria-label")
    ).to.equal("Extrait de code");
  });
});
...
describe("activatable-lines", () => {
  ...
  it("names a blank gutter with effective-locale digits and keeps source separate", async () => {
    const wrapper = (await fixture(html`
      <div lang="ar-EG">
        <lr-code-block
          code=${"alpha\n\nomega"}
    ...
    expect(gutter.contains(source)).to.be.false;
  });
  ...
  it("does not restore a line after focus deliberately moves outside during a code update", async () => {
    const wrapper = await fixture(html`
      <div>
        <lr-code-block
          ...
          activatable-lines
        ></lr-code-block>
        <button id="outside-code-block">Outside</button>
      </div>
    `);
    const el = wrapper.querySelector("lr-code-block") as LyraCodeBlock;
    ...
    el.code = "a\nb";
    wrapper.querySelector<HTMLButtonElement>("#outside-code-block")!.focus();
    await el.updateComplete;
    expect(document.activeElement?.id).to.equal("outside-code-block");
  });

  it("does not emit lr-line-activate while activatable-lines is off", async () => {
    const el = (await fixture(
      html`<lr-code-block code=${"a\nb"} line-numbers></lr-code-block>`
    )) as LyraCodeBlock;
    await el.updateComplete;
    expect(
  ...
  it("keeps a named gutter control separate from selectable Shiki source text", async () => {
    const el = (await fixture(html`
      <lr-code-block
        language="json"
        .languages=${{ json: jsonGrammar }}
    ...
    `)) as LyraCodeBlock;
    await waitUntil(
      () => !!el.shadowRoot!.querySelector('.shiki [part~="line-button"]'),
      "interactive shiki gutters missing",
      {
        timeout: 8000,
      }
    );
    const line = el.shadowRoot!.querySelector(
      '.shiki .line[data-line="1"]'
    ) as HTMLElement;
    const gutter = line.querySelector(
      '[part~="line-button"]'
    ) as HTMLButtonElement;
    expect(gutter.getAttribute("aria-label")).to.equal("Line 1");
    ...
    let activations = 0;
    el.addEventListener("lr-line-activate", () => activations++);
    const source = line.querySelector(".line-source") as HTMLElement;
    const range = document.createRange();
    range.selectNodeContents(source);
    expect(range.toString()).to.contain('"a"');
    source.dispatchEvent(
      new MouseEvent("click", { bubbles: true, composed: true })
    );
    expect(
      activations,
      "clicking selectable source must not activate its gutter"
    ).to.equal(0);
    const event = oneEvent(el, "lr-line-activate");
    gutter.click();
    expect(
      ((await event) as CustomEvent<{ line: number }>).detail.line
    ).to.equal(1);
...
describe("gutter line-button hover specificity", () => {
  it("a ::part(line-button):hover override wins in rendered computed style", async () => {
    ...
    try {
      ...
      await waitUntil(() => getComputedStyle(gutter).color === "rgb(1, 2, 3)");
      expect(getComputedStyle(gutter).color).to.equal("rgb(1, 2, 3)");
    } finally {
      await resetMouse();
      style.remove();
    }
...
describe("tab width (--lr-code-block-tab-size)", () => {
  ...
  it("renders a tab-indented code string at the width the token asks for", async () => {
    const el = (await fixture(
      html`<lr-code-block
        .code=${"\tone\n\t\ttwo"}
        style="--lr-code-block-tab-size: 5"
      ></lr-code-block>`
    )) as LyraCodeBlock;
    await el.updateComplete;
  ...
  it("keeps honouring the token on the shiki-highlighted path, where <pre> carries an inline style", async () => {
    ...
    expect(
      ...
      "shiki writes its own inline style on this element"
    ).to.exist;
    expect(
      pre.style.tabSize,
      "the component must never write tab-size inline"
    ).to.equal("");
...
describe("active-line outline color (--lr-code-block-active-line-outline-color)", () => {
  ...
  it("defaults to --lr-color-brand, unchanged from before the override existed", async () => {
    ...
    expect(getComputedStyle(line).outlineColor).to.equal(
  ...
  it("retints the active-line outline without touching --lr-color-brand", async () => {
    const line = await activeLine(
      "--lr-code-block-active-line-outline-color: rgb(1, 2, 3)"
    );
...
describe("highlighted-line background color (--lr-code-block-highlighted-line-bg)", () => {
  ...
  it("defaults to --lr-color-warning-quiet, unchanged from before the override existed", async () => {
    ...
    line.parentElement!.appendChild(probe);
    expect(getComputedStyle(line).backgroundColor).to.equal(
      getComputedStyle(probe).color
    );
  });
  ...
  it("retints the highlighted-line background without touching --lr-color-warning-quiet", async () => {
    const line = await highlightedLine(
      "--lr-code-block-highlighted-line-bg: rgb(4, 5, 6)"
    );
...
describe("shiki dark-theme signal", () => {
  ...
  it('gates the shiki dark-theme override on [data-dark-theme="true"], not @media (prefers-color-scheme: dark) alone', () => {
    ...
    expect(css).to.match(
      /\[part='body'\]\[data-dark-theme='true'\] \[part='pre'\][^{,]*\{[^}]*--shiki-dark/
    );
    expect(css).to.not.match(
      /@media \(prefers-color-scheme: dark\)[^{]*\{[^}]*--shiki-dark/
    );
  });
  ...
  it('resolves a highlighted token span\'s computed color from --shiki-dark only once data-dark-theme="true", never merely from the OS dark-mode preference', async function () {
    ...
    async function firstDarkAwareSpan(
      ...
      const spans = Array.from(
        el.shadowRoot!.querySelectorAll('[part="pre"] span')
      ) as HTMLElement[];
      const span = spans.find(
        (s) => s.style.getPropertyValue("--shiki-dark") !== ""
      )!;
      return { span, body };
    }
...
it("rejects a max-height that would inject extra declarations onto the body", async () => {
  const el = (await fixture(html`
    <lr-code-block
      code="const x = 1;"
      max-height="1px;background-image:url(https://example.test/b.png)"
    ></lr-code-block>
  `)) as LyraCodeBlock;
  await el.updateComplete;
  const body = el.shadowRoot!.querySelector('[part="body"]') as HTMLElement;
  expect(
    getComputedStyle(body).backgroundImage,
    "no injected paint server"
  ).to.equal("none");
  expect(
    body.style.getPropertyValue("--lr-code-block-max-height").trim()
  ).to.equal("");
  ...
  expect(
    after.style.getPropertyValue("--lr-code-block-max-height").trim()
  ).to.equal("20rem");
});
...
it("re-arms fine-grained highlighting when a pending instance reconnects", async () => {
  let resolveCore!: (value: ShikiHighlighterCore | null) => void;
  const pending = new Promise<ShikiHighlighterCore | null>((resolve) => {
    resolveCore = resolve;
  });
...
describe("lr-code-block under a right-to-left document", () => {
  ...
  async function mount(dir: "rtl" | "ltr", filename = "./src/main.cpp", lineNumbers = false): Promise<HTMLElement> {
    const host = await fixture<HTMLElement>(html`<div dir=${dir} style="inline-size: 360px">
      <lr-code-block language="c++" filename=${filename} ?line-numbers=${lineNumbers} .code=${code}></lr-code-block>
    </div>`);
    const el = host.querySelector("lr-code-block") as HTMLElement & { updateComplete: Promise<boolean> };
    await el.updateComplete;
