...
export class LyraScroller extends LyraElement<LyraScrollerEventMap> {
  ...
  render(): TemplateResult {
    ...
    return html`<div part="base" role="region" aria-label=${label}>
      ${this.controls ? html`<button part="control previous" type="button" aria-label=${this.localize('scrollPrevious')} ?disabled=${this.canScrollStart} @click=${() => this.scrollByDirection(-1)} @dblclick=${() => this.scrollToEdge('start')}>${vertical ? '↑' : '‹'}</button>` : nothing}
      <div part="viewport" tabindex="0" @scroll=${this.onScroll}><div part="content"><slot @slotchange=${this.updateEdges}></slot></div></div>
      ${this.controls ? html`<button part="control next" type="button" aria-label=${this.localize('scrollNext')} ?disabled=${this.canScrollEnd} @click=${() => this.scrollByDirection(1)} @dblclick=${() => this.scrollToEdge('end')}>${vertical ? '↓' : '›'}</button>` : nothing}
    </div>`;
  }
}
