...
it('exports the FLOW_PALETTE_MIME_TYPE constant used by the drop/palette handshake', () => {
  expect(FLOW_PALETTE_MIME_TYPE).to.equal('application/lyra-flow-node');
});

it('renders lyra-empty with the noData message when nodes is empty', async () => {
  const el = (await fixture(html`<lyra-flow-canvas></lyra-flow-canvas>`)) as LyraFlowCanvas;
  const empty = el.shadowRoot!.querySelector('[part="empty"]');
  expect(empty).to.exist;
  expect(empty!.tagName.toLowerCase()).to.equal('lyra-empty');
...
describe('static rendering', () => {
  it('adopts a default lyra-flow-node card per node into slot="node-{id}"', async () => {
    const el = (await fixture(html`<lyra-flow-canvas></lyra-flow-canvas>`)) as LyraFlowCanvas;
    el.nodes = nodes;
    await el.updateComplete;
    const a = el.querySelector('[node-id="a"]') as HTMLElement;
    expect(a).to.exist;
    expect(a.tagName.toLowerCase()).to.equal('lyra-flow-node');
  ...
  it('routes a user-authored child into its wrapper by node-id instead of creating a default card', async () => {
    const el = (await fixture(
      html`<lyra-flow-canvas><div node-id="a">Custom</div></lyra-flow-canvas>`,
    )) as LyraFlowCanvas;
    el.nodes = nodes;
    await el.updateComplete;
  ...
  it('warns and leaves a stale user-authored child unslotted when its node-id matches no node', async () => {
    const el = (await fixture(
      html`<lyra-flow-canvas><div node-id="ghost">Gone</div></lyra-flow-canvas>`,
    )) as LyraFlowCanvas;
  ...
  it('reflects grid onto the background layer as --lyra-flow-canvas-grid-size', async () => {
    const el = (await fixture(html`<lyra-flow-canvas grid="16"></lyra-flow-canvas>`)) as LyraFlowCanvas;
    el.nodes = nodes;
    await el.updateComplete;
    const bg = el.shadowRoot!.querySelector('[part="background"]') as HTMLElement;
    expect(bg.style.getPropertyValue('--lyra-flow-canvas-grid-size')).to.equal('16px');
  });
});
...
describe('auto-layout', () => {
  ...
  it('assigns a position to every unpositioned node and fires lyra-layout-change with exactly those ids', async () => {
    const el = (await fixture(html`<lyra-flow-canvas></lyra-flow-canvas>`)) as LyraFlowCanvas;
  ...
  it('transposes the returned axes for orientation="vertical"', async () => {
    const horizontalEl = (await fixture(html`<lyra-flow-canvas></lyra-flow-canvas>`)) as LyraFlowCanvas;
    horizontalEl.nodes = [{ id: 'a' }, { id: 'b' }];
    horizontalEl.edges = [{ id: 'a-b', source: 'a', target: 'b' }];
    await horizontalEl.updateComplete;
    await new Promise((r) => requestAnimationFrame(() => requestAnimationFrame(r)));

    const verticalEl = (await fixture(html`<lyra-flow-canvas orientation="vertical"></lyra-flow-canvas>`)) as LyraFlowCanvas;
...
describe('pan & zoom', () => {
  it('zoomIn/zoomOut/resetZoom change viewport.zoom, clamped to [minZoom, maxZoom]', async () => {
    const el = (await fixture(html`<lyra-flow-canvas min-zoom="0.5" max-zoom="1.5"></lyra-flow-canvas>`)) as LyraFlowCanvas;
    el.nodes = nodes;
    await el.updateComplete;
  ...
  it('setViewport/viewport getter round-trip, clamping zoom but not x/y', async () => {
    const el = (await fixture(html`<lyra-flow-canvas max-zoom="2"></lyra-flow-canvas>`)) as LyraFlowCanvas;
    el.nodes = nodes;
    await el.updateComplete;
  ...
  it('emits lyra-viewport-change on setViewport', async () => {
    const el = (await fixture(html`<lyra-flow-canvas></lyra-flow-canvas>`)) as LyraFlowCanvas;
    el.nodes = nodes;
    await el.updateComplete;
    let detail: { x: number; y: number; zoom: number } | undefined;
    el.addEventListener('lyra-viewport-change', (e) => (detail = (e as CustomEvent).detail));
  ...
  it('fit() centers content and clamps zoom to the configured bounds', async () => {
    const el = (await fixture(
      html`<lyra-flow-canvas style="width:400px;height:300px" min-zoom="0.1" max-zoom="4"></lyra-flow-canvas>`,
    )) as LyraFlowCanvas;
    el.nodes = [
      { id: 'a', position: { x: 0, y: 0 } },
  ...
  it('wheel zooms toward the cursor and emits a throttled lyra-viewport-change', async () => {
    const el = (await fixture(html`<lyra-flow-canvas style="width:400px;height:300px"></lyra-flow-canvas>`)) as LyraFlowCanvas;
    el.nodes = nodes;
    await el.updateComplete;
    ...
    let changes = 0;
    el.addEventListener('lyra-viewport-change', () => changes++);
  ...
  it('locked disables wheel zoom, background pan, and the keyboard shortcuts', async () => {
    const el = (await fixture(html`<lyra-flow-canvas locked style="width:400px;height:300px"></lyra-flow-canvas>`)) as LyraFlowCanvas;
    el.nodes = nodes;
    await el.updateComplete;
    const viewportEl = el.shadowRoot!.querySelector('[part="viewport"]') as HTMLElement;
  ...
  it('mirrors the pan/zoom surface under orientation="horizontal" + dir="rtl"', async () => {
    const el = (await fixture(
      html`<div dir="rtl"><lyra-flow-canvas></lyra-flow-canvas></div>`,
    )) as HTMLElement;
    const canvas = el.querySelector('lyra-flow-canvas') as LyraFlowCanvas;
    canvas.nodes = nodes;
...
describe('selection & roving focus', () => {
  it('click on a node emits lyra-node-click and replaces selection with just that node', async () => {
    const el = (await fixture(html`<lyra-flow-canvas></lyra-flow-canvas>`)) as LyraFlowCanvas;
    el.nodes = nodes;
    el.edges = edges;
    ...
    el.addEventListener('lyra-selection-change', (e) => (selectionDetail = (e as CustomEvent).detail));
  ...
  it('click on an edge emits lyra-edge-click and selects it, clearing node selection', async () => {
    const el = (await fixture(html`<lyra-flow-canvas></lyra-flow-canvas>`)) as LyraFlowCanvas;
    el.nodes = nodes;
    el.edges = edges;
    ...
    el.addEventListener('lyra-edge-click', (e) => (detail = (e as CustomEvent).detail));
  ...
  it('Enter on a node toggles selection and emits lyra-node-click', async () => {
    const el = (await fixture(html`<lyra-flow-canvas></lyra-flow-canvas>`)) as LyraFlowCanvas;
    el.nodes = nodes;
    await el.updateComplete;
    let fired = false;
    el.addEventListener('lyra-node-click', () => (fired = true));
  ...
  it('Delete/Backspace emits lyra-selection-delete only while an editor flag is set and selection is non-empty', async () => {
    const el = (await fixture(html`<lyra-flow-canvas nodes-draggable></lyra-flow-canvas>`)) as LyraFlowCanvas;
    el.nodes = nodes;
    el.selectedNodeIds = ['a'];
    await el.updateComplete;
    let detail: { nodeIds: string[]; edgeIds: string[] } | undefined;
    el.addEventListener('lyra-selection-delete', (e) => (detail = (e as CustomEvent).detail));
    (el.shadowRoot!.querySelector('[part="node"]') as HTMLElement).dispatchEvent(
  ...
  it('does not emit lyra-selection-delete when no editor flag is set (pure viewer)', async () => {
    const el = (await fixture(html`<lyra-flow-canvas></lyra-flow-canvas>`)) as LyraFlowCanvas;
    el.nodes = nodes;
    el.selectedNodeIds = ['a'];
    await el.updateComplete;
    let fired = false;
    el.addEventListener('lyra-selection-delete', () => (fired = true));
    (el.shadowRoot!.querySelector('[part="node"]') as HTMLElement).dispatchEvent(
      new KeyboardEvent('keydown', { key: 'Delete', bubbles: true, cancelable: true }),
    );
...
describe('node drag', () => {
  it('pointer-drags a node (grid-snapped) and emits lyra-node-move on release', async () => {
    const el = (await fixture(html`<lyra-flow-canvas nodes-draggable></lyra-flow-canvas>`)) as LyraFlowCanvas;
    ...
    el.addEventListener('lyra-node-move', (e) => (detail = (e as CustomEvent).detail));
  ...
  it('does not drag when locked, even with nodes-draggable set', async () => {
    const el = (await fixture(html`<lyra-flow-canvas nodes-draggable locked></lyra-flow-canvas>`)) as LyraFlowCanvas;
    el.nodes = [{ id: 'a', position: { x: 0, y: 0 } }];
    await el.updateComplete;
    const wrapper = el.shadowRoot!.querySelector('[part="node"]') as HTMLElement;
    wrapper.setPointerCapture = () => {};
  ...
  it('Ctrl/Cmd+Arrow nudges the focused node by grid and emits lyra-node-move', async () => {
    const el = (await fixture(html`<lyra-flow-canvas nodes-draggable></lyra-flow-canvas>`)) as LyraFlowCanvas;
    el.nodes = [{ id: 'a', position: { x: 40, y: 40 } }];
    await el.updateComplete;
    const wrapper = el.shadowRoot!.querySelector('[part="node"]') as HTMLElement;
    ...
    // `lyra-node-move` (Slice A) and the sibling pointer-drag test just above both require `previous`
...
describe('connect gesture', () => {
  it('pointer-drags from an output handle to an input handle and emits lyra-connect', async () => {
    ...
    el.addEventListener('lyra-connect', (e) => (detail = (e as CustomEvent).detail));
  ...
  it('keyboard: "c" on a focused node starts connect mode, arrows cycle targets, Enter commits', async () => {
    ...
    // (Slice A: `'lyra-connect': CustomEvent<{ source; target; sourceHandle; targetHandle }>`), the
    // class JSDoc (`@event lyra-connect - detail: { source, target, sourceHandle, targetHandle }`),
...
// the `lyra-empty` state -- no `[part='viewport']` at all -- while `nodes` is empty, so a droppable
...
describe('droppable', () => {
  it('accepts a FLOW_PALETTE_MIME_TYPE drop and emits lyra-node-add with a grid-snapped position', async () => {
    const el = (await fixture(
  ...
  it('ignores a drop when droppable is unset or the canvas is locked', async () => {
    ...
    const viewportEl = el.shadowRoot!.querySelector('[part="viewport"]') as HTMLElement;
    let fired = false;
    el.addEventListener('lyra-node-add', () => (fired = true));
  ...
  it('marks data-drop-active on dragover with the recognized MIME type and clears it on dragleave/drop', async () => {
    const el = (await fixture(html`<lyra-flow-canvas droppable style="width:400px;height:300px"></lyra-flow-canvas>`)) as LyraFlowCanvas;
    el.nodes = [{ id: 'seed', position: { x: 0, y: 0 } }];
    await el.updateComplete;
    const viewportEl = el.shadowRoot!.querySelector('[part="viewport"]') as HTMLElement;
    const dataTransfer = new DataTransfer();
...
describe('locked (consolidated)', () => {
  it('leaves pan, zoom, drag, connect, and drop all inert while focus/click/keyboard-activation still work', async () => {
    const el = (await fixture(
      html`<lyra-flow-canvas locked nodes-draggable connectable droppable style="width:400px;height:300px"></lyra-flow-canvas>`,
    )) as LyraFlowCanvas;
    el.nodes = [
      { id: 'a', position: { x: 0, y: 0 } },
      { id: 'b', position: { x: 200, y: 0 } },
    ...
    wrapperA.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true, cancelable: true }));
    let connected = false;
    el.addEventListener('lyra-connect', () => (connected = true));
