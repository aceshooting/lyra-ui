...
describe('lyra-file-tree', () => {
  it('renders a row per file/directory, deepest-first order preserved', async () => {
    const el = (await fixture(html`<lyra-file-tree></lyra-file-tree>`)) as LyraFileTree;
    el.nodes = nodes;
    await el.updateComplete;
    const tree = el.shadowRoot!.querySelector('lyra-tree')!;
  ...
  it('emits lyra-file-select when a file row is activated', async () => {
    const el = (await fixture(html`<lyra-file-tree></lyra-file-tree>`)) as LyraFileTree;
    el.nodes = nodes;
    await el.updateComplete;
    const listener = oneEvent(el, 'lyra-file-select');
    el.shadowRoot!.querySelector('lyra-tree')!.dispatchEvent(
      new CustomEvent('lyra-node-select', { detail: { id: 'README.md' }, bubbles: true, composed: true }),
    );
    const event = (await listener) as CustomEvent<{ path: string }>;
    expect(event.detail.path).to.equal('README.md');
  });

  it('emits lyra-file-open on a second select of an already-selected file (keyboard-open parity)', async () => {
    const el = (await fixture(html`<lyra-file-tree></lyra-file-tree>`)) as LyraFileTree;
    el.nodes = nodes;
    el.selectedPath = 'README.md';
    await el.updateComplete;
    const listener = oneEvent(el, 'lyra-file-open');
    el.shadowRoot!.querySelector('lyra-tree')!.dispatchEvent(
      new CustomEvent('lyra-node-select', { detail: { id: 'README.md' }, bubbles: true, composed: true }),
    );
    const event = (await listener) as CustomEvent<{ path: string }>;
  ...
  it('emits lyra-load-children exactly once when a lazy (hasChildren, no children) directory expands', async () => {
    const el = (await fixture(html`<lyra-file-tree></lyra-file-tree>`)) as LyraFileTree;
    el.nodes = [{ path: 'lazy-dir', kind: 'directory', hasChildren: true }];
    await el.updateComplete;
    const listener = oneEvent(el, 'lyra-load-children');
    el.shadowRoot!.querySelector('lyra-tree')!.dispatchEvent(
      new CustomEvent('lyra-node-toggle', { detail: { id: 'lazy-dir', expanded: true }, bubbles: true, composed: true }),
    );
    const event = (await listener) as CustomEvent<{ path: string }>;
