...
it('renders an empty item collection as a disabled listbox option', async () => {
  const el = (await fixture(
    html`<lr-node-palette .items=${[]}></lr-node-palette>`
  )) as LyraNodePalette;
  const empty = el.shadowRoot!.querySelector('[part="empty"]');

  expect(empty?.getAttribute('role')).to.equal('option');
  expect(empty?.getAttribute('aria-selected')).to.equal('false');
...
it('drops empty and whitespace-only item labels while retaining a valid neighbor', async () => {
  const el = (await fixture(html`
    <lr-node-palette
      .items=${[
        { type: 'empty', label: '' },
        { type: 'blank', label: '   ' },
        { type: 'valid', label: 'Valid node' },
      ]}
    ></lr-node-palette>
  `)) as LyraNodePalette;
  await el.updateComplete;
  ...
  expect(rendered[0]!.textContent?.trim()).to.equal('Valid node');
  await expect(el).to.be.accessible();
});

it('treats a non-array items value as empty and omits malformed rows before a valid item', async () => {
  const el = await fixture<LyraNodePalette>(
    html`<lr-node-palette></lr-node-palette>`
  );

  (el as unknown as { items: unknown }).items = null;
  await el.updateComplete;
  ...
  (el as unknown as { items: unknown }).items = [
    null,
    { type: 'missing-label' },
    { type: 'bad-keywords', label: 'Bad', keywords: {} },
    { type: 'valid', label: 'Valid node' },
  ];
  await el.updateComplete;
  const rendered = [
...
it('focuses the search field as an interim fallback when the preserved item cannot be found among the previous elements, then the roving-position follow-up takes over', async () => {
  ...
  expect(inputFocused, 'the search field is focused as a fallback').to.be.true;
...
it("renders an item's optional description text", async () => {
  const el = (await fixture(
    html`<lr-node-palette
      .items=${[{ type: 'x', label: 'Node', description: 'Does a thing' }]}
    ></lr-node-palette>`
  )) as LyraNodePalette;
  await el.updateComplete;
  expect(
...
it("lets a consumer's ::part(item):hover override win under a real pointer", async () => {
  const wrapper = await fixture<HTMLElement>(html`
    <div>
      <style>
        lr-node-palette::part(item):hover { background: rgb(7, 8, 9); }
      </style>
      <lr-node-palette .items=${items}></lr-node-palette>
    </div>
  `);
