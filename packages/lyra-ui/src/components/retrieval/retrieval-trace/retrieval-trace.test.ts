  ...
  },
  {
    ...
    evidence: {
      chunks: [
        ...
        {
          id: "c2",
          text: "Rattlesnake Ledge offers sweeping views.",
          score: 0.84,
          source: { id: "s2", name: "trail-guide.pdf" },
        },
      ],
    },
  },
...
describe("lr-retrieval-trace", () => {
  ...
  it("maps each stage kind to its localized default name, with a per-stage label override winning", async () => {
    ...
    const names = [
      ...waterfall.shadowRoot!.querySelectorAll('[part="row"] [part="name"]'),
    ].map((n) => n.textContent);
    ...
    await el.updateComplete;
    const overridden = waterfall.shadowRoot!.querySelector(
      '[part="row"] [part="name"]'
    );
  ...
  it("keeps the host name distinct from the timeline's localized purpose", async () => {
    ...
    ).to.equal("Span timeline");
    expect(el.getAttribute("aria-label")).to.equal("Custom trace label");
  });

  it("keeps explicit-empty and dynamic host naming distinct from an explicit timeline label", async () => {
    const el = (await fixture(
      html`<lr-retrieval-trace
    ...
    expect(el.getAttribute("aria-label")).to.equal("Host trace label");
    ...
    expect(el.getAttribute("aria-label")).to.equal("Changed host label");
  ...
  it("forwards activeStageId to the internal waterfall as activeSpanId", async () => {
    ...
    const bar = waterfall.shadowRoot!.querySelector(
      '[data-id="retrieve"]'
    ) as HTMLElement;
  ...
  it("only stages with evidence grow an evidence row", async () => {
    ...
    await el.updateComplete;
    const rowIds = [
      ...el.shadowRoot!.querySelectorAll('[part="evidence-row"]'),
  ...
  it("renders no evidence-list at all when no stage has evidence", async () => {
    ...
    const el = (await fixture(
      html`<lr-retrieval-trace .stages=${bare}></lr-retrieval-trace>`
    )) as LyraRetrievalTrace;
    await el.updateComplete;
    expect(el.shadowRoot!.querySelector('[part="evidence-list"]') == null).to.be
      .true;
  });
  ...
  it("evidence rows start collapsed", async () => {
    ...
    const body = el.shadowRoot!.querySelector(
      '[data-id="retrieve"] [part="evidence-body"]'
    ) as HTMLElement;
  ...
  it("clicking the evidence-toggle button directly toggles aria-expanded/hidden and emits lr-stage-toggle both ways", async () => {
    ...
    await el.updateComplete;
    const toggle = el.shadowRoot!.querySelector(
      '[data-id="rewrite"] [part="evidence-toggle"]'
    ) as HTMLButtonElement;
    const body = el.shadowRoot!.querySelector(
      '[data-id="rewrite"] [part="evidence-body"]'
    ) as HTMLElement;
  ...
  it("renders free-form text evidence", async () => {
    ...
    const text = el.shadowRoot!.querySelector(
      '[data-id="rewrite"] [part="evidence-text"]'
    );
  ...
  it("renders chunk evidence via a compact lr-chunk-inspector, preserving source metadata and locators", async () => {
    ...
    let leaked = 0;
    el.addEventListener("lr-chunk-open", () => leaked++);
    const opened = oneEvent(el, "lr-stage-chunk-action");
    (
  ...
  it("renders metadata evidence as a key/value list", async () => {
    ...
    (
      el.shadowRoot!.querySelector(
        '[data-id="embed"] [part="evidence-toggle"]'
      ) as HTMLButtonElement
    ).click();
    await el.updateComplete;
    const keys = [
  ...
  it("prunes a stale expanded stage id when stages shrinks, instead of throwing or leaving it expanded (dangling-reference fixture)", async () => {
    ...
    await el.updateComplete;
    expect(el.shadowRoot!.querySelector('[data-id="retrieve"]') == null).to.be
      .true;
  ...
  it("falls back to built-in English stage labels and honors a strings override", async () => {
    ...
    expect(
      waterfall.shadowRoot!.querySelector('[part="row"] [part="name"]')!
        .textContent
    ).to.equal("Query rewrite");
    ...
    expect(
      waterfall.shadowRoot!.querySelector('[part="row"] [part="name"]')!
        .textContent
    ).to.equal("Reformulation de requête");
  });
  ...
  it("falls back to the built-in English evidence-toggle template and honors a strings override", async () => {
    ...
    await el.updateComplete;
    const toggled = el.shadowRoot!.querySelector(
      '[data-id="rewrite"] [part="evidence-toggle"]'
    )!;
  ...
  it('reverses the collapsed evidence-toggle chevron under dir="rtl"', async () => {
    ...
    const ltrIcon = ltr.shadowRoot!.querySelector(
      '[data-id="rewrite"] [part="evidence-toggle-icon"] svg'
    ) as SVGElement;
    ...
    const rtlIcon = rtl.shadowRoot!.querySelector(
      '[data-id="rewrite"] [part="evidence-toggle-icon"] svg'
    ) as SVGElement;
  ...
  it("renders lr-empty inside the timeline when stages is empty (via lr-span-waterfall)", async () => {
    const el = (await fixture(
      html`<lr-retrieval-trace></lr-retrieval-trace>`
    )) as LyraRetrievalTrace;
    await el.updateComplete;
    const waterfall = el.shadowRoot!.querySelector(
      "lr-span-waterfall"
  ...
  it("uses instance- and occurrence-safe evidence ids for hostile and duplicate stages", async () => {
    ...
    const traces = [
      ...wrapper.querySelectorAll("lr-retrieval-trace"),
    ] as LyraRetrievalTrace[];
    const firstControls = [
      ...traces[0]!.shadowRoot!.querySelectorAll('[part="evidence-toggle"]'),
    ].map((toggle) => toggle.getAttribute("aria-controls")!);
    const secondControls = traces[1]!
    ...
    for (const controls of firstControls) {
      ...
      expect(
        traces[0]!.shadowRoot!.getElementById(controls)?.getAttribute("part")
      ).to.equal("evidence-body");
    }
  });
  ...
  it("is accessible with stages, an expanded chunk evidence panel, and an expanded metadata panel", async () => {
    ...
    await el.updateComplete;
    expect(
      el.shadowRoot!.querySelector('[data-id="retrieve"] lr-chunk-inspector')
    ).to.exist;
    expect(
      el.shadowRoot!.querySelector(
...
describe("active-evidence cssprop escape hatch", () => {
  ...
  async function activeTrace(
    style = ""
  ): Promise<{ el: LyraRetrievalTrace; row: HTMLElement }> {
    const wrapper = (await fixture(
  ...
  it("recolors the active evidence-row border from an ancestor via --lr-retrieval-trace-active-border", async () => {
    const { row } = await activeTrace(
      "--lr-retrieval-trace-active-border: rgb(0, 51, 102)"
    );
