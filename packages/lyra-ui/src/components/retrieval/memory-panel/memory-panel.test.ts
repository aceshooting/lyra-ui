 ...
 */
async function readyConfirmBar(el: Element): Promise<LyraConfirmBar> {
  const confirmBar = el.querySelector('lr-confirm-bar')!;
...
describe('lr-memory-panel', () => {
  ...
  it('fails non-array runtime memory collections closed', async () => {
    const el = await fixture<LyraMemoryPanel>(
      html`<lr-memory-panel></lr-memory-panel>`
    );
    const invalid = {
      0: shortTermItems[0],
      length: 1,
    } as unknown as LyraMemoryItem[];
  ...
  it('omits memory rows with missing, null, or blank text before rendering actions', async () => {
    ...
    expect(rows[0]!.textContent).to.contain('Visible memory');
    expect(
      el.shadowRoot!.querySelectorAll('[part="add-button"]')
    ).to.have.lengthOf(1);
    expect(
      el.shadowRoot!.querySelectorAll('[part="remove-button"]')
    ).to.have.lengthOf(1);
