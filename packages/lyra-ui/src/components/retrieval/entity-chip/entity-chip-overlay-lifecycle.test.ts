...
it('registers the open popover with the shared overlay stack', async () => {
  const el = (await fixture(
    html`<lr-entity-chip text="Marie Curie">Physicist, 1867-1934</lr-entity-chip>`,
  )) as LyraEntityChip;
...
it('defers Escape to a newer dialog opened on top, instead of closing its own popover first', async () => {
  const wrapper = await fixture<HTMLDivElement>(html`<div>
    <lr-entity-chip text="Marie Curie">Physicist, 1867-1934</lr-entity-chip>
    <lr-dialog label="Newer" style="--lr-duration-base:0ms"
      ><button>Newer work</button></lr-dialog
    >
  </div>`);
  const chip = wrapper.querySelector<LyraEntityChip>('lr-entity-chip')!;
  const dialog = wrapper.querySelector<LyraDialog>('lr-dialog')!;
  const chipWrapper = chip.shadowRoot!.querySelector('.wrapper') as HTMLElement;
  const popover = chip.shadowRoot!.querySelector('[part="popover"]') as HTMLElement;
  try {
    ...
    // from the chip's local wrapper-scoped keydown listener.
    chipWrapper.dispatchEvent(new PointerEvent('pointerenter', { bubbles: true }));
    ...
    expect(popover.hidden, 'the chip popover is not topmost and must stay open').to.be.false;

    await sendKeys({ press: 'Escape' });
    await waitUntil(() => popover.hidden, 'the chip popover owns the second Escape');
  } finally {
