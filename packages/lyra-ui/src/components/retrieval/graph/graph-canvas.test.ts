...
describe("indexToPickColor / pickColorToIndex", () => {
  it("round-trips several indices through the encoded rgb() color", () => {
    for (const i of [0, 1, 42, 255, 4096]) {
      ...
      expect(
        pickColorToIndex(Number(match[1]), Number(match[2]), Number(match[3]))
      ).to.equal(i);
    }
  });
...
describe("drawGraphScene", () => {
  ...
  it("paints non-color dashed keyboard focus cues for links and community hulls", () => {
    const scene = {
      hulls: [],
      links: [],
      edgeLabels: [],
    ...
    const linkCtx = make2dContext();
    drawGraphScene(
      linkCtx,
      { k: 1, x: 0, y: 0 },
      {
        ...scene,
        keyboardFocusLink: { x1: 10, y1: 20, x2: 90, y2: 20, width: 1 },
      }
    );
    const linkPixels = linkCtx.getImageData(0, 16, 100, 9).data;
    expect(
      [...linkPixels].filter(
        (_value, index) => index % 4 === 3 && linkPixels[index]! > 0
      ).length
    ).to.be.greaterThan(0);

    const hullCtx = make2dContext();
    drawGraphScene(
      hullCtx,
      { k: 1, x: 0, y: 0 },
      {
        ...scene,
        keyboardFocusHull: { d: "M 20 50 L 80 50 L 80 80 L 20 80 Z" },
      }
    );
    const hullPixels = hullCtx.getImageData(15, 45, 70, 40).data;
    expect(
      [...hullPixels].filter(
        (_value, index) => index % 4 === 3 && hullPixels[index]! > 0
      ).length
    ).to.be.greaterThan(0);
  });
  ...
  it("keeps node labels physically after their anchor under inherited RTL", () => {
    ...
    for (let y = 0; y < 100; y += 1) {
      for (let x = 0; x < 100; x += 1) {
        if (pixels[(y * 100 + x) * 4 + 3]! > 0)
          firstPaintedX = Math.min(firstPaintedX, x);
      }
    }
...
describe("drawPickingScene", () => {
  ...
  it("assigns a distinguishable pick color to a hull region, ordered before links/nodes", () => {
    ...
    expect(
      pickColorToIndex(hullPixel[0]!, hullPixel[1]!, hullPixel[2]!)
    ).to.equal(0);
    expect(
      pickColorToIndex(nodePixel[0]!, nodePixel[1]!, nodePixel[2]!)
    ).to.equal(1);
  });
