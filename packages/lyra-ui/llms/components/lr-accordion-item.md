...
- **Import** `import '@aceshooting/lyra-ui/components/lr-accordion-item.js';` (stable tag alias; registers the tag)
...
- **Deprecated part** `base` since `8.2.3`; use part `::part(accordion-item)`; removal not before `10.0.0` — The accordion-item part names the outer wrapper explicitly; base remains on that same node during the compatibility window.
- **Optional peers** none
