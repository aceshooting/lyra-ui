...
Icon tags and attributes can be renamed mechanically, but icon names are a separate vocabulary.
Lyra's dependency-free default library contains only `add`, `check`, `close`, `search`, `menu`,
`chevron-left`, `chevron-right`, `chevron-down`, `calendar`, `command`, and `trash`; any other
migrated `<wa-icon>`, `<sl-icon>`, or `<sl-icon-button>` name renders no glyph. The migration CLI
reports `BEHAVIOR_REVIEW_REQUIRED` for those tags. Replace each name explicitly or register the
required vocabulary with `registerIconLibrary('default', { resolver })`.
...
**Which tags a per-tag entry registers.** Each stable per-tag entry
...
registration entries before defining `<lr-table>` itself. `custom-elements.json` has no field for
this — it declares one custom element per family source module, not per stable entry specifier —
so build tooling that needs to verify statically "does importing this specifier register that tag"
(a lint rule, a contract test, a bundler safety check) reads the generated `registrations.json`
instead:

```ts
import registrations from '@aceshooting/lyra-ui/registrations.json' with { type: 'json' };

const table = registrations.entries.find((entry) => entry.tag === 'lr-table');
table?.registers; // ['lr-empty', 'lr-pagination', 'lr-skeleton', 'lr-spinner', 'lr-table']
```

Each entry's `registers` is derived from the same transitive-import analysis
`scripts/check-component-dependencies.mjs` already performs (not a second hand-maintained list),
covers every published component, and is regenerated by `pnpm run registration-graph`.
...
| `<lr-entity-card>`              | — (extra) | Knowledge-graph entity dossier card — type badge, description, key/value property rows, degree, community chip, and a built-in "focus in graph" action (`lr-entity-select`); never fetches or focuses a graph itself                                                                                                                                                                      |
...
| `<lr-embedding-explorer>`       | — (extra) | Accessible SVG projection of host-provided embedding points with cluster coloring and a visible cluster legend, roving keyboard focus, and `lr-point-select`; it never filters points, computes embeddings, or owns dimensionality reduction                                                                                                                                                |
...
| `<lr-drop-zone>`  | — (extra)       | Drag-and-drop region wrapper with no file input of its own: makes any region (a composer, a whole viewport) a drop target with a themeable drag-over overlay, `accept`/size/count limits, and the same `File[]` events as `<lr-file-input>`                                                |
...
| `<lr-message-feedback>`                                                                | — (extra)                                                                               | Thumbs up/down for one assistant message with an optional inline reason-chips + free-text comment disclosure; emits, never persists — the host reflects a prior rating back via `rating`                                                                                                                          |
...
| `<lr-streaming-text-core>`                                                             | — (extra)                                                                               | Build-lean `<lr-streaming-text>` variant with identical behaviour for a consumer whose `languages` map covers every fenced-code language it streams, or that renders no fenced code                                                                                                                               |
