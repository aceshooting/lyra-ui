...
uses `package.json#packageManager` (`pnpm@12.4.1`). The package's supported engine remains
...
- `./scripts/ci.sh --platform-matrix` (or `--all`) runs the primary aggregate and then the exact
  ...
  Firefox (4 shards), and Safari (1 shard). Node 20 needs pnpm 10.34.5; Node 22 needs pnpm 12.4.1.
