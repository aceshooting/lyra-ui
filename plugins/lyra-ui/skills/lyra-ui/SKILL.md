...
name: lyra-ui
description: >
  Use when writing or reviewing code that imports @aceshooting/lyra-ui, uses any lyra-* custom
  ...
  and the wa-*/sl-* -> lyra-* migration paths.
---
...
under the `lyra-` prefix, ships its own design tokens, localization, RTL support, and (for form
...
```html
<lyra-combobox label="Fruit"></lyra-combobox>
```
...
Never guess a `lyra-*` component's attributes, slots, events, parts, or CSS custom properties from
...
- **Theming only through `--lyra-theme-*` custom properties.** Never hardcode a color, spacing, or
  font value in a way that fights the token system — override the relevant `--lyra-theme-*`
  property at any ancestor instead. Every component works with zero configuration and rethemes
  fully from that one mechanism.
- **Events are `lyra-*`-prefixed custom events** (e.g. `lyra-change`, `lyra-input`), except where a
...
Selected components document a Web Awesome-compatible surface 1:1 under the `lyra-` prefix — for
those, migration is a mechanical `wa-*` → `lyra-*` tag/import rename (see the "Web Awesome mirror"
note in each component's `llms-full.txt` section for exactly which ones, and any intentional
differences). Shoelace users get a similar but best-effort `sl-*` → `lyra-*` map, since Web Awesome
